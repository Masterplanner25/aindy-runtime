...
**Rationale:** `AINDY/platform_layer/sandbox_runner.py` (lines 41-43) defines
...
documents the full platform matrix (Linux, Windows, macOS, Other) and correctly
states that strong-sandbox and `hostile-third-party` profile support is Linux-only.
Non-Linux platforms degrade to `insecure_dev_subprocess` or containerized OCI
(without Linux kernel hardening controls). This limitation is correctly documented,
correctly enforced in code, and correctly surfaced through the operator-visible
platform matrix. Closing this gap requires platform-specific sandbox runtimes —
Windows Containers or WSL-mediated isolation, macOS Sandbox entitlements or
Virtualization.framework — which are infrastructure investments outside this
milestone.

**Why deferred:** The Linux-only limitation does not affect the tiered model's
binary boundary claim. Tier 2 is externalized on all platforms; the strength of
that externalization differs by platform, and the matrix honestly reports this.

**Condition to reopen:** A non-Linux host platform gains a supported sandbox runner
type with assurance class `container-grade-sandbox` or higher, verified through the
shared worker policy certification suite.
...
**C2 — Cross-platform strong sandbox (Gap 4)**

**Why deferred:** `STRONG_SANDBOX_SUPPORTED_HOST_PLATFORMS = (PLATFORM_LINUX,)` is
the correct current state. Non-Linux platforms correctly degrade and the platform
matrix is published. Closing this requires platform-specific sandbox runtimes on
Windows (Windows Containers, WSL-mediated isolation), macOS (Sandbox entitlements,
Virtualization.framework), and Other — all requiring new infrastructure and runner
implementations outside this milestone.

**Where to track:** Open an issue: "Add production-grade sandbox runner for non-Linux
hosts."

**Condition to reopen:** A non-Linux host platform produces a sandbox runner type
passing the shared worker policy certification suite with assurance class at or above
