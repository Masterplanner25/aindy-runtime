...
> pre-split monolith archive. This is the **historical** changelog of the
> combined monolith. Entries below predate the runtime/apps split, so the file
> paths they reference (e.g. `routes/`, `services/`, `apps/...`,
...
* **Architecture (Fix 6):** Earlier work replaced a hardcoded Windows path in `routes/memory_router.py::execute_nodus_task()` with an environment-based Nodus import path. That source-tree model has since been superseded by the pip-installed `nodus` package.
