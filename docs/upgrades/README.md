...
| **2.22.0** | 2026-09-21 | [`APP_HANDOFF_v2.22.0.md`](./APP_HANDOFF_v2.22.0.md) | **yes** — `system_events.source` 32 → 128 | **`0020`** | **`2026-09-20`** | no | **`bootstrap-schema --reconcile` required outside compose** (exit 3; the app's entrypoint branches on it). Four app FRs shipped: **FR-38** the authority gate on `nodus_vm` (the resume route takes `{decision, note}` — the app's resume surface must pass it through), **FR-39** providers get `user_id` as a string and no `db`, **FR-40** the `warn` recipe holds on `nodus_vm`, **FR-41** the width + an over-width source refused at pipeline entry. **FR-37** ui-kit 2.1.0 (app bumps + deletes workarounds). **nltk/textstat removed in the release after this, ≥ 2026-10-01.** `[mcp]` no longer caps `mcp<2`; nodus-lang 5.14.0. DEC-067..070. Asks: re-run the manufactured denial on `nodus_vm`; pass the resume body through. |
| 2.21.0 | 2026-09-18 | [`APP_HANDOFF_v2.21.0.md`](./APP_HANDOFF_v2.21.0.md) | none | `0019` | `2026-09-16` | no | Pin bump + rebuild. **★ Behaviour change: a pipeline route handler that raises is now ROLLED BACK before `execution.failed` (#721) — a write-then-raise-without-commit route used to keep the write; shared-session test suites must test 4xx routes on a private engine.** **`user.id` removed from `syscall.*` spans (dated in 2.20.0) → `enduser.id`.** `call_tool` gains an optional third argument; `replayed: true` on continued runs (#722). `EGRESS-INPROC-1` (flag-gated), `AUTHORITY-LIFETIME-1` (stale tokens refused `permission`), `AUDIT-CORRELATION-1` (additive event keys). DEC-046/047 accepted. **New ask: declare `nltk` + `textstat` in the app's own pyproject (`PACK-DEBT-6`) — the runtime will drop both pins after.** |
| **2.23.0** | 2026-09-23 | [`APP_HANDOFF_v2.23.0.md`](./APP_HANDOFF_v2.23.0.md) | none | `0020` | `2026-09-20` | no | Pin bump + rebuild, **no schema step** (so the `Upgrade Path Guard`'s meaning rests on its **negative control** — say so when citing it). **FR-42**: the `agent_capability_mappings` FK WARNING on every non-agent token mint is gone; the token now carries `mapping_recorded` (#750, DEC-071). **FR-15** (distributed mode only): a FOLLOWER api used to queue a woken resume nothing drained — `woken: true` and a run `waiting` forever — now forwarded to the dispatcher (`aindy_scheduler_resume_forwarded_total`); and a worker that never opened its scheduler dropped every bus event past 1000 (#751, DEC-072). **New opt-in, default OFF: `AINDY_TOOL_IDEMPOTENCY_STRICT`** — without it, concurrent duplicates of an `EXACTLY_ONCE` TOOL all execute (measured: 5 concurrent sends → 5 real messages; 8-way → 8 runs); with it, 1 run + 7 replays (#755, `IDEM-13`). Dependency group #749 (`cloudpickle` newly pinned). **`nltk`/`textstat` drop after 2026-10-01 — declare them now.** Asks: the `nodus_vm` denial re-run (third release); a strict-mode window if you have concurrent `EXACTLY_ONCE` tool duplicates. |
| **2.24.0** | *unreleased* | [`APP_HANDOFF_v2.24.0.md`](./APP_HANDOFF_v2.24.0.md) | none | `0020` | `2026-09-20` | no | Pin bump + **`@aindy/ui-kit` 2.1.1** + rebuild, no migration — but **FR-43**: `bootstrap-schema` now sees a column widening (a stack still at `system_events.source` 32 exits **3**; corrects the 2.22.0 handoff's exit-3 promise). **FR-44** resume arms the wait (`run_status` may be `"waiting"`), **FR-45** runtime adapters stamp, **FR-46** step references (flag off), **IDEM-14** tool key per step (`execute_tool(step_index=)` — stubs), **FR-47** ui-kit `timeoutMs` (a caller's abort is no longer 408). nodus-lang 5.15.0. Asks: FR-46 evidence run, FR-44 two-process re-run. |
