...
| ★ CORRECTION (2026-09-23) — Claw canNOT exercise this row | measured: Claw's effects go through `execute_tool` → the TOOL gate, and this flag is read in `syscall_dispatcher` only. The tool path has no strict mode at all (`IDEM-13`), so no amount of Claw traffic moves this row. Its real traffic DOES exercise the tool gate: 10 sequential retries replayed with nothing sent; 5 concurrent sends of one key delivered 5 messages (`degraded` ×5). This row still needs a SYSCALL consumer |
