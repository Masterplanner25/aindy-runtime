...
> **Note:** `summarize_tasks` must be a registered node on your server. For this tutorial, any registered terminal node works — the flow demonstrates the routing, not the node logic. See the published **aindy-sdk** package docs to add custom nodes _(the `docs/sdk/` docset was not migrated into this runtime repo)_.
...
> **Runtime note (validated 2026-06-27):** `extra` is an SDK convenience field;
> the `sys.v1.memory.write` syscall schema is `content` (required), `tags`,
> `node_type`, `path` (see `AINDY/kernel/syscall_registry.py`). Anything passed
> as `extra` is carried by the SDK, not validated by the v1 syscall schema.
