...
|---|---|---|---|
| [`AUDIT_CORRELATION_DESIGN.md`](./AUDIT_CORRELATION_DESIGN.md) | `AUDIT-CORRELATION-1` | **Design filed 2026-09-17** — four decisions pending | Re-measures the three joins: `ExecutionAuthority` does not exist, (2) already fell out of `EXEC-ENV-BIND-1`, and `EffectRecord` has no `trace_id` — the real join is unindexed JSONB. Three additive payload keys on `syscall.executed`; **no FK in either direction**; the join is time-bounded by retention on both sides. |
| [`AUTHORITY_LIFETIME_DESIGN.md`](./AUTHORITY_LIFETIME_DESIGN.md) | `AUTHORITY-LIFETIME-1` | **Design filed 2026-09-17** — four decisions pending | Authority ends with the run: `CANCEL-REACH-1`'s cached own-session fail-open read widened to every terminal status, sticky once terminal, at the same two sites; the HMAC check stays stateless. **A `waiting` run keeps its authority.** |
...
| [`EGRESS_INPROC_DESIGN.md`](./EGRESS_INPROC_DESIGN.md) | `EGRESS-INPROC-1` | **Design filed 2026-09-17** — four decisions pending | **§1 finds a defect the entry did not name: the isolated tool branch returns before `egress_scope`, so a declared-isolation tool has no egress enforcement at all.** Decision once at `execute_tool`; enforcement per provider; the worker installs the guard from its payload; `env_applied.network` reports the mechanism. |
| [`EVENT_OUTBOX_DESIGN.md`](./EVENT_OUTBOX_DESIGN.md) | `EVENT-OUTBOX-1` | **Design filed 2026-09-17** — three decisions pending | The buffer engages only inside a request pipeline (every other path already co-locates), and the capture read is a score, not a gate. Inside a pipeline the event row is added to the handler's session without commit and rides its next commit; a raising handler leaves no row. No outbox table. |
