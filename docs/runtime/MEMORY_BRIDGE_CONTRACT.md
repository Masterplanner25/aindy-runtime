...
- HMAC signature validation is no longer enforced on bridge write routes.
- JWT (`Depends(get_current_user)`) is the write guard.
- `permission: TracePermission` is accepted but ignored for backward compatibility.
...
### Current Implementation
- Invalid permission: ignored (JWT governs access).
