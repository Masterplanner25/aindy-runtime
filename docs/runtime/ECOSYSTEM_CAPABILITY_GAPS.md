...
| **G6** | **Execution-path test coverage.** `worker/` and the Surface-B path are low/zero; integration tier mocked in CI. | **Debt** | **P2** | `ECOGAP-6` (new) | internal hygiene; no external leader |
