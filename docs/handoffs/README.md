...
| [`SDK_HANDOFF_memory_tree_flat.md`](./SDK_HANDOFF_memory_tree_flat.md) | `aindy-sdk` | 2026-09-13 | `MemoryAPI.tree()`'s docstring promises `data["flat"]`; the runtime returns `tree` + `node_count` and never has. Plus two notes: `extra` on `write()` works by the absence of `additionalProperties: false`, and there is no whoami route (the tutorials decode the JWT `sub`). | **Open.** Found by the tutorials correctness pass; the runtime's own Tutorial 1 had inherited the wrong key. |
