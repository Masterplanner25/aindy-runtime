...
platform models only. App-owned models are loaded later through app bootstrap
callbacks that invoke ``register_models``, so the runtime can provide shared DB
infrastructure without importing ``apps.*`` directly for domain ORM types.
"""
