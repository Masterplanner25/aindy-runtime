...
def register_node(request: Request, body: NodeRegistration, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), _scope: None = _REQUIRE_PLATFORM_ADMIN):
...
def list_nodes(request: Request, current_user: dict = Depends(get_current_user), _scope: None = _REQUIRE_PLATFORM_ADMIN):
    def handler(ctx):
...
@limiter.limit("60/minute")
def get_node(request: Request, name: str, current_user: dict = Depends(get_current_user), _scope: None = _REQUIRE_PLATFORM_ADMIN):
    def handler(ctx):
...
@limiter.limit("30/minute")
def delete_node(request: Request, name: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user), _scope: None = _REQUIRE_PLATFORM_ADMIN):
    def handler(ctx):
