...
## FR-22 — 51 runtime routes were documented app-side and guarded by nobody

**Status: FIXED 2026-08-22.** Filed by the app team the same day, 🟡 drift, and the cheapest of
the six they raised this week. Their `docs/api/API_REFERENCE.md` is guarded by
`scripts/check_api_reference.py`, which boots the app and diffs the live route tree — but scoped
to `APP_PREFIX = "/apps/"`. The document also carries ~51 runtime-owned entries (43 `/platform/*`,
5 `/health`, 2 `/auth`, 1 `/ready`) that **nothing checked**. Two halves of one file with
different guarantees: theirs cannot drift without CI failing, ours could drift silently for
months.

**★ They were right to refuse the obvious fix, and said why: extending their guard over our
routes would make the app responsible for policing a surface it does not own, and the real
problem stops being visible to whoever could fix it.** That is FR-20's lesson applied by the
people who learned it. Worth keeping as a rule for handoffs in both directions.

**Shipped:** `AINDY/route_inventory.json` — every `(method, path)` served in the `runtime-only`
profile, with tags — generated by `scripts/check_route_inventory.py` and pinned by
`tests/unit/test_route_inventory.py` (which runs in `Runtime Contracts`, so no new required
check was needed). It ships in the wheel already: `[tool.setuptools.package-data]` globs
`AINDY/*.json`, so a consumer reads the surface **for the version they installed** without
booting anything.

**★★ The finding that came out of building it, and it corrects a premise on BOTH sides: the
`/apps/` prefix is not an ownership boundary. 35 routes under `/apps/*` — coordination, memory,
agent — are served by the runtime ALONE, with no plugins loaded.** So their guard, scoped to
`/apps/*`, is already enforcing over runtime-owned routes, and the "app half / runtime half"
split in their file does not fall where the prefix suggests. The useful consequence is better
than what they asked for: subtracting this inventory from their booted surface *derives* the
genuinely app-owned set, replacing a curated list with a computed one.

**★ Definition choice: boot profile, not path prefix.** Everything served in `runtime-only` mode
is runtime-owned by construction, so the inventory needs no hand-maintained ownership
classification — which is exactly the artifact that would drift the way the thing it describes
was drifting. (`docs/runtime/ROUTE_OWNERSHIP_INVENTORY.md` remains a *classification* doc — core
/ operator / candidate — and is a different question from *what is served*.)

**★ Two deliberate exclusions, because absence has to mean something precise.** The legacy alias
surface is off (`AINDY_ENABLE_LEGACY_SURFACE`): the file publishes the supported surface, not
compatibility shims — and note `tests/conftest.py` turns those aliases ON session-wide, so the
test builds its own app rather than using the shared fixture, or it would compare against 51
extra routes. And there is **no version field**: the file is committed, so a stamped version
would make every release bump edit it and every concurrent branch conflict on it.

**★ The check runs in both directions on purpose.** An added route failing the build is mild; a
route silently *leaving* the published surface is what breaks a consumer pinned to the file, and
that direction is the one a one-way "is everything documented?" check would miss. Mutation-tested
2/2 — dropping an entry from the inventory and hiding a route from the schema each fail it.
