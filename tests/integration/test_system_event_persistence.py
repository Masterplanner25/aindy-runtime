...
────────────────────────────────────────────────────
Integration tests: SystemEvent persistence and scheduler notification.

Verifies the end-to-end path from queue_system_event / emit_system_event
through to database persistence and scheduler wake-up.

No app-domain imports.  All tests use db_session (per-test transaction
rollback) and test_user from tests/integration/conftest.py.
