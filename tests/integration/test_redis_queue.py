...
──────────────────────────────────────
Integration tests for the distributed Redis queue backend.

Requires a live Redis instance (set REDIS_URL).  The redis_backend fixture
provides an isolated namespace; all keys are deleted on teardown.
