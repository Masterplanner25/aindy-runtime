...
- **RTR-\*** — Runtime Roadmap (`RTR-1..8`): Nodus-first execution substrate + runtime primitives, consolidated from the app-side evolution docs and validated against source 2026-06-29. **BUILD** (greenfield): RTR-1 (Nodus `register_nodus_workflow`/VM-backed agent path — keystone), RTR-5 (autonomous execute-window), RTR-6 (reasoning event model). **HARDEN** (built, finish gaps): RTR-2 (durable queue exists, opt-in — see SYSMAX-1/TIER3-10), RTR-3 (AgentRun↔FlowRun unification), RTR-4 (delegation core built), RTR-7 (execution-causality canonical via `EventEdge`; "RippleTrace" name dissolved into the primitive by design). RTR-8 (PyPI) is stale/CLOSED. Source: `TECH_DEBT.md` RTR-*.
...
- **SDK-SYSCALL-GRANT-1** — Open (2026-07-05): `/platform/syscall` grants only `DEFAULT_NODUS_CAPABILITIES`, so the SDK's `client.flow.run` (cap `flow.run`) is denied for all callers and `client.events.emit` (cap `event.emit`) is denied for API-key callers. Capability↔scope bridge needs reconciling. Distinct from TIER3-V2V3 (domain-scope *gate*, closed). Surfaced closing the `sys.v1.execution.get` gap.
