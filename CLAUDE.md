...
- **FLAKY-1 — `test_platform_only_startup::test_platform_only_registers_runtime_agent_defaults` fails ~50% of full-suite runs, and it gates merges. ★ MITIGATION SHIPPED 2026-08-15 (FR-11 budget 10s→30s, configurable), but deliberately NOT closed — the mechanism was never confirmed, only forced, so this is a well-argued mitigation not a demonstrated fix. Closing needs ~10+ consecutive clean `-m runtime_only` runs (4 clean in a row happen ~6% of the time by luck against a 50% base rate) or a captured traceback.** Measured over six `tests/unit/` runs across two worktrees: `main` 1-fail/1-pass, feature branch 2-fail/2-pass. **Pre-existing, not branch-caused** — and two earlier readings of that same data were wrong, recorded so they aren't repeated: two consecutive failures read as *deterministic* (a third run refuted it), and correlation with a branch adding neighbouring files read as *caused by them* (a second `main` baseline refuted it). **Don't conclude anything here from fewer than ~4 runs per tree.** It passes 100% in isolation. It carries `runtime_only`, so `Runtime Contracts` runs it — and with all ten checks now required, a ~50% test blocks merges at random and trains people to re-run red. **★ Confirmed 2026-08-15 under `python -m pytest tests -m runtime_only -q` — byte-for-byte the `Runtime Contracts` command — so an earlier guess that the marker-filtered collection might dodge the ordering is WRONG.** The required check will red-line at random; it simply hasn't yet. (The same commit passed in CI minutes later — that's what a coin flip looks like, and why "it was green" proves nothing here.) **Leading mechanism = FR-11's hardcoded 10.0s subprocess budget** (`runtime_callback_host.py:43`): forcing it reproduces `RuntimeError("runtime callback command timed out")` from `registry.py:433`, propagating through the evaluator assertion at `test_platform_only_startup.py:229`. **Strongly indicated, NOT confirmed** — that signature was *forced*, not captured from a natural failure. **★ Why no natural traceback exists: all 3 observed failures ran as `pytest ... -q | tail`, which discarded the traceback** and kept only the summary; by the time runs switched to `--tb=long`-to-file the flake didn't recur (2 attempts, both clean). Tally: 7 runs, 3 fail / 4 pass. **When chasing a flake, never pipe the run through `tail`.** Ruled out: stale `_runtime_callback_invocations` (the fixture restores 57 registry globals incl. that one). **Blast radius is two files** — `test_extension_ownership.py` asserts on the same boundary and is exposed to the same race. **★ Do NOT fix by dropping the marker — that's CI-MARKER-1 in reverse: green because it stopped running.** **Open.** Source: `TECH_DEBT.md` FLAKY-1.
