...
- **NODUS-WARMPOOL-1** — CLOSED 2026-07-19. Warm worker pool (`AINDY_NODUS_WARM_POOL`, default off) + boot-allowance clock split; falls back to a fresh subprocess on any fault. **★ Its soak had a precondition: on `nodus-lang <=5.0.2` a reused worker leaked guest memory between tenants, so the pool's own safety claim was false. Fixed at 5.0.4.** **★ SOAKED 2026-08-19 — `tests/unit/test_soak_warm_pool_contention.py`, 6 concurrent callers vs a pool of 2 against REAL workers, mutation-tested 4/4. The existing suite ran entirely against FAKE processes and deferred end-to-end to "app-side PG-tier integration" — i.e. to a consumer that does not exercise it. CI has had the flag on for months, but functionally and SEQUENTIALLY; that is not evidence about contention.** Remaining is the default flip, now a decision rather than a measurement.
