...
- **WAIT-TYPED-CONTRACT-1** — *(MAF research)* **PHASE 1 SHIPPED 2026-09-15; open for the request-EU path + the guest-wait product decision.** A resume payload was **trusted, not checked** — the asymmetry: `dispatch()` validates syscall inputs, the wait path (outside data, after a restart) validated nothing. Now a WAIT may declare `resume_schema` in the **dispatcher's own dialect**; the runner records `state["__pending_request"]` (only when declared); `route_event` checks BEFORE injecting or waking; the route answers **422** and the run stays waiting; guest key `nodus_wait_resume_schema`; counter `aindy_flow_resume_payload_total{outcome}`. **★ The filing's second claim was stale: the node binding EXISTED (`current_node` + run-scoped wake); only the schema did not.** **★ Liveness: an untyped `{}` resume CONSUMES the wait and fails the run one step later.** **★ State key, NOT a column (no `--reconcile`) — so the DUR-4 fold resumes UNTYPED, pinned. ★ Seeding `nodus_output_state` back: DECLINED.** Source: `AINDY/core/pending_request.py`.
...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.16.0.md` — pin bump + rebuild, no schema step, no required code change. Closes the app's FR-30 (`EU-FINALIZE-UNCOMMITTED-1`, #673): a request's `execution_units` row now actually reaches `completed`/`failed` — it never had, on any release; FR-29's rehydration-seed addendum stops the boot-time FK noise. **★ Verified live by the app team 2026-09-16 (§6): zero `executing` after 18 requests, boot FK noise 10 → 0; no FR. Its §2 over-claimed their pause hook's commit path — corrected; the finding it led to (`TASK-EU-NOT-PERSISTED-1`) is theirs.** **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** — the 2.11.0 handoff's correction records the cwd trap that bit both repos |
