...
- **FLOW-PARALLEL-1** — **CONFLICT HALF #569; SCHEDULER PHASE 0 SHIPPED 2026-09-08 (#603); open for 1–4.** No fan-out/join yet, so apps needing parallelism route AROUND the engine, losing history/retry/quota. **★ An undeclared double-write RAISES — no default, since last-write-wins is right for a latest-reading cell and wrong for a counter; disjoint writes need none. ★ Determinism is the property, not merging: `last_write_wins` resolves in DECLARATION order.** **★ Phase 0 widened the TRANSACTION, not the concurrency: ordinals allocated per SUPERSTEP at the barrier (the `max()+1` it replaced documented the very precondition fan-out removes), and the merge moved OUT of per-node status handling — called per node it can only ever see one patch, which IS completion order.** **★★ Before phase 1: branches CANNOT share the runner's session (`AGENT_WORKING_RULES` §5), and a width of N spends N connections from the budget SHARED with request handling (`SYSMAX-5`), so unbounded fan-out is an API availability regression.** Design: `docs/runtime/FLOW_PARALLEL_DESIGN.md`.
...
- **AUTHORITY-NEGOTIATION-1** — **PHASE 0 SHIPPED 2026-09-08 (#600); open for 1–3.** A denied capability terminates the step; approval is whole-plan, so recovery discards the durable state nine good steps accumulated. Bounded (exactly once), **downgrade-only**, recorded. **★★ DO NOT CLOSE ON PHASE 0 — `register_tool(..., degraded_variant=)` is validated and consulted by NOTHING; a declaration nothing reads is `G4a` and we already have one.** **★ The TOOL declares it, never the plan — the thing being constrained must not choose its own constraint (`env_spec`'s rule); no chains, so the bound is structural.** **★ Validation SPLITS of necessity: local checks in the decorator, the cross-tool rules in a STARTUP sweep — a forward reference is legitimate and capability definitions load later. ★ It reports UNEVALUABLE apart from FAILED, or an unloaded provider reads as an operator's typo (variant 10 reversed).** **★ `amend_token` is NOT needed here; `simulate` substitution is EXCLUDED — it belongs at the WAIT gate.** Design: `docs/runtime/AUTHORITY_NEGOTIATION_DESIGN.md`.
