...
- **TENANT-FROZEN-SHALLOW-1** — `TenantContext` is `@dataclass(frozen=True)` and documented "Immutable", but `capability_scope` is a `list` and `frozen` doesn't deep-freeze: `ctx.capability_scope.append("admin.everything")` succeeds and `assert_capability` then passes. **No known exploit path** (nothing in `AINDY/` mutates it) — a weak invariant, not a live breach. Fix is a one-line field change to `tuple[str, ...]`; the suite already pins both current and fixed shapes. **Adjacent, pinned:** `TenantContext.validate_memory_path` *rejects* the exact tenant root `/memory/t1` (requires the trailing slash) while `memory_address_space.validate_tenant_path` *accepts* it — two tenant guards, two answers for one string. **Open.**
