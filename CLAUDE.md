...
- **COST-GOVERNOR-1** — **METER SHIPPED 2026-09-03 (#563, #564); ADOPTED 2026-09-08 (app #321); ★ PHASE 3 SHIPPED 2026-09-13 (#635): tokens are a RESOURCE DIMENSION with a subject — accrued in `ResourceManager` per unit and per tenant window, `aindy_llm_calls_total{attributed=run|unit|tenant|none}` shows what a budget could reach.** **★★ Planning runs BEFORE the AgentRun row exists, so only the TENANT can be named there — a per-run budget cannot catch a runaway planner.** **★ `observed_unit` is observed NOT admitted: wrapping a run in `owned_execution` would make it one admitted, 100-syscall unit by side effect (`EXEC-ENV-BIND-1` ph4's decision).** ★★ PHASE 2 DONE 2026-09-13: one planner call on the live container (2.12.0, path printed) → `{anthropic, claude-opus-4-8}` prompt 2021 / completion 223. THE GOVERNOR IS UNBLOCKED. Scope: `LLM_SEAM_ADOPTION_SCOPE.md`. Source: `TECH_DEBT.md`.
