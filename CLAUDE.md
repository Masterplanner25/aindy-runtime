...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. **★ Corrected 2026-08-18: the filed entry said THREE and missed `nodus_lang_workflow`, a SQLite `LocalWorkflowStore` reimplementing this runtime's whole durability vocabulary.** Failure is crash-only: a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ Worse than untracked — UNCONFIGURED, and it loses data: `AINDY/` has ZERO references to it, so the store roots relative to the worker's CWD — which no spawn path sets — in a directory with NO compose volume.** Store 4 IS injectable (`runner.py:437`). **★★ MEASURED 2026-09-09: 623 records, 5.4 MB in the repo root — CWD-rooting was inference, now evidence. NOT live (all terminal), but nodus 6.0.0 flips the store JSON→SQLite and neither sees the other. ★ "SQLite `LocalWorkflowStore`" was WRONG — Local IS the JSON class.**
