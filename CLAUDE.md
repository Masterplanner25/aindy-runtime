...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.15.0.md` — pin bump + rebuild, no schema step, no required code change. Closes the app's FR-29 (`WAIT-DETECT-SHAPE-1`, #670): a request that reads or starts a waiting run no longer parks its own execution unit; `nodus/run`'s envelope says `success` + `data.status: WAITING`; their `flow_run_get` result key can STAY. **★ Its §4 is the first live run of #670 — read `execution_units` after Tutorial 2, not the tutorial's output — and carries the optional one-off `UPDATE` that retires the `route` rows every prior release leaked.** **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** — the 2.11.0 handoff's correction records the cwd trap that bit both repos |
