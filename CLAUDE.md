...
- **KEY-SCOPE-ESCALATION-1** — **Re-levelled P0 -> P2 2026-08-18: the hole is closed; what remains is not one.** **security; core FIXED 2026-08-16 (#463), broader hole still open.** `POST /platform/keys` checked only that a requested scope *exists*, never that the caller could grant it. **Demonstrated end-to-end on real PG** from a `flow.read`-only key: mint `platform.admin` (201) → `GET /platform/admin/users` (200) → **promote own user row to `is_admin` (200)** — which *survives revoking the key*. **Root cause: `require_platform_admin_access` returns any `auth_type=="api_key"` unconditionally**, gating all 56 `/platform/*` routes, with a docstring claiming *"scope enforcement happens per-endpoint or per-syscall"* — `keys_router` had none (the `ROUTE-AST-UNWIRED-1` shape: the defect is the claim). Fixed by a **delegation rule** — `grantable_scopes()`: a key grants only its own scopes, a session only its derived ones, `platform.admin` anything (deliberate — it already reaches user promotion). 403 `scope_not_grantable`; unknown scope stays 422. **★ SQLite CANNOT reproduce it: `platform_api_keys.scopes` is a PG `ARRAY`, so the ORM insert dies at the driver AFTER the gate passes — a 201 shows up as a 500 and reads as an unrelated bug.** **★ SECOND HALF ALSO FIXED 2026-08-16 (#465), and it was worse than filed:** probing what a `flow.read` key REACHED found **46 of 53** `/platform` routes open — DLQ drain (200, drained it) and **`POST /ops/rotate-secret-key` (200, rotated the platform signing key)**. **That is auth compromise, not vandalism: the caller picks the new key, so they can then forge tokens for any user incl. admin — and #463's delegation rule does NOT touch it** (it bounds what a key may *grant*, never what it may *do*). **★ The 400 that nearly hid it:** a first probe sent a `new_key` equal to the active `SECRET_KEY` and got 400 — which reads as refusal but is `same as the current active key`, raised AFTER authorization. **A status-code-only probe mis-reads validation as authorization**; the test uses a distinct valid key so it can't pass against unfixed code. Fixed with **per-endpoint scopes on 44 routes** (`platform.admin` keys/queue/nodes/observability/flow-runs/flow-CRUD/rotation; `webhook.manage` webhooks; `flow.execute` nodus; `execution.read` tenant usage). **The router gate was deliberately NOT tightened — `POST /platform/syscall` is the SDK's whole surface used with narrow scopes, so requiring `platform.admin` there breaks every SDK caller**; it and `GET /syscalls` stay route-ungated by decision (authority is per-syscall), pinned by equality. **Zero JWT impact** (parent gate already required `is_admin`; admin sessions derive both scopes). Still open: the two admin guards disagree about what an API key is — now tidiness, not a hole. Source: `TECH_DEBT.md` KEY-SCOPE-ESCALATION-1.
- **HTTP-SCOPE-GAP-1** — **Re-levelled P0 -> P2 2026-08-18: route coverage is complete; the remainder is a design question, not a gap.** the capability model does not reach the runtime's own front door. **`Depends(enforce_api_key_scope)` on 7 of 147 route decorators**; everything else (all of `/memory/*`) depends only on `get_current_user` — identity, not authority. `memory_router.py` reaches effects with **zero** dispatcher references. **★ CLOSED (first half) 2026-08-16 (#449): a JWT no longer bypasses scopes.** A session carries `session_scopes` derived from `User.is_admin` **per request, not from a token claim** — so no session is invalidated (unlike 2.0.0's `purpose`) and a grant/revocation lands on the next call. Ordinary = `flow.read/execute`, `memory.read/write`, `agent.run`, `execution.read`; admin adds `webhook.manage`, `platform.admin`; **neither includes `memory.delete`/`event.emit`**. **Ships ENFORCING, not default-off, because the blast radius is countable: only 7 of 147 decorators enforce anything, and all three scopes they need are in the ordinary set — pinned by a source-scanning test so a future enforcement an ordinary user can't satisfy fails in CI, not in a browser.** Hatch: `AINDY_JWT_SCOPE_ENFORCEMENT=0`. **★ SECOND SLICE CLOSED 2026-08-16 (#462) — all 22 `/memory/*` routes now gated** (reads `memory.read` OR `memory.write`; writes `memory.write`; `/nodus/execute` + `/execute` + `/execute/complete` **`flow.execute`**, since they run caller-supplied code and filing that under `memory.write` makes "may I remember" and "may I run" one permission). Counted on a **booted app, not the source: 29 of 126 registered routes gated, was 7** (126 registered ≠ 147 decorators — stacked decorators + app-profile-only routers; don't mix the two in one ratio). **★ `enforce_api_key_scope` gained ANY-OF alternatives** so a read gate accepts `memory.write`, matching `_DISPATCH_CAPABILITY_SCOPES` — otherwise one key reads via `/platform/syscall` and 403s on `GET /memory/nodes`. **★ Adding that form silently BLINDED the safety scan**: `test_every_enforced_scope_is_held_by_an_ordinary_session` regexed `enforce_api_key_scope\(Scopes\.([A-Z_]+)\)`, so a 2-arg 2-line call matched nothing and the test passed on a shrinking sample — now paren-balanced with a liveness control. **★ And the first coverage draft was vacuous: scanning `app.routes` for `/memory` finds ZERO** (FastAPI ≥0.137 keeps a lazy `_IncludedRouter`) — "nothing ungated" was true of an empty set; only a `checked >= 20` floor caught it. Use `_iter_api_routes` + endpoint `__module__`. Blast radius measured: no first-party API-key caller hits `/memory/*` (SDK `client.memory.*` is `MemoryAPI(self.syscalls)` → `/platform/syscall`). **★ CORRECTION 2026-08-16: the "97 of 126 enforce nothing" figure shipped with #462 was WRONG and alarming-direction wrong** — it walked each route's own `dependant`, which **excludes dependencies declared on the router it was included into**, and the `/platform` tree carries `Depends(require_platform_admin_access)` on the parent `APIRouter`. True census of 126: **29 scope-gated, 56 admin-gated, 21 public, 20 identity-only.** *Method rule: accumulate router `dependencies` down each nesting level; a per-route walk under-reports enforcement and `_IncludedRouter` hides the nesting.* **★ THIRD SLICE CLOSED 2026-08-16 (#464): the last 18 identity-only routes gated** — `coordination_router` 13 + `platform/agents_router` 5. **Three gates, not one:** `agent.run` (agent surface), `execution.read` (`/runs`, `/runs/{id}/children`, `/conflict/run`), `memory.read` OR `memory.write` (`/memory/shared`, `/conflict/memory` — they query `memory_nodes`; **gating those on `agent.run` would make the agent router a second door onto memory**), asserted in BOTH directions. **No `agent.read`/`agent.manage` invented** — a finer grain obliges every consumer to grant a scope answering no question they ask today. **★ `/platform/agents` never inherited the `/platform` admin gate** (included on the app directly, not via `platform_router`) — which is why FR-12b works for ordinary users AND why it had zero authority check; owner scoping still does what a scope cannot. **Census: 47 scoped / 56 admin / 21 public / 2 identity-only of 126.** The 2 (`/auth/logout`, `/auth/password/change`) act only on the caller's own account and are pinned **by equality**, so gating them also fails CI. **Gotcha: `coordination_router` is registered TWICE in a booted app** (`APP_ROUTERS` under `/apps` + again via `get_routers()`) — dedupe any census by `(method, path)`. **★ FOURTH SLICE CLOSED 2026-08-16 (#465): the `/platform` tree** — 44 routes gained per-endpoint scopes (see `KEY-SCOPE-ESCALATION-1`). **Census now 91 scoped / 12 admin / 21 public / 2 identity-only of 126.** **★ The safety guard was REWRITTEN because it had become an argument for weakening `platform.admin`:** `test_every_enforced_scope_is_held_by_an_ordinary_session` demanded every gate be satisfiable by an ORDINARY session — right while every gated route was one an ordinary user should reach. Replaced by `test_no_route_enforces_a_scope_nobody_can_satisfy`: route-derived, two branches — ordinary-satisfiable **OR** admin-gated with an admin-derived scope. A gate failing both is a permission nobody can hold. **Nothing structural remains on the HTTP surface**; what's left of the entry is the non-coverage question it raised — `execution.read` conflates scope with data ownership, and a scope cannot answer *"may I read someone else's"*. Original decision: **DECIDED 2026-08-15 (owner): a JWT must NOT bypass scopes** — but a JWT carries no scopes today, so implementing that literally denies every session request. Decide where JWT authority comes from first (recommend: derive from the user row). **★ UNBLOCKED 2026-08-15 — the app team answered with their real call surface** (`RUNTIME_FEATURE_REQUESTS.md` → *Response to v2.1.0 §6*). Their UI is **two privilege classes sharing one JWT**, and the client already draws that line itself (`useAuth().isAdmin`, `<AdminAccessRequired />`) — **frontend-only today**, so deriving from the user row makes the server enforce a boundary the UI already draws rather than imposing a new model. Ordinary session: `memory.read/write`, `flow.read/execute`, `agent.run`, `execution.read`. Admin: those + `webhook.manage`, `platform.admin`. **Not needed at all: `memory.delete`, `event.emit`.** Two constraints they name: tie admin scopes to the **existing user-row admin flag** (two sources of truth for 'is this an operator' is worse than none), and **`execution.read` conflates scope with data ownership** — a scope cannot answer *"may I read someone else's"*, the same distinction `memory_agents_list` owner-scoping just hit. Roll out permissive→narrow, and **name the scopes in the handoff for the release that enforces them** or the UI fails as scattered 403s that read as a frontend bug.
...
- **ROUTE-EFFECT-BYPASS-1** — **CLOSED 2026-08-16: A+B (#460), C (#461), D (#462).** *(D was the scope enforcement on `memory_router.py`; it landed with `HTTP-SCOPE-GAP-1`'s second slice, and this line said "D open" for two days afterwards — the drift this section's maintenance rule exists to catch.)* `POST /nodes` → `sys.v1.memory.write` (gains at-most-once, since it is `EXACTLY_ONCE`) and `POST /recall` → `sys.v1.memory.read` now dispatch. **★ Trap found doing it: `memory.write` REPLACED the caller's `extra` rather than merging, so a naive rewire was silent data loss behind a 201** — fixed, with `execution_unit_id` still winning a collision so provenance is unforgeable. **C shipped `sys.v1.memory.link` (floor 23→24, `EXACTLY_ONCE` — a retry builds a SECOND edge). ★ It carries `memory.link`, which `memory.write` does NOT grant** — a syscall adding a mediation hop and no authority granularity just relocates the same undifferentiated power behind a longer call path; a test drives the dispatcher with a `memory.write`-only context and requires refusal. Tenant scoping moved into the syscall, and a foreign node reports **identically to a missing one** (else the route is an existence oracle for other tenants' ids). Route keeps 404-vs-422 rather than collapsing to 400. **Deliberately absent from `_DISPATCH_CAPABILITY_SCOPES`, so SDK `/platform/syscall` gets an empty grant and is denied** — publishing a `stable=False` syscall to SDK callers is the half you can't withdraw; 2 tests pin the omission as a decision. Adding it later needs its own `Scopes.MEMORY_LINK` (mapping onto `MEMORY_WRITE` would undo the split at the scope layer). **Still bypassing (1, pinned by a test): `POST /nodes/search` (uses `find_similar`+`min_similarity`; `memory.search` calls `recall` and accepts neither — rewiring would change SEMANTICS under cover of a mediation fix).** Was: **Four** `memory_router.py` routes reach effects with no dispatcher AND no scope check (`enforce_api_key_scope` count in that file is **0**). ★ Split from `HTTP-SCOPE-GAP-1` because the fix differs — a scope decorator on a route that skips the dispatcher still leaves the effect unmediated. **★ Measured smaller than that entry implies: 18 of 22 routes DO go through `_mem_run_flow` → `run_flow` → dispatcher.** The four: `POST /nodes` (write → `sys.v1.memory.write`, now `EXACTLY_ONCE`), `POST /links` (write → **no syscall exists**, net-new), `POST /nodes/search` and `POST /recall` (reads → existing syscalls). **★ The "18 are mediated" half is weaker than it sounds:** `run_flow` falls back to `_run_flow_direct` when `user_id` is absent — absent identity *skips* the boundary (that is `AUTHORITY-VALUE-1`). Same shape as the native-enforcement plan's W4 but far cheaper — the syscall channel already exists, so no new execution plane, no Rust, no Linux dev loop. Source: `TECH_DEBT.md` ROUTE-EFFECT-BYPASS-1.
- **CAPABILITY-PROVIDER-TIMEOUT-1** — **FIXED 2026-08-16 (#466).** Filed as `TEST-ISOLATION-REGISTRY-1` (wrong name, wrong cause) and then at P1 (**wrong severity too**). Real mechanism: `_load_capability_definition_providers` is reached from `check_tool_capability`, so **every tool capability check spawned a subprocess per provider** on a 30s budget; under CPU contention it timed out, the exception was swallowed into a `logger.warning`, and the capability set came back empty. **★ It fails CLOSED, not open** — `check_tool_capability` refuses a tool with no mapping (`if not required_capabilities and tool_name in TOOL_REGISTRY`), so the cost is *availability*, not a vacuous capability model. **Established by running it; the P1 filing had assumed the opposite.** **Measured: 10 lookups = 10 subprocess spawns / 56.4s before, 1 / 11.4s after — ~5.6s of subprocess PER tool capability check.** Fixed by caching each provider's bundle (called once, not per check), **never caching a failure** (a transient timeout would otherwise persist for the process lifetime), and logging at ERROR naming what it costs. **★ The cache lives ON THE PROVIDER OBJECT, not a module global** — a `_loaded` latch would need adding by hand to **two** registry-reset dicts, and forgetting either leaves a stale `True` that empties capabilities permanently, i.e. this bug reintroduced by its own fix; `_capability_definition_providers` is already in both dicts, so an object-attached cache is invalidated for free. **Rejected: adding it to `_STATEFUL_IN_PROCESS_CALLBACK_SURFACES`** — that set is for callbacks reading live in-process state, and `runtime_capability_bundle` returns a literal dict, so moving it would weaken a documented boundary for speed. **Residual: the first lookup per process still spawns one subprocess**, so a contended host can still fail it once; the budget is the honest next lever if it recurs.
...
- **SYSMAX-5** — **CLOSED 2026-08-16 (#453)**, was latent by construction. The scheduler thread pool is **smaller than the job count and was never sized deliberately**: APScheduler's default `ThreadPoolExecutor()` = **10 workers** vs **12 runtime jobs** in `platform-only` (the floor) plus **21** app `register_scheduled_job` sites → **~33 jobs on 10 workers** in a real deployment; app jobs are added with no `executor=` so all land on `default`. Two holders are unbounded: `scheduler_heartbeat_tick` holds a worker for a whole INLINE execution (~13 min in the FR-15 incident), and several maintenance jobs block up to `DB_POOL_TIMEOUT` (60s) under pool exhaustion (`RT-MEMTXN-LEAK-1`). Failure mode is a **maintenance brownout** — recovery jobs stop running exactly when the condition they exist to clean up is happening, and nothing raises. **FR-15 (b) protected `scheduler_wait_tick` by name with its own executor; the ratio itself is untouched.** ★ **Do not close by raising the number alone** — that moves the threshold without bounding either holder; re-read after FR-15 (a), which may reduce this to "dedicated recovery executor + emit pool saturation". **Fixed by isolation, not capacity:** three lanes — `default` (10), `recovery` (2) for the six jobs whose value peaks under saturation (incl. `queue_backend_reconnect`, where a dead backend + a saturated pool means the reconnector itself can't run), `waits` (1). **★ Raising `default` would have been the WRONG fix, not just a weaker one:** 30 DB connections are shared with request handling, so more scheduler threads starve the API instead (RT-MEMTXN-LEAK-1's shape) — a test pins total lane threads to half the DB budget so that trade can't be made accidentally. Plus `aindy_scheduler_job_starved_total{job_id,reason}` from an APScheduler listener on `EVENT_JOB_MAX_INSTANCES`/`EVENT_JOB_MISSED`, since saturation was a per-job log line only. Source: `TECH_DEBT.md` SYSMAX-5.
