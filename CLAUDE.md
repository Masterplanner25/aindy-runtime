...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. Failure is crash-only: a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ Store 4 is UNCONFIGURED: `AINDY/` has ZERO references, so it roots at the worker's CWD — which no spawn path sets — with NO compose volume.** Injectable (`runner.py:492`, NOT `:437` — that moved). **★★ nodus 6.0.0 flips JSON→SQLite and neither store sees the other. DO NOT trust `migrate-store` as the mitigation — it carried 432 of 629 reporting `skipped: 0`, dropping files by MTIME past 30d, not status.** **★ §8 proposal filed: store 4 is WRITE-ONLY from the runtime, so DECLARE, don't migrate — and the old interim's `NODUS_WORKFLOW_STORE_ROOT` is the WRONG variable, use `NODUS_RUN_STATE_ROOT`.**
