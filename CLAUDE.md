...
- **EU-WAIT-SIGNAL-DEAD-1** — **CLOSED 2026-09-15 by REMOVAL.** `ExecutionWaitSignal` let a route handler park the REQUEST's execution unit; nothing raised it in 4 repos, and nothing COULD — a route has answered its client before it can raise, so `resumed → executing → ?` had nothing to execute. **★ Its resume callbacks (pipeline + boot rehydration) flushed and closed WITHOUT commit — probed: `waiting` after the callback, `executing` with a commit. FR-30's shape, third instance in a week: a callback that owns its session must commit.** **★ A request's unit now CANNOT enter `waiting` by any path** (pinned behaviourally + by AST over `core/execution_pipeline/`). ★ `rehydrate_waiting_eus` still registers a rollback-only callback — redundant with the flow path; follow-up, NOT by adding the commit alone.
