...
- **FR-15** — dispatch is serialised: `_scheduler_heartbeat_tick` is the only queue drainer and `schedule()` runs each item **synchronously**. (b)+(c) shipped 2026-08-15/16; **★ (b) was a CORRECTNESS bug, not throughput.** **★★ (a) SPLIT + SOAKED 2026-09-01, DELIBERATELY NOT FLIPPED.** "Flip one line" was never one change: `AINDY_ASYNC_HEAVY_EXECUTION` gated the scheduler **and** two live routes that answer **202-queued instead of a result**, so the scheduler now asks `async_scheduler_dispatch_enabled()` (third one-flag-two-meanings split here — cf. `EVENTBUS-PUBLISH-LATCH-1`, FR-17). **★★ THE BLOCKER: under `EXECUTION_MODE=distributed` — which `docker-compose.prod.yml` SETS — the ASYNC branch drops `handler_fn`, and the scheduler's work IS a closure; the worker finds no JobLog, warns, ACKs, returns SUCCESS. The resume is lost silently and forever — worse than the starvation being fixed.** Gate refuses distributed first. **★ The ASYNC branch was unreachable from EVERY test, so the obvious soak passes running INLINE.** Source: `TECH_DEBT.md` FR-15.
