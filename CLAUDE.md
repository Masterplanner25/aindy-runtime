...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.14.0.md` — pin bump + rebuild, no schema step, no required code change. Four UNFLAGGED WAIT/resume + job-retry fixes (#654–#657); every consumer-visible edge checked against the app's source, none fires (its one WAIT node sends no patch; its agent resume was already correlation-unique; it submits no jobs directly). **★ Verified live by the app team 2026-09-14 (handoff §6): Tutorial 2 Steps 1–6 complete on their rebuilt 2.14.0 container — #654/#655/#656 hold; `history` has a third row (`nodus_record_outcome`, correct). Their FR-29 came out of that run: a GET of a waiting run parks the READER's EU (pipeline `_detect_wait` sniffs the returned row's `status`), armed by the app's `flow_run_get` result-key registration — pre-existing, awaiting intake.** **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** — the 2.11.0 handoff's correction records the cwd trap that bit both repos |
