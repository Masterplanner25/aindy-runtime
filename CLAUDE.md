...
- **AUTH-V\*** — Auth system wiring gaps (2026-06-06 audit). AUTH-V1: `auth/__init__.py` verbatim duplicate; closed 2026-06-06 (replaced with re-export shim). AUTH-V4: frontend logout never called `POST /auth/logout`; closed 2026-06-06 (`logoutUser()` added to `@aindy/ui-kit`, `AuthContext.logout()` calls it best-effort). AUTH-V6: `require_platform_admin_access()` passed ALL API keys regardless of scope; closed 2026-06-06 (scope check added; `admin_invalidate_sessions` now uses `Depends(require_platform_admin_access)`). AUTH-V2/V3 (dead scope code / parallel auth systems): open — wiring `require_scope()` to routes is a separate feature.
