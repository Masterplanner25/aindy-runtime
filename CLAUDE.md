...
- **EXEC-ENV-BIND-1** — an execution unit cannot declare the environment it needs; `ExecutionUnit` has 21 columns and no confinement field — only *measured actuals*. The descriptor's value is **accountability, not variation**: today "was this the containment you asked for?" has no answer for any given run. **★ DESIGN SETTLED 2026-08-19 — `docs/runtime/EXECUTION_ENVIRONMENT_SPEC_DESIGN.md`, three axes (visibility / authority / resources), no code yet.** **★ Two traps it records: the filed proposal resolved at `gate_and_dispatch`, which has ZERO callers (and `MEDIATED_EFFECT_BOUNDARY_PROGRAM.md:50` already said so) — the live seam is `require_execution_unit`; and that function returns `None` non-fatally by contract, so a refusal cannot ride its return value.** Keep separate from `GUEST-CONFINE-1`.
