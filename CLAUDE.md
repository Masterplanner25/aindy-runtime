...
- **CANCEL-REACH-1** — **NARROWED 2026-09-03 (#566); open for the isolated seam.** A cancelled run now refuses its **next tool call** at `execute_tool`, moving the window from SEGMENT to EFFECT granularity; `aindy_run_cancel_observed_total{surface}` makes the narrowing measurable. **★ COOPERATIVE by contract, not by omission — a tool already executing is never interrupted.** **★ It fails OPEN, unlike every other guard here: an unreadable status means "not cancelled", because a missed cancel costs one more effect and a false cancel costs the run.** **★ It never queries per effect** — own short-lived session, one read per run per 2s, `cancelled` cached forever; RT-MEMTXN-LEAK-1 and MEM-RECALL-N1-1 are both this shape. **★★ The residual is an OVER-CLAIM to correct, not a gap to fill: the out-of-process worker passes `run_id=None` on the stated grounds that it is "hard-killable by its isolation class" — that capability exists (`subprocess.run(timeout=…)`) and NOTHING invokes it on cancel. Only the timeout ever kills that worker.** Source: `AINDY/kernel/cancellation.py`.
- **FLOW-PARALLEL-1** — **CONFLICT HALF #569; PHASES 0+1+2 SHIPPED (2 on 2026-09-13, #640); open for 3–4.** `FanOutEdgeGroup(targets, join=all|any|quorum, quorum=k)` runs a node's successors as ONE superstep: per-branch sessions, one ordinal block at the barrier, central merge. Default-OFF (the flag gates CONCURRENCY, not semantics). **★★ Phase 2: the join is resolved at the barrier and a lenient join proceeding past a failed branch is the runtime's FIRST `partial` EMITTER** — on the run state, the completion event and the `flow.run` envelope; convergence is required of the SUCCEEDED branches. **★ `all` keeps phase 1's RECORDED signature digest; a non-default join is shape and changes it.** **★★ The width bound is PROCESS-WIDE (`SYSMAX-5`).** **★ Open: phase 3 named predicates/switch-case; phase 4 the default flip on evidence.** Design: `docs/design/FLOW_PARALLEL_DESIGN.md`.
- **AUTHORITY-NEGOTIATION-1** — **PHASES 0+1 SHIPPED (1 on 2026-09-10); open for 2–3.** A denied capability terminates the step; approval is whole-plan, so recovery discards the durable state nine good steps accumulated. Bounded (exactly once), **downgrade-only**, recorded, **default-OFF** (`AINDY_AUTHORITY_NEGOTIATION`, opt-IN). **★★ It CANNOT grant authority, structurally: it picks WHICH TOOL to attempt and `execute_tool` re-runs `check_tool_capability`, so §7's no-widening rule is unreachable, not merely unauthorised.** **★ The §2 subset rule is ASKED of the enforcing code, never reimplemented — a hand-rolled check omits granted_tools AND agent capabilities.** **★★ The design said TWO `CAPABILITY_DENIED` sites; there are FOUR and exactly ONE is negotiable — the rest deny a MISSING TOKEN or run-level `execute_flow`, where there is no tool (variant 12, corrected in §1).** **★ The TOOL declares it, never the plan. ★ Phase 0's inertness guard was REPLACED, not deleted.** Design: `docs/design/AUTHORITY_NEGOTIATION_DESIGN.md`.
- **NODUS-RESUME-BRIDGE-1** — *(tutorials run live, 2026-09-13)* **a Nodus script can suspend a run but can NEVER receive what resumed it — through any path, ever.** `_merge_superstep` merges SUCCESS patches only (its docstring says so), so the WAIT patch carrying `nodus_wait_event_type` reaches `flow_history` and never `flow_runs.state`; on re-run the bridge at `nodus_adapter.py:672` finds no pending type, drops the injected `event`, and the script re-waits. **Observed: `WAIT, WAIT, WAIT…` in history; state has 5 keys, none the patch's.** `GUEST-BUILTINS-DEAD-1` proved the suspend and stopped; the guide documented the resume from the design. Fix: persist `wait_for` into state on the WAIT branch (one line) or merge WAIT patches deliberately; then a test whose assertion is the script's SECOND run. Subsumes `WAIT-PAYLOAD-PATH-1`.
- **RESUME-FANOUT-UNSCOPED-1** — **`POST …/runs/{A}/resume` injected the payload into run B too.** `route_event` peeks all waits on the event type with no tenant or run filter and `db.query(FlowRun)` has no `user_id`; the ownership check covers only the path parameter. `platform.admin`-gated, so an isolation defect not an exploit — an operator approving one tenant's wait resumes every tenant's run parked on that event name. Fix: inject into the named run only (its `trace_id` is the correlation); broadcast is a different verb. `TENANT-3` under `DEPLOY-TARGET-2`, already live.
- **ACTIVE-COUNT-WAIT-LEAK-1** — **a waiting run holds a tenant concurrency slot until the process restarts.** `mark_started` at flow start, `mark_completed` on success/failure, **nothing on WAIT** (`runner_steps.py:348`); cap is 5. **Observed: after four parked waits a read-only GET returned 429.** With `NODUS-RESUME-BRIDGE-1` every resume makes another wait — the tutorial locked its own tenant out in under a minute; under `hostile-third-party` it is a five-request self-DoS. Fix: release on WAIT, re-acquire on the resume claim; test parks N ≥ cap runs then makes any request. **Not a cap raise.** Also seen: 105 EU rows stuck `executing` for one user.
- **ASYNC-JOB-UNREGISTERED-STORM-1** — **an unregistered job handler is re-dispatched in a tight in-process loop, uncounted, forever** — `raise` at `async_job_service.py:1153` precedes `attempt_count += 1` at `:1157`, so the except sees `0 < 1`, sets `pending`, and `_dispatch`es again with no backoff. **Measured ~87/s per job, 45k log lines in ten minutes**, from two rows a consumer left in the DB. It rewrites `status='failed'` back to `pending` within the second; only stop → edit → start breaks it, and boot recovery re-dispatches anything still pending. Fix: unregistered is NOT retryable — fail terminally; increment before raising; backoff on generic retry. Test: one attempt, terminal `failed`.
...
- **GUEST-BUILTINS-DEAD-1** — `runtime/nodus_builtins.py` (530 lines) has **ZERO importers** under `AINDY/`: the guest `event` and `memory` namespaces and `NodusWaitSignal` are unreachable. **★ DEMONSTRATED: its own docstring example `event.wait("approval.received")` returns `Undefined variable: event`** — the worker never imports the module. **★ The wait IS live, reachable ONLY by an undocumented magic state key (`nodus_wait_requested`), so the supported-looking API fails and the unsupported one works.** **★ Nothing is broken: waits are HOST-level, `agent_plan_compiler` segments the plan at each wait step, which is why nobody noticed.** `WorkerWaitSignal` is the vestigial twin — caught, never raised. **★★ `CLAUDE.md` documented `sys.v1.event.wait`, a syscall that does not exist (corrected).** **★ Do NOT delete 530 lines first — `WAIT-TYPED-CONTRACT-1` owns the guest-wait contract decision.** **★ Catalogue variant 14 lives here.**
- **SANDBOX-EVIDENCE-1** — *(found writing `SANDBOX_CONTRACT.md`)* the `hostile-third-party` **post-launch kill** (`plugin_host.py:858` — live attestation fails → `force_kill` + `contract_violation`) has **no test**; only the pre-spawn refusal is pinned. `ROUTE-GUARD-1` one layer down: the guard is written, nothing proves the process is dead. Fix: fake runner that passes policy and fails attestation; assert termination; **mutation-test by removing the kill call.** Do NOT close on the health endpoint alone.
...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. Failure is crash-only: a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ CONFIG HALF SHIPPED 2026-09-09 (#611), and it does NOT close this:** the worker declares `sqlite` + autosweep OFF, compose sets `NODUS_RUN_STATE_ROOT` behind a volume — store 4 was CWD-rooted with NO volume, so guest state was container-ephemeral. **It configures a durability layer; it does not remove one. (a) a `WorkflowStore` over Postgres (`runner.py:492`, NOT `:437` — that moved) and (b) the contract are untouched.** **★ Store 4 is WRITE-ONLY from the runtime, which is why declaring beat migrating — and DO NOT trust `migrate-store`: it carried 432 of 629 reporting `skipped: 0`, dropping files by MTIME past 30d, not status.**
...
- **WAIT-PAYLOAD-PATH-1** — **SUBSUMED 2026-09-13 by `NODUS-RESUME-BRIDGE-1`** (run live: the route path does not deliver either). Kept for its two halves: **the event bus resumes a waiting run WITHOUT its payload** — `publish_event` has no payload parameter and the resume callback is zero-arg; only `POST …/runs/{id}/resume` → `route_event` injects one. A Nodus script resumed by `event.emit` re-runs, finds nothing, and re-waits — silently. **★ And the emit usually does not even match: the wait keys on the run's `trace_id`, the emit carries its own.** `NODUS_DEVELOPER_GUIDE` §4 claimed the opposite; corrected. **★ Local and cross-instance correlation rules DISAGREE** (`waits.py:111` vs `cross_instance.py:56`). Fix: mark payload-less resumes + one rule with a test on both paths. Design question first.
...
- **RECOVERY-GRANULARITY-1** — *(LangGraph research)* **recovery granularity is welded to scheduling granularity: the runtime checkpoints at the boundary of the unit it schedules, so control flow INSIDE that unit is invisible to recovery.** **★ The repair exists at one layer and was not carried down: the flow layer commits `FlowHistory` BEFORE the snapshot advance (`runner.py:347-359`); the agent layer does not, so `_count_completed_segments` restarts a partially-executed segment from step ONE.** **★ P2 not P0 — `DUR-2` means mediated effects do not double-fire, so the re-run is CORRECT; what it costs is WORK — re-issuing every LLM call in the segment.** **★★ Worked reference (DBOS): `operation_outputs`, one row per step keyed `(workflow_uuid, function_id)`, checked before every step to REPLAY the recorded output. Step granularity — exactly ours; its monotonic ORDINAL is cheaper than a vector clock and the thing to build FIRST.** ★ Does NOT reopen the declined kernel-replay decision — see `ECOGAP-1`'s taxonomy row 4.
