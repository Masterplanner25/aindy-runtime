...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.18.0.md` — pin bump + rebuild, no schema step, no required code change. Closes the app's **FR-31** (#689): a run parked across a restart resumes on a fresh process's FIRST wake — `nodus_execute` was registered lazily, `agent_execution` NEVER (so a gate-parked agent run could not survive a restart); the app's "run a script first" workaround is retired. **★ Consumer-visible: `resumed` on `POST …/runs/{id}/resume` now means WOKEN; `woken` added; `payload_injected` is "stored"** — nothing of theirs calls the route (checked). Removal: `nodus_builtins` / `WorkerWaitSignal` (they import neither). Their two 2.17.0 asks stand. Not yet verified by the app. **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** |
