...
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next (`tool_registry.py:366`). **★ SCOPED — `docs/runtime/TOOL_SEAM_ISOLATION_SCOPE.md`, which corrects four filed claims: a tool is a CALLABLE not a command (the "settled" transform is one indirection off; the serialization boundary is the real cost); the three call sites are in THREE processes, only `nodus_adapter.py:263` host-side; ZERO foreign in-process tools exist (MCP tools are runtime-owned proxies); all 18 tool fns take `db` and NONE uses it.** **★ STEP A SHIPPED** — tools get a `RevocableToolSession`, revoked at return. **★ STEP B SHIPPED** — `register_tool(..., isolation=<assurance class>)` declares a minimum and is REFUSED fail-closed when the host cannot meet it; an assurance class NOT a mechanism, reusing `EXEC-ENV-BIND-1`'s vocabulary. **★ Neither CONFINES: an allowed tool still runs in-process with ambient authority — pinned by a same-pid assertion. Step C (the process boundary) is the open half.**
