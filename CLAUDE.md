...
- **★ NATIVE-DISCOVERY-1** — **the two crate consumers search different directories, so the C++ cosine kernel is unreachable from the recall path in any release-built environment.** `native_scorer._load_bridge` searches `target/release` **then** `target/debug`; `embedding_service.cosine_similarity` searches **`target/debug` only**. CI builds `--release`, as would any deployment — so `cosine_similarity` silently uses pure Python while `native_scorer` in the *same process* uses native (verified locally). Also in that function: `except (ImportError, AttributeError, Exception)` is just `except Exception` and swallows the extension's `ValueError` on ragged input (a real bug returns `0.0`, indistinguishable from "not similar"); and `sys.path.insert(0, …)` runs on **every call**. Fix: one shared loader. **Gotcha for local work: `cargo build` emits `memory_bridge_rs.dll` on Windows and Python will not import that — copy it to `memory_bridge_rs.pyd`.** **Open.**
...
- **★ EVENTBUS-COVERAGE-1 — the pub/sub wire has never been exercised end to end, so `Integration Tests (PostgreSQL + Redis)` is NOT evidence for an event-bus change.** Measured on `main`: **0** integration tests call `EventBus.publish()`/`get_event_bus()`; all **4** `notify_event(...)` calls in `tests/integration/` pass `broadcast=False` (production default is `True`, `waits.py:79`); **0** tests anywhere call `start_subscriber()`. Untested: `publish()` against real Redis, `_subscriber_loop` (the thread — the unit suite drives `_handle_message` directly, skipping `pubsub.listen()` and the reconnect back-off), and the whole reason the module exists (publish on A → Redis → subscriber on B → local `notify_event`). **Don't mis-read `test_multi_instance_resume.py`** — it tests real cross-instance resume via **`RedisWaitRegistry`** (shared Redis *state*), not the bus. **Why it matters: `EVENTBUS-PUBLISH-LATCH-1` survived in `main` until found by *reading*, because no test runs the publisher.** Same family as DOCS-COVERAGE-CLAIM-1 / CI-MARKER-1 — a check whose name implies coverage it doesn't provide, now gating every merge. **Cheap to close:** the Integration job already provisions live `redis:7-alpine`. **★ Pitfalls when writing it:** poll for an effect with a deadline (never `sleep` — see FLAKY-1); give the two buses **distinct `_instance_id`s** or the own-instance filter silently eats the message; mark rehydration complete or `_handle_message` buffers instead of dispatching; and do **not** add `pytest.mark.integration` (it needs Redis, not PG — that marker triggers the conftest skip guard). **Open.** Source: `TECH_DEBT.md` EVENTBUS-COVERAGE-1.
