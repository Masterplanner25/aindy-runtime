...
- **RT-MEMTXN-LEAK-1** — FIXED 2026-07-19 (app handoff, HIGH — login took ~40s, exceeded the web client's 30s timeout). `MemoryNodeDAO.recall` held a DB connection `idle in transaction` across the synchronous `generate_query_embedding` LLM/embedding API call (~seconds); under a browser login's concurrent fan-out ~60–85 connections piled up and exhausted the pool. Fix: `embedding_service.release_read_transaction(db)` (guarded rollback when no pending writes) called before embedding at all 3 sites (`recall`, `/memory/nodes/search`, `memory_nodes_search_similar` flow node) — the connection is held only for the fast DB queries, not the API call. **Rule: never call `generate_query_embedding` (or any slow external call) while holding an open DB transaction on a request-shared session.** See `TECH_DEBT.md`.
