...
- **INFINITY-RUNTIME-1** — Open (accepted 2026-07-05): runtime-side counterpart to the app-side handoff `aindy-apps-monolith/TECH_DEBT.md` → `INFINITY-RUNTIME-HANDOFF-1`. Tracks the five Infinity loop-closure gaps from `docs/runtime/INFINITY_LOOP_AUDIT.md` (recall→planning link, missing `RecallUsed`/`ScoreComputed`/`NextActionChosen` events, no execution-level score record, no Next-Action engine primitive, async jobs outside the loop) + runtime-gated support-input aggregate syscalls. Gap 4 gates app-side Infinity Phase 2. Infinity scoring/orchestrator/loop is app-owned. **Advance 2026-07-08:** the 3 event constants added (`RECALL_USED`/`SCORE_COMPUTED`/`NEXT_ACTION_CHOSEN`, un-prefixed by `execution.`); **Gap 3 CLOSED** — `core/execution_score.py` emits one `SCORE_COMPUTED` SystemEvent per finished run (`{run_id, score, status, dimensions[, duration_ms]}`, event-row-as-record, no schema bump) at agent-run completion + generic `ExecutionLoop`. **Advance 2026-07-08 (2):** **Gap 1 CLOSED** — `generate_plan` recalls objective-keyed memory pre-plan (runtime-owned `_recall_planner_memory`, not the app provider) + injects via `_build_planner_prompt` `memory_block`, gated by `AINDY_PLANNER_MEMORY_INJECTION` (default off, opt-in); `core/execution_recall.py` `emit_recall_used` fires `RECALL_USED` at planning + execution recall sites. `SCORE_COMPUTED`+`RECALL_USED` wired; `NEXT_ACTION_CHOSEN`(Gap 4) constants-only. **Advance 2026-07-08 (3):** **Gap 5 CLOSED (opt-in)** — root cause: async path emits `EXECUTION_COMPLETED` but the contract gate raises it + `_emit_async_system_event` swallows → never persists. Fixed behind flag `AINDY_ASYNC_JOB_LOOP_CLOSURE` (default off): `_execute_job_inline` activates async-exec context (events persist → auto-capture writes memory) + `_emit_async_job_score` emits `SCORE_COMPUTED` (`source=async_job`). Recall-into-jobs NOT wired (infra jobs). **Advance 2026-07-08 (4): Gap 4 CLOSED (record-first) — ALL FIVE STRUCTURAL GAPS NOW CLOSED.** `core/next_action.py` (NextAction contract + coerce + runtime-default + emitter); `execution.py` captures the `_run_completion_hooks` return (was discarded at :218), coerces/defaults, emits `NEXT_ACTION_CHOSEN` for completed+failed. Record-first (runtime records, no autonomous action; app consumes) — **lifts app-side Infinity Phase 2 gate.** No syscall/schema. **Advance 2026-07-08 (5): item-3 CLOSED → INFINITY-RUNTIME-1 FULLY CLOSED.** New syscall `sys.v1.observability.support_metrics` (cap `execution.read`, already on the `/platform/syscall` dispatch surface → app fetches via `dependency_adapter`, no grant changes) + `platform_layer/support_metrics_service.py`: tenant-scoped read-only rollup of request/health (Step 3) + agent/async/loop-event distributions (Step 4), `window_hours` default 24. `SYSCALL_REGISTRY_MIN_COUNT` 21→22; added to `_STABLE_SYSCALLS`. **Advance 2026-07-09: Deliverable C (autonomous acting on NextAction) SHIPPED opt-in** — `core/next_action_dispatch.py` `maybe_act_on_next_action` (called from `_emit_agent_next_action` after the record emit) dispatches ONE bounded follow-up run for an **app-sourced** `trigger_execution` (with an objective in `args`): create_run→execute_run via async job `agent.next_action_followup`, reusing the admission (`count_active_executions`) + approval gates (a `pending_approval` follow-up is NOT force-executed) and adding one net-new rail — a **chain-depth cap** (`parent_run_id` hops) so a hook that always returns `trigger_execution` can't self-perpetuate. Gated `AINDY_NEXT_ACTION_ACTING` (default off); `AINDY_NEXT_ACTION_MAX_CHAIN`/`_MAX_ACTIVE` tune the rails. Never acts on a runtime-default decision (`trigger_execution` is never a runtime default + explicit source guard). Agent runs only; async/flow paths have no NextAction seam. No syscall/schema; no new SystemEventType. Deferred non-blocking follow-ups: flip the opt-in flags after soak; broaden acting verbs (`retry`/`schedule_follow_up`) if a need arises.
