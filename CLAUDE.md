...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.16.0.md` — pin bump + rebuild, no schema step, no required code change. Closes the app's FR-30 (`EU-FINALIZE-UNCOMMITTED-1`, #673): a request's `execution_units` row now actually reaches `completed`/`failed` — it never had, on any release; FR-29's rehydration-seed addendum stops the boot-time FK noise. **★ Its §4 is the first live run of #673 — after a handful of requests, `select … from execution_units where source_type='route' and created_at > <upgrade>` must show ZERO `executing`; the optional `UPDATE` retires the ~900 pre-upgrade rows.** **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** — the 2.11.0 handoff's correction records the cwd trap that bit both repos |
