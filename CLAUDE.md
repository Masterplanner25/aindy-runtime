...
Numbers are sequential per prefix and never reused (next: **FR-43**, **IDEM-13**). Closing an entry:
...
- **SUBSTRATE-WITNESS-1** — *(Claude Code)* ★ WITNESSED 2026-09-21 (Claw delivers through `execute_tool`, `claw.channel.send` EXACTLY_ONCE; a retry refused by the ledger on real PG; tenant = a UUID FK to `users`; FR-42). ★ 2026-09-22 step-3 attempt: **WebChat cannot witness it** — it streams; `deliver()` is the non-WebChat branch (4 turns → 0 rows), and a live Claw's `/metrics` lacks the runtime registry. Needs one of Claw's five external adapters. Never a synthetic fixture.
