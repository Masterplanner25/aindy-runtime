...
- **AUTHORITY-NEGOTIATION-1** — **PHASES 0+1+2 SHIPPED (2 on 2026-09-15); open for 3 — the flip, on evidence (zero tools declare a variant or gate at HEAD).** A denied capability terminates the step; approval is whole-plan, so recovery discarded nine good steps. Bounded (exactly once), **downgrade-only**, recorded, **default-OFF** (`AINDY_AUTHORITY_NEGOTIATION`). **★★ It CANNOT grant authority, structurally: it picks WHICH TOOL to attempt and `execute_tool` re-runs `check_tool_capability`.** **★ The §2 subset rule is ASKED of the enforcing code, never reimplemented.** **★ Phase 2: `register_tool(..., on_denial="wait")` PARKS the run on `agent.authority.decision` with a typed `resume_schema`; operator decides `skip | abort`, never `grant`. ★★ `AGENT_FLOW` had NEVER waited — its orchestration read any non-SUCCESS as FAILED, and a failure AFTER a resume never reached the AgentRun; both fixed. ★ The TOOL declares it, never the plan.** Design: `docs/design/AUTHORITY_NEGOTIATION_DESIGN.md`.
