...
- **IDEM-11** — at-most-once is built and **shipped disabled**; the audit is DONE, the flag flip is not. **★★ `EXACTLY_ONCE` IS NOT EXACTLY-ONCE UNDER CONTENTION — measured 2026-08-19, 8 concurrent identical calls ran the handler TWICE.** By design: `resolve_effect_record` degrades to `AT_LEAST_ONCE` when it loses the insert race to a *live* pending row (strict at-most-once needs advisory locking), logging a WARNING. `IDEMPOTENCY_CONTRACT.md` documents it; **this line said "at-most-once is built" with no caveat, and the index is what gets read before a flip.** Pinned by `tests/integration/test_soak_idempotency_contention.py`. The durable guarantee that DOES hold: one `success` row per `action_id`. **★ `_durable` (DUR-2) engages the gate for ANY syscall, bypassing both flag and declaration.** **★ `register_syscall` had NO `execution_guarantee` parameter**, so the gate was unreachable for plugin syscalls; now forwarded and validated. **★ No metric observes the gate at all** — a hit/replay/degrade counter is a prerequisite for a production soak.
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next; `execute_tool` evaluates token/tools/capabilities/policy/egress then calls the tool in-process (`tool_registry.py:366`). **★ SCOPED 2026-08-19 — `docs/runtime/TOOL_SEAM_ISOLATION_SCOPE.md`, which corrects four filed claims: a tool is a CALLABLE not a command (so the "settled" transform is one indirection off and the serialization boundary is the real cost); the three call sites are in THREE processes and only `nodus_adapter.py:263` is host-side; ZERO foreign in-process tools exist at HEAD (MCP tools are runtime-owned proxies); and all 18 tool fns take `db` while NONE uses it.** **★ Step A SHIPPED: the tool now gets a `RevocableToolSession`, revoked when the call returns — it narrows ONE ARGUMENT and does NOT bound the process, so this entry is not closed.** Next: declare `isolation=`; sequence the process boundary behind a named consumer.
...
- **AUTHORITY-VALUE-1** — the syscall capability check reads a value the calling frame supplied; **`child_context()` could WIDEN — clamp shipped opt-in 2026-08-16 (#448)** behind `AINDY_CHILD_CONTEXT_CLAMP` (default off), with a WARNING on every widening so the exposure is countable. **★★ NOT SOAK-GATED, verified 2026-08-19 — the blocker is a CALLER FIX in one file.** No caller exists under `AINDY/`, but `aindy-apps-monolith`'s `apps/automation/syscalls/syscall_handlers.py:45` passes `child_context(capabilities=[capability])` with the *nested* syscall's capability while the parent holds only the *outer* one, so the clamp intersects to **EMPTY** and denies a working call. Fix that caller, then flip; no evidence needed. Not a vulnerability (every entry point has its own authorisation) — the point is the chokepoint is not an independent second gate. **Quiet bit: absent identity SKIPS the boundary rather than denying**, logged at `debug`.
