...
- **DEP-UPGRADE-DEFERRED-1** — Deferred deliberate dependency upgrades. **OTel half CLOSED 2026-08-01: bumped to 1.44.0** (`api`/`sdk`/`exporter-otlp-proto-grpc` 1.44.0 + `instrumentation-fastapi` 0.65b0; the other six otel packages are unpinned and resolve transitively). **Two lessons:** (1) the otel packages are version-locked, so single-package PRs always die `ResolutionImpossible` (#251/#254/#307 all closed for it) — a `groups` config for `opentelemetry-*` now forces them into one PR; (2) **grouping is necessary but NOT sufficient** — dependabot's own grouped PR (#325) still failed because it resolved each package independently and picked `api==1.43.0` against `sdk==1.44.0`. Grouping controls *which PR*, not *which versions*; expect to hand-align the set and verify with `pip install --dry-run` before pushing. **Still open — the UI major unit:** vite 6→8 (#298) + `@vitejs/plugin-react` 4→6 (#308) + tailwind 3→4 (#310), which are **one unit** (plugin-react 6 peer-locks to vite ^8). react-router 6→7 was originally lumped in and **landed alone 2026-08-02 (#345)** once `@aindy/ui-kit@2.0.0` widened its `^6.0.0` peer pin. The unit's blocker is **NOT** "breaking UI major" (the old, wrong diagnosis) — it's `LOCKFILE-PLATFORM-1`. Source: `TECH_DEBT.md` DEP-UPGRADE-DEFERRED-1.
- **LOCKFILE-PLATFORM-1** — **Open (2026-08-02), blocks the UI major unit.** A Windows-generated `platform/package-lock.json` cannot satisfy Linux `npm ci`: vite 8 (**rolldown**) and tailwind 4 (**oxide**) both pull `@napi-rs/wasm-runtime`, whose `@emnapi/*` deps are *transitive* deps of packages that never install on Windows, so npm prunes them and emits a lock that is complete here and `Missing: @emnapi/runtime@1.11.3 from lock file` in CI. **Not fixable from this machine** — `npm install`, `--package-lock-only`, a from-scratch regenerate, and npm 11's `--os/--cpu/--libc` targeting all yield the same tree; a Linux container over the *mounted* dir is worse (it reconciles against the Windows `node_modules` and produces a win32-only lock). Contrast **esbuild**, which declares all 26 platform variants as *explicit optional* deps so npm records every one — the question is never "are the other platforms' packages in the lock" but "are their **transitive** deps". **Fix SHIPPED 2026-08-02: `.github/workflows/platform-lockfile.yml`** (`Platform Lockfile`) — `workflow_dispatch` with inputs `ref` (branch to resolve) + `push` (commit back, default off → artifact `platform-package-lock`). Runs `npm install` on `ubuntu-latest`, then **verifies with `rm -rf node_modules && npm ci` + `npm run build`** (that verify step is the point — without it the job repeats the mistake it exists to prevent). Deliberate: dispatch-only (it's a tool, not a gate — so it must be on the **default branch** before it can run against any ref, i.e. it can't be exercised on its own PR); incremental `npm install` not from-scratch (keeps the diff reviewable); **no `cache: npm`** (setup-node keys the cache off the lockfile being regenerated). Linux is authoritative — a win32-only omission is accepted because dev uses `npm install` (self-repairs) and CI uses `npm ci` (doesn't). **Smoke-verified green on `main` 2026-08-03** (run `30815998362`). **Expect a benign `"peer": true` diff — don't chase it:** that run reported `LOCK_CHANGED=true` on an untouched `main` and the whole diff was 16 lines of `"peer": true` flags, zero package/version changes. Cause is npm-major skew — dev is node 24/npm 11 (writes the flag), all four workflows are node 20/npm 10 (don't). The resolver pins node 20 to match `Platform UI Build`, so it's on the correct side. Read the *"Packages added"* output, not the changed/unchanged bit. **Adjacent, unfixed:** that node 24-vs-20 skew is its own divergence source; closing it wants a `.nvmrc` (none exists) + a node bump across four workflows — deliberate, not drive-by. **Process rule: verify a lockfile change with `npm ci`, never `npm install` + `npm run build`** — `install` silently repairs a mismatch and a build keeps passing off a populated `node_modules`. Source: `TECH_DEBT.md` LOCKFILE-PLATFORM-1.
