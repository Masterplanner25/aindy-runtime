...
- **CAPABILITY-PROVIDER-TIMEOUT-1** (filed 2026-08-16 as `TEST-ISOLATION-REGISTRY-1`; **that name and diagnosis were WRONG — it is not test isolation**). `get_capability_definitions()` returns **`{}` silently** when the provider subprocess exceeds its 30s budget: `capability_definition_provider` is **not** in `_STATEFUL_IN_PROCESS_CALLBACK_SURFACES`, so it is subprocess-wrapped, and `_load_capability_definition_providers` catches the timeout into a `logger.warning`. Symptom is `test_platform_only_registers_runtime_agent_defaults` failing under *some* runs — which reads like state leaking across app boots and is not; it is **load-dependent, not order-dependent**. **P1 because it is a runtime path**: capability definitions gate tools/agents, so a contended host yields an empty capability vocabulary and every consumer degrades quietly — nothing distinguishes "defines no capabilities" from "the definer timed out". **Do NOT close by raising `AINDY_RUNTIME_CALLBACK_TIMEOUT_SECS` or by touching the harness.** Fix order: make the failure loud → reconsider whether this is a 4th member of `_STATEFUL_IN_PROCESS_CALLBACK_SURFACES` (it reads registry state, the `PLANNER-SUBPROC-1` argument) → only then the budget. **Trigger is CPU contention starving the subprocess** — both observed failures had a second heavy pytest run competing; isolated re-runs were clean. **Diagnostic: `grep -c "Capability definition provider failed"` on the run output** — the assertion failure itself says nothing about the cause, which is how the first diagnosis went wrong. Source: `TECH_DEBT.md`.
