...
- **SUBSTRATE-WITNESS-1** — *(Claude Code research)* **the substrate claim has no first-party consumer that exercises it.** Measured against `C:\dev\claw`: the flagship app integrates in 334 lines across 3 files, all optional, mostly HTTP; `execute_tool`/`EffectRecord`/`execution_token` appear **zero** times in its own source. Its real effects cross no chokepoint. **★ Corollary for reading this file: the coverage percentages across nine comparative audits describe capabilities the runtime HAS, not capabilities anything USES.** **★ This is why the flag backlog is stuck — soak needs production traffic THROUGH the path being flipped, and no first-party consumer sends any.** Pair with `PERF-BASELINE-1`: one says no instrument, this says no traffic. Recommended slice: route only Claw's outbound message delivery through `execute_tool` with `EXACTLY_ONCE`. **★ Do NOT close with a synthetic fixture** — what is missing is a consumer that would NOTICE if the guarantee broke.
