...
- **EFFECT-OUTCOME-UNKNOWN-1** — *(browser-automation design notes, 2026-08-22)* **the runtime has no word for "dispatched, outcome unobserved".** Two CLOSED binary vocabularies: `frozenset({AT_LEAST_ONCE, EXACTLY_ONCE})` (**`AT_MOST_ONCE`: zero hits repo-wide**) and `EffectRecord.status = pending|success|failed` (**no `unknown`**). For a non-transactional counterparty the honest label is UNREGISTERABLE — one over-claims, the other under-claims. **★ Do NOT borrow `pending`: the cleanup job already warns on any pending row >1h as a stuck handler, so a recorded ambiguity would read as a malfunction** (EVENTBUS-PUBLISH-LATCH-1 shape). **★ Cheap: `status` is `String(32)`, no CHECK, assigned unvalidated → NO MIGRATION; settle it WITH `EFFECT-PARTIAL-1`, same column, one change.** ★ Live-ish: `outbound_request` catches `httpx.HTTPError` (base class) so `ConnectError` and `ReadTimeout` retry alike, no method guard — but it has NO caller in `AINDY/`, only tests. ★ `CANCEL-REACH-1` blocks the release-on-cancel phase; a browser syscall UN-DEFERS `EFFECT-PRECONDITION-1`.
...
- **CLI-EXEC-SURFACE-1** — *(CLI lens, 2026-08-22)* **a lens, not a defect — but scoping it found one (`QUOTA-ACCRUAL-ORPHAN-1`).** All 8 `aindy-runtime` subcommands ADMINISTER the server; **none executes anything**. `nodus` BELOW has `run`/`repl`/`workflow resume`; `claw` ABOVE has a daemon lifecycle — **the middle level is the only one that cannot be asked to do work**, so a terminal user drops to `nodus run` and routes AROUND the dispatcher/token/ledger/quota. **★ Nine audits missed it because the CLI was on both sides of every comparison and the subject of neither** — `COMPARATIVE_RESEARCH_INDEX.md` has ZERO "CLI" hits; their *architecture* audits mention it 4–36×, the *ON_AINDY/LENS* ones 0–3×. The mismatch: their CLI is an EXECUTION surface, ours is ADMINISTRATION. Subsumes facets of `EMBEDDED-FLOOR-1` (deployment), `PROGRESS-CHANNEL-1` (streaming), `SUBSTRATE-WITNESS-1` (witness) — **do not merge them.** Scope: `CLI_EXECUTION_SURFACE_SCOPE.md`.
...
- **APP-FR-\*** — app-side feature requests from `aindy-apps-monolith`. **Next available: FR-19.** FR-1..13, 16, 18 shipped; **FR-17** is fixed in a companion PR. Open: **FR-14** — `bootstrap-schema` refuses additive drift, so a release adding a runtime column crash-loops under `set -e` + `restart: unless-stopped`; **their claim that `serve` self-migrates is VERIFIED FALSE.** Branchable exit codes shipped (#450): **3** additive-reconcile (*the only one safe to automate*), **4** offline-migration, **5** manual-repair — 4 ranks above 3 so an entrypoint never auto-reconciles a DB needing a person. **★ STILL OPEN, the half that prevents recurrence: the upgrade path is never exercised against an EXISTING database** — CI builds a fresh one where `create_all` makes the columns, so no green check can see this class; the same blind spot hid FR-8. Also open: **FR-6 items 2+3**, blocked structurally — the runtime ships no `email` connector; recommend runtime SMTP or hybrid, never the app-connector option that inverts the split. FR-18's retention half is `SYSEVENT-RETENTION-1`.
