...
- **IDEM-11** — at-most-once is built, tested, and **shipped disabled**. Three conditions must hold; the first two default off (`AINDY_SYSCALL_IDEMPOTENCY` unset, `SyscallEntry` defaults `AT_LEAST_ONCE`). **Measured: exactly 1 of 27 registry entries declares `EXACTLY_ONCE`** — so flipping the flag alone dedups one syscall. The per-syscall audit is the work, not the flag.
- **HTTP-SCOPE-GAP-1** — the capability model does not reach the runtime's own front door. **`Depends(enforce_api_key_scope)` on 7 of 147 route decorators**; everything else (all of `/memory/*`) depends only on `get_current_user` — identity, not authority. `memory_router.py` reaches effects with **zero** dispatcher references. **DECIDED 2026-08-15 (owner): a JWT must NOT bypass scopes** — but a JWT carries no scopes today, so implementing that literally denies every session request. Decide where JWT authority comes from first (recommend: derive from the user row).
...
- **AUTHORITY-VALUE-1** — the syscall capability check reads a value the calling frame supplied; `child_context()` inherits and never narrows. Not a vulnerability (every entry point has its own authorisation) — the point is the chokepoint is not an independent second gate. **Quiet bit: absent identity SKIPS the boundary rather than denying**, logged at `debug`.
...
- **APP-FR-\*** — app-side feature requests from `aindy-apps-monolith`. **Next available: FR-15.** Open: **FR-14** (`bootstrap-schema` refuses additive drift → crash-loops on any release adding a runtime column; **their claim that `serve` self-migrates is VERIFIED FALSE** — one behaviour, two gates, neither default-on) and **FR-6 items 2+3** (forgot/reset — blocked structurally: the runtime ships no `email` connector, so "the runtime sends it" has nothing to send with; recommend runtime SMTP or hybrid, never the app-connector option that inverts the split). FR-1/2/3/4/5/7/8/9/10/11/12/12b/13 all shipped. **Do not re-add the "their FR-7 status is stale" note — withdrawn 2026-08-15; the note was itself the stale thing.**
...
`MEM-NODETYPE-1` · `LEASE-1` · `REPLAY-1` · `NODUS-UPGRADE-1` · `PROMETHEUS-PIN-1` ·
`OPER-DEFER-001` · `OPER-DEFER-002` · `IDEM-1..10` (IDEM-10 closed at the mechanism level
2026-07-11 via the MEB program; next available **IDEM-12**) · `DOCS-BUCKET-A-1` · `DOCS-STALE-1` ·
