...
- **MAS-FLATTEN-1** — `flatten_tree` drops any node that is the parent of another node (its `roots` set subtracts every path that is some node's parent, so an intermediate node is never walked). **Zero callers under `AINDY/`**, so recorded not fixed; pinned by xfail(strict). **Related and live:** `build_tree` — which backs `sys.v1.memory.tree` — **never nests for canonical MAS data**; MAS nodes are always 5-segment leaves so a node's parent path is never itself a node, and every entry returns `children: []`. The tree endpoint returns a flat map by construction. **Open.**
