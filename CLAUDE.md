...
- **CLI-EXEC-SURFACE-1** — *(CLI lens, 2026-08-22)* **a lens, not a defect — but scoping it found one (`QUOTA-ACCRUAL-ORPHAN-1`).** All 8 `aindy-runtime` subcommands ADMINISTER the server; **none executes anything** — `nodus` BELOW has `run`/`repl`, `claw` ABOVE has a daemon, so a terminal user drops to `nodus run` and routes AROUND the dispatcher/token/ledger/quota. **★★ SHARPENED by the CLI-review corpus (21 systems): the gap is VERBS, not capability.** Its test — *can the system draw itself?* — puts us in a group of TWO of 25 with the Linux kernel: topology IS data (`FLOW_REGISTRY` dicts) and so is trace (`build_trace_graph`, `FlowHistory`). We hold what DBOS/Temporal build their operator surfaces on and expose none of it. **★ Newly named: `fork` = ZERO hits (a decision, not a debt), and no pre-run topology `validate`. ★ Do NOT build a renderer — navigable beats drawable.** Scope: `CLI_EXECUTION_SURFACE_SCOPE.md` §8; analysis: `COMPARATIVE_RESEARCH_INDEX.md` §4b.
