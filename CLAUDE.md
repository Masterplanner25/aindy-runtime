...
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next (`tool_registry.py:366`). Scope: `docs/runtime/TOOL_SEAM_ISOLATION_SCOPE.md`. **★ A+B+C1 SHIPPED:** revocable DB handle; `register_tool(..., isolation=<assurance class>)` declaring a minimum and refused fail-closed; and `aindy_tool_return_contract_violations_total`, which MEASURES (never rejects — the effect has landed by then) whether a return would survive a process boundary. **★ NONE of them confine: an allowed tool still runs in-process with ambient authority, pinned by a same-pid assertion.** **★★ "Sequence C behind a named consumer" was WRONG and this entry argued it while containing the refutation — the exposure is that the runtime cannot bound what a consumer registers, which is a SUBSTRATE obligation, structural and not contingent. The boundary does not vary by consumer: every tool is `(dict, str, db) -> dict` and the variation is already parameterised by `ExecutionEnvironmentSpec`.** C2 remains: opt-in per tool, cost is latency not design.
