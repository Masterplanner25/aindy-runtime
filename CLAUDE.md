...
- **FLOW-PARALLEL-1** — **CONFLICT HALF SETTLED 2026-09-03 (#569); open for the SCHEDULING half.** Still no fan-out/join/barrier, so apps needing parallelism route AROUND the flow engine, losing history/retry/quota. **★ What shipped is the part that had to come FIRST: a declared per-cell conflict policy (`state_policies`), because `state.update(patch)` silently becomes a COMPLETION-ORDER race the moment a second writer exists, and retrofitting a policy afterwards means auditing every flow that came to depend on the accidental ordering.** **★ An undeclared double-write RAISES — no default is picked, since last-write-wins is right for a latest-reading cell and wrong for a counter; disjoint writes need no declaration.** **★ Determinism is the property, not merging: `last_write_wins` resolves in DECLARATION order; `reduce` takes only commutative+associative ops.** **★ The barrier-vs-independent-commits decision is SETTLED with `EFFECT-PARTIAL-1`: a superstep where some branches fail is a `partial`, and a branch is a unit.** Remaining: the scheduler. Source: `flow_engine/state_merge.py`.
...
- **AUTHORITY-NEGOTIATION-1** — **DESIGNED 2026-09-03, not built.** A denied capability terminates the step; approval is whole-plan, so recovery discards the durable state nine good steps accumulated. Bounded (exactly once), **downgrade-only**, recorded. **★★ The entry's own proposed primitive is NOT needed: `amend_token` is unnecessary here, because a fallback requiring capabilities the token ALREADY grants needs nothing minted. The condition is `required(fallback) ⊆ token.allowed_capabilities` — NOT `⊆` the denied capability, which is the tempting form and says nothing about whether the token grants it either.** **★ The TOOL declares the fallback at registration, never the plan — the thing being constrained must not choose its own constraint (`env_spec`'s rule).** **★ A fallback may not declare a fallback: no chains, so the bound is structural rather than a counter.** **★ Automatic `simulate` substitution is EXCLUDED — feeding predicted output downstream is a lie in `EFFECT-PARTIAL-1`'s sense; simulate belongs at the WAIT gate, informing a human.** Design: `docs/runtime/AUTHORITY_NEGOTIATION_DESIGN.md`.
