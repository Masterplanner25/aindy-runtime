...
- **AGENT-HARDEN-\*** — Open (2026-07-05, from the agent-framework self-assessment). Runtime-owned safety/security/resilience hardening: `-1` emergency-stop/cancel, `-2` HMAC/Ed25519 capability-token integrity (replaces unkeyed SHA-256), `-3` compensating-undo log (rides EffectRecord), `-4`/`-4b` effect-simulation dry-run + env-virtualization (shadow `call_tool` seam), `-5` LLM provider fallback. Each rides existing primitives. MCP is NOT here — it's `ECOGAP-4` (G4a runtime egress/secret-broker, G4b plugin wire adapter).
