...
- **ORCHESTRATOR-SPLIT-1** — **★★ (b) PUBLISHED 2026-09-16: `docs/runtime/DURABLE_STATE_OWNERSHIP_CONTRACT.md`** — one AUTHORITY per unit of work; stores 3/4 are WRITE-ONLY and recovered by NOBODY (the runtime never reads them; the guest sweep cannot resume and is off), so the crash-overlap needs an actor that does not exist. Recommends DECLINING (a) until the runtime wants to READ guest state. **★★ CONFIG HALF SHIPPED 2026-09-09 (#611), and it does NOT close this:** the worker declares `sqlite` + autosweep OFF, compose sets `NODUS_RUN_STATE_ROOT` behind a volume — store 4 was CWD-rooted with NO volume, so guest state was container-ephemeral. **It configures a durability layer; it does not remove one. (a) a `WorkflowStore` over Postgres (`runner.py:492`, NOT `:437` — that moved) and (b) the contract are untouched.** **★ DO NOT trust `migrate-store`: 432 of 629 carried, `skipped: 0`, dropped by MTIME not status.** 
...
- **RETRY-CLASSIFY-1** — *(SWE-agent research)* **retryability is decided by SUBSTRING MATCHING on a lowercased error string** — `_NON_RETRYABLE_SUBSTRINGS` at `retry_policy.py:95` (`"404"`, `"invalid"`, `"not found"`, `"permission"`…). **★ Wired in FOUR places, not a helper awaiting adoption:** flow-node retry, `nodus_adapter.py`, registered as a NODUS HOST FUNCTION, and EMITTED INTO GENERATED AGENT PLANS (`agent_plan_compiler.py`). ★ MEASURED on `execute_tool`'s OWN strings: cancelled, missing-token and enforcement-crashed are all classified RETRY. **★ P2 because the direction is safe — a false positive GIVES UP rather than duplicating an effect — but it is SILENT: nothing records that a classification fired, so it is indistinguishable from a hard failure.** **★ Build WITH `RETRY-CONTEXT-1`** — classify and carry-forward are two halves of one payload. ★ `execute_with_retry` has ZERO callers — four live sites, not five. Design: `docs/design/RETRY_CLASSIFICATION_AND_CONTEXT_DESIGN.md`.
- **RETRY-CONTEXT-1** — *(GPT Engineer research)* **a retry re-attempts the same call; it cannot make a better-informed one.** `execute_with_retry(fn, …)` (`retry_policy.py:224`) calls `fn()` with **no argument carrying the prior failure** — it classifies the error to decide *whether* to retry, and sleeps. `last_error|failure_context|error_context` hits only `plugin_host` reporting fields. **★ Derived twice as two halves of one primitive: gpt-engineer names *why the last attempt failed*; Aider names *which sub-units failed* (→ `EFFECT-PARTIAL-1`, which does NOT cover the whole-call case).** **★ Runtime-level because the runtime owns the loop:** three hand-rolled retry loops across two codebases exist largely to carry a string forward. **★ The boundary that keeps it small — the runtime CARRIES, it does not INTERPRET** (precedent: `is_retryable_error` classifies, never interprets). ★ The channel is a SCOPE, never an argument — a failure folded into `args` changes the `EffectRecord` key and un-dedups the retry. Design: `RETRY_CLASSIFICATION_AND_CONTEXT_DESIGN.md` (with CLASSIFY-1).
...
- **SYSEVENT-RETENTION-1** — *(out of `FR-18`, 2026-08-22)* **`system_events` grows without bound and nothing prunes it.** Stale job logs and expired `EffectRecord` rows are pruned; the table every execution, signal and causal edge lands in is not. FR-18 removed the loudest writer (3.3 GB of one 3.8 GB db), not the class: growth is proportional to work, still unbounded. **★ Not "add a third cleanup job": a `SystemEvent`'s value is uniform by TYPE, not by age.** a blanket age policy deletes the audit trail (`EVENT-OUTBOX-1`, `AUDIT-CORRELATION-1`) and keeps the keepalives. ★★ DESIGNED 2026-09-16: FOUR FKs onto the table are `NO ACTION` (a referenced row cannot be deleted) and `event_edges` is `CASCADE` (the causal graph vanishes silently) — so PRUNE LEAVES ONLY; class per type, unclassified = KEEP; 22 runtime types live outside the enum. **★ Do NOT close by documenting a `DELETE`** — that is the FR-18 mitigation every deployment already runs by hand. Design: `docs/design/SYSEVENT_RETENTION_DESIGN.md`.
...
| **Retry classification + carried context design (RETRY-CLASSIFY-1 + RETRY-CONTEXT-1) — one payload; §2 corrects the census (four live sites, `execute_with_retry` has none); §6 the scope-not-argument constraint** | `docs/design/RETRY_CLASSIFICATION_AND_CONTEXT_DESIGN.md` |
| **`system_events` retention design (SYSEVENT-RETENTION-1) — prune LEAVES only (four `NO ACTION` FKs + one `CASCADE` on the causal graph); class per type, unclassified = keep** | `docs/design/SYSEVENT_RETENTION_DESIGN.md` |
| **Background lease fencing design (LEASE-FENCE-1) — §2 names the least-idempotent job; the fence is a `FOR SHARE` read inside the job's transaction** | `docs/design/LEASE_FENCE_DESIGN.md` |
| **OTel GenAI semconv design (OTEL-GENAI-SEMCONV-1) — an additive EMIT at three seams, not a rename; content capture stays out** | `docs/design/OTEL_GENAI_SEMCONV_DESIGN.md` |
| **Durable state ownership contract (ORCHESTRATOR-SPLIT-1 (b)) — one authority per unit of work; stores 3/4 write-only and unrecovered; §7 recommends declining (a)** | `docs/runtime/DURABLE_STATE_OWNERSHIP_CONTRACT.md` |
...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.19.0.md` — pin bump + rebuild, no schema step, no required code change. Fixes the runtime half of the app's own `SYSCALL-SILENT-ERRORS-1` (#692) — their three `compute_current_state` callers will read HIGHER `active_runs` (the true reading; trigger thresholds tuned on the old zero may fire more). **Consumer-visible: `sys.v1.agent.list_recent_durations` REMOVED** (they never called it). `AINDY_NODUS_MAX_MEMORY_MB` unset by default. DEC-021 touches their `task_service.py:716` paused-task hook — works, two truthful shapes offered, no urgency. Their two 2.17.0 asks stand. Not yet verified by the app. **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** |
