...
- **AGENT-HARDEN-\*** — Open (2026-07-05, from two agent-framework self-assessments). Runtime-owned safety/security/resilience/architecture hardening, each riding existing primitives: `-1` **CLOSED 2026-07-05** — `sys.v1.agent.cancel` (cap `agent.cancel`) + terminal `AgentRunStatus.CANCELLED` + cooperative check at `_execute_agent_segment_chain` segment boundaries (nodus_vm backend); AGENT_FLOW node-granularity interrupt deferred to RTR-1. `-2` HMAC/Ed25519 capability-token integrity (replaces unkeyed SHA-256), `-3` compensating-undo log (rides EffectRecord), `-4`/`-4b` effect-simulation dry-run + env-virtualization (shadow `call_tool` seam), `-5` LLM provider fallback, `-6` **Verifier stage** (post-condition check + verify→rollback; the missing letter in Plan→Dry-Run→Approve→Execute→Verify; pairs with -3), `-7` contract/record-playback tests (respx installed, 0 usage), `-8` declarative per-capability policy (rate/recipient/domain-egress allowlists), `-9` secrets broker (JIT keychain/Vault), `-10` signed plugin bundles + SBOM (provenance currently "unsupported"). MCP is NOT here — it's `ECOGAP-4` (G4a runtime egress/secret-broker, G4b plugin wire adapter). Priority order: 1→2→5, then 3→6→4/4b, with 7–10 opportunistic.
