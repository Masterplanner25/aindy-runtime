...
- **DOCS-\*** — docset findings. **DOCS-BUCKET-A-1 CLOSED.** **DOCS-STALE-1 open (2026-08-13):** seven `docs/runtime/` files declare a `last_verified` *earlier than this repo's first commit* (2026-05-17) — four have only the extraction commit, so nothing in them has been checked against runtime source. `Runtime Docs Validation` checks the five keys are present, never that the date is plausible; a `last_verified >= 2026-05-17` assertion would have caught all seven. **DOCS-COVERAGE-CLAIM-1 (2026-08-13): docs corrected, coverage gap open** — 6 docs cited 8 test files that have *never existed in either repo*, several with counts ("61 tests (Groups A–K)"); `NATIVE_MEMORY_BRIDGE.md` did it under the heading *"Focused tests that exist in this repository"*, and nothing under `tests/` touches `memory_bridge_rs` at all — so the native scorer has no behavioural coverage in either language (agrees with NATIVE-CI-1). MAS, OS isolation, event bus, native scorer each lack a suite; overlaps ECOGAP-6, don't double-track.
