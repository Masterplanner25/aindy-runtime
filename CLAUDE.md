...
- **EXEC-ENV-BIND-1** — **PHASES 1+2+3 SHIPPED (3 on 2026-09-03, #567); open for 4 only.** 1: an EU can DECLARE its environment (three columns, Alembic `0017`) — declare/refuse/record, **confines nothing**; `env_evidence_class` says whether anything was enforced. 2: the GUEST path asks — `nodus_worker` derives every confinement arg from a spec clamped to `GUEST_FLOOR`. 3: the TOOL seam asks — `register_tool(..., env_spec=)` bounds the worker's env, cwd and clock; **an undeclared tool gets NO spawn kwargs, which is what made landing it safe.** **★ A declared spec is CLAMPED to its floor, never merged — nothing widens its own sandbox; and `assurance_rank()` ranks unknown LOW, because ranking it high makes a typo satisfy every minimum.** **★ Neither seam can enforce `authority.{network,subprocess}` — no spawn argument does; hence the `insecure-dev` tier and the container runner.** **★ Phase 4 (resources become ENFORCING) is gated on `COST-GOVERNOR-1`'s governor half, itself gated on the LLM seam having a consumer — do not start it first.** Design: `docs/runtime/EXECUTION_ENVIRONMENT_SPEC_DESIGN.md`.
...
- **FLOW-PARALLEL-1** — **CONFLICT HALF #569; PHASES 0+1 SHIPPED (1 on 2026-09-10, #616); open for 2–4.** `FanOutEdgeGroup` runs a node's successors as ONE superstep: per-branch sessions, one contiguous ordinal block at the barrier, central merge on the runner's session. Default-OFF. **★ An undeclared double-write RAISES; determinism is the property — `last_write_wins` resolves in DECLARATION order.** **★★ The width bound is PROCESS-WIDE, not per-run: runners come from request handlers, syscall dispatch, rehydration AND scheduler recovery, so per-run W allows runs×W sessions against the budget SHARED with request handling (`SYSMAX-5`).** **★ The flag gates CONCURRENCY, not SEMANTICS — off runs the group sequentially, same result.** **★ Phase 1 ENFORCES convergence on one successor; join policies are phase 2 — fan-out without a join is half a primitive, do NOT close on it.** **★★ Mutation testing found the one gap that mattered: "branches share the runner's session" survived everything else.** Design: `docs/runtime/FLOW_PARALLEL_DESIGN.md`.
- **COST-GOVERNOR-1** — **METER SHIPPED 2026-09-03 (#563, #564); ADOPTED 2026-09-08 (app #321); the GOVERNOR is still BLOCKED, and not on its design.** The runtime enforces a 300s wall-clock and 256MiB ceiling on work whose dominant cost is tokens and did not measure them — the usage object was **DISCARDED at the boundary**, so it lived one stack frame. `aindy_llm_tokens_total` records it on the **RAW** response path, never `chat()` (which delegates there — metering both double-counts, and a silently-2x number is a fabricated measurement a governor would reserve against). **★★ The old blocker — *“the seam has NO CONSUMER”* — IS GONE: the app's Claude planner routes through it. ★ Naming the wrong one is how this gets built early: what remains is EVIDENCE the meter moves in a real deployment (it shipped in v2.9.0 — an older install records NOTHING) and run/tenant IDENTITY at the call site.** Scope: `LLM_SEAM_ADOPTION_SCOPE.md`. Budget scoping is SETTLED (agent run + tenant, both binding; refuse on breach). Source: `TECH_DEBT.md`.
- **AUTHORITY-NEGOTIATION-1** — **PHASES 0+1 SHIPPED (1 on 2026-09-10); open for 2–3.** A denied capability terminates the step; approval is whole-plan, so recovery discards the durable state nine good steps accumulated. Bounded (exactly once), **downgrade-only**, recorded, **default-OFF** (`AINDY_AUTHORITY_NEGOTIATION`, opt-IN). **★★ It CANNOT grant authority, structurally: it picks WHICH TOOL to attempt and `execute_tool` re-runs `check_tool_capability`, so §7's no-widening rule is unreachable, not merely unauthorised.** **★ The §2 subset rule is ASKED of the enforcing code, never reimplemented — a hand-rolled check omits granted_tools AND agent capabilities.** **★★ The design said TWO `CAPABILITY_DENIED` sites; there are FOUR and exactly ONE is negotiable — the rest deny a MISSING TOKEN or run-level `execute_flow`, where there is no tool (variant 12, corrected in §1).** **★ The TOOL declares it, never the plan. ★ Phase 0's inertness guard was REPLACED, not deleted.** Design: `docs/runtime/AUTHORITY_NEGOTIATION_DESIGN.md`.
...
- **APP-FR-\*** — app-side feature requests from `aindy-apps-monolith`. **Next available: FR-28.** FR-1..13, 16..18, 20..22, 24, FR-19's runtime half and **FR-25 (a)+(c) (2026-09-11)** shipped. **★ FR-22: `/apps/*` is NOT an ownership boundary — 35 such routes are served by the RUNTIME alone; inventory `AINDY/route_inventory.json`.** Open: **FR-14** (exit **3** additive-reconcile is the only branch safe to automate; **★ the half that prevents recurrence — the upgrade path is never run against an EXISTING database — is still open**), **FR-6 items 2+3** (no `email` connector), **FR-23** (`/observability/system` counts `platform_layer._syscalls`, a dict DISPATCH NEVER READS → 0 of ~90), **FR-25 (b)** (`_id: str` path params 500 on a bad id; route contract → approval), **FR-26** (pipeline mints a 2nd trace id so body and `X-Trace-ID` disagree; one-line default → approval), **FR-27** (`EXACTLY_ONCE` degrades **N−1 of N** concurrent duplicates — IDEM-11 said "twice"; advisory lock, proposal-first). All verified against source 2026-09-11. FR-18's retention half is `SYSEVENT-RETENTION-1`.
...
| Latest app-team handoff | `docs/runtime/APP_HANDOFF_v2.11.0.md` — **★★ read its 2026-09-11 CORRECTION first: §0's "you are on 2.6.0" was true of their dev venv (and of what their `pytest` imports) and NOT of their deployment — the container installs `constraints.txt`'s pin (2.9.0 → 2.11.0) and the DB is past `0018`. Their rebuttal was wrong too: no editable link exists; `import AINDY` says 2.11.0 only when cwd is THIS repo. ★ Verify a version by PRINTING `AINDY.__path__` beside the number, from the directory the thing runs in — `importlib.metadata`, `pip show` and `__version__` are all cwd-sensitive.** No required code change in 2.10.0 or 2.11.0 |
