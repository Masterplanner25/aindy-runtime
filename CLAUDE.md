...
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next: `execute_tool` evaluates token/tools/capabilities/policy/egress, then calls the tool **in-process with the live DB session** (`tool_registry.py:366`). **★ SCOPED 2026-08-19 — `docs/runtime/TOOL_SEAM_ISOLATION_SCOPE.md`, and it corrects four things: (1) a tool is a Python CALLABLE, not a command, so the "settled" command-transform answer is one indirection off — you transform a tool WORKER's argv, and the serialization boundary is the real cost; (2) the three call sites are in THREE DIFFERENT processes and only `nodus_adapter.py:263` is host-side; (3) ZERO foreign in-process tools exist at HEAD — MCP tools are runtime-owned proxies — so the gap is structural, not live; (4) all 18 tool fns take `db` and NONE uses it, making the revocable-handle step a measured no-op.** Recommendation: ship the handle, declare `isolation=`, do not build the process boundary without a named consumer.
