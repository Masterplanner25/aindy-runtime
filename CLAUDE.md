...
- **IDEM-11** — at-most-once is built and **shipped disabled**; the per-syscall audit is DONE 2026-08-15, the flag flip is not. The gate fires only when `AINDY_SYSCALL_IDEMPOTENCY` is on, the entry declares `EXACTLY_ONCE`, an EU id is present, and `_gate_scope_engaged`. **★ A fourth path the filing omits: `_durable` (DUR-2) engages the gate for ANY syscall, bypassing both the flag and the declaration.** **★ `register_syscall` had NO `execution_guarantee` parameter** — `SyscallEntry` accepted it, the function never forwarded it, so the gate was unreachable for plugin syscalls by construction; now forwarded and validated. **★ The gate caches the handler's return in JSONB and the syscall path had no serializability check** — a `UUID`/`datetime` return came back as an error envelope *after the effect had landed*; guard ported from the tool path. Declared entries 1 → 7. Remaining = flip after soak. Guard: `tests/unit/test_syscall_execution_guarantee.py`.
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next: `execute_tool` evaluates token/tools/capabilities/policy/egress, then calls the tool **in-process with the live DB session**. Proposed `register_tool(..., isolation=...)`, failing closed, default unchanged. **★ How it ATTACHES is settled: a command TRANSFORM, not an execution ABC** — take command+policy, return argv carrying its own sandbox wrapper (`codex-rs`'s `SandboxManager`). It fits because there is exactly ONE chokepoint (`tool_registry.py:366`) reached from THREE call sites, so a transform needs zero backend implementations where an ABC would need N. **★ With `GUEST-CONFINE-1` closed this is the LAST seam where foreign code runs unconfined with ambient authority**, and it is the enforcement point `FS-SCOPE-1` has none without.
...
- **EXEC-ENV-BIND-1** — **PHASE 1 SHIPPED 2026-08-19; open for phases 2-4.** An execution unit can now DECLARE the environment it needs (`env_spec` on `require_execution_unit`; three columns on `execution_units`, Alembic `0017`). **★ It CONFINES NOTHING — declare/refuse/record only. A populated `env_applied` is not evidence of confinement; `env_evidence_class` is the field that says whether the environment was enforced.** **★ The re-raise guard is the mechanism: `require_execution_unit` ends in a broad `except Exception` returning `None`, so `except ExecutionEnvironmentError: raise` sits BEFORE it (the `SyscallContractViolation` shape) — and it catches the BASE class so a malformed spec propagates too.** **★ `assurance_rank()` ranks unknown LOW on purpose; ranking it high makes a typo satisfy every minimum.** The clamp is narrow-only and unflagged, so the worst failure is a loud over-strict refusal, never a silent under-confined run. Design: `docs/runtime/EXECUTION_ENVIRONMENT_SPEC_DESIGN.md`.
...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. **★ Corrected 2026-08-18: the filed entry said THREE and missed `nodus_lang_workflow`, a SQLite `LocalWorkflowStore` that independently reimplements this runtime's whole durability vocabulary** (claims, waits, retry scheduling, rehydration, terminal classification). Failure mode is crash-only: `continue_crashed_agent_runs` restarts a partially-executed segment from step one, so a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ Worse than untracked — UNCONFIGURED, and it loses data: `AINDY/` has ZERO references to it, so the store roots relative to the worker's CWD — which no spawn path sets — in a directory with NO compose volume.** Store 4 IS injectable (`runner.py:437`), so implementing `WorkflowStore` over our Postgres collapses it into store 1. Same missing `cwd=` as `GUEST-CONFINE-1`'s residual.
...
- **APP-FR-\*** — app-side feature requests from `aindy-apps-monolith`. **Next available: FR-17.** FR-1..13 (incl. 12b) shipped. Open: **FR-14** — `bootstrap-schema` refuses additive drift, so a release adding a runtime column crash-loops under `set -e` + `restart: unless-stopped`; **their claim that `serve` self-migrates is VERIFIED FALSE.** Branchable exit codes shipped (#450): **3** additive-reconcile-required (*the only one safe to automate*), **4** offline-migration-required, **5** manual-repair — 4 ranks above 3 so an entrypoint never auto-reconciles a DB needing a person. **★ STILL OPEN, the half that prevents recurrence: the upgrade path is never exercised against an EXISTING database** — CI builds a fresh one where `create_all` makes the columns, so no green check can see this class; the same blind spot hid FR-8. Also open: **FR-6 items 2+3**, blocked structurally — the runtime ships no `email` connector; recommend runtime SMTP or hybrid, never the app-connector option that inverts the split.
...
- **GUEST-CONFINE-1** — CLOSED 2026-08-15. The guest VM ran unconfined — `nodus_worker.py` passed none of the confinement args, so a guest script reached subprocess/network/host env without touching the dispatcher, token, ledger, egress guard or tool registry. **Demonstrated, not inferred.** **Deliberately not env-configurable: a global flag re-opens the hole for every run at once; per-execution variation is `EXEC-ENV-BIND-1`.** **★ RESIDUAL OPEN: nothing sets the guest's `cwd`, so `allowed_paths` inherits the server's working directory** — `/home/aindy` in Docker, which holds `alembic/`, so a guest can rewrite migrations that run on next boot. The escape is closed; the *bound* is an undeclared inherited default. Smallest true instance of `FS-SCOPE-1`, **but do not close `FS-SCOPE-1` with it.**
...
- **NODUS-WARMPOOL-1** — CLOSED 2026-07-19. Warm worker pool (`AINDY_NODUS_WARM_POOL`, default off) + boot-allowance clock split; falls back to a fresh subprocess on any fault. **★ Its soak has a precondition, found 2026-08-19: on `nodus-lang <=5.0.2` a reused worker leaked guest memory between tenants, so the pool's own safety claim was false. Before enabling the pool after ANY dependency bump, re-run `test_two_runtimes_in_one_process_do_not_share_guest_memory` — a reused process is only as isolated as its most import-bound dependency.**
