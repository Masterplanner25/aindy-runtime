...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. Failure is crash-only: a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ CONFIG HALF SHIPPED 2026-09-09 (#612), and it does NOT close this:** the worker declares `sqlite` + autosweep OFF, compose sets `NODUS_RUN_STATE_ROOT` behind a volume — store 4 was CWD-rooted with NO volume, so guest state was container-ephemeral. **It configures a durability layer; it does not remove one. (a) a `WorkflowStore` over Postgres (`runner.py:492`, NOT `:437` — that moved) and (b) the contract are untouched.** **★ Store 4 is WRITE-ONLY from the runtime, which is why declaring beat migrating — and DO NOT trust `migrate-store`: it carried 432 of 629 reporting `skipped: 0`, dropping files by MTIME past 30d, not status.**
