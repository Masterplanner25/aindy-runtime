...
- **NODUS-UPGRADE-2** — *(CrewAI/Nodus research)* P3, routine: pin is `nodus-lang==5.0.1`, Nodus is at **`v5.0.4-2`**. **★ `NODUS-UPGRADE-1` is closed but its rules are not — bump ALL THREE sites (`pyproject.toml`, `AINDY/requirements.txt`, and the `Install MCP extra` step in `runtime-ci.yml`, which installs directly; `--no-deps` means pyproject pins are NEVER applied in CI).** Check `nodus-mcp` resolves first (`MCP-SDK-2X-1`: a cap there once blocked the 5.0.0 bump, and isolating the MCP tests would have shipped an uninstallable extra). **Sequence with the `GUEST-CONFINE-1` residual and the `ORCHESTRATOR-SPLIT-1` store work — all three touch the guest seam, so one confinement re-verification instead of three.**
