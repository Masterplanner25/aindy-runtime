...
- **★ EVENTBUS-PUBLISH-LATCH-1** — **three consecutive failed publishes disable cross-instance WAIT/RESUME propagation permanently.** `EventBus.publish` sets `self._enabled = False` on the third failure (`event_bus.py:192-198`) and **nothing ever sets it back** — when Redis recovers, `publish` returns `False` from the top guard without even contacting the server (verified). The bus exists so a flow that WAITed on instance A can be resumed by an event on instance B; once latched, that stops for the life of the process, so a *transient* Redis blip silently produces exactly the failure the module prevents. Only a restart cures it; nothing surfaces the state but one WARNING at latch time. **The module docstring misleads here** — "reconnects with exponential back-off" describes the *subscriber*; the publisher has no recovery path. `_consecutive_failures` resets on success, so intermittent failures never latch — it is specifically three in a row. **Open.**
