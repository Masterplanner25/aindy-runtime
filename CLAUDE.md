...
- **TOOL-SEAM-ISOLATION-1** — every authority check at the tool seam is advisory with respect to the code that runs next: `execute_tool` evaluates token/tools/capabilities/policy/egress, then calls the tool **in-process with the live DB session**. Proposed `register_tool(..., isolation=...)`, failing closed, default unchanged. **★ How it ATTACHES is now settled (2026-08-17, `codex-rs` via the Claude Code research set): a command TRANSFORM, not an execution ABC. `SandboxManager` takes command+policy and returns argv carrying its own sandbox wrapper — one spawn path, isolation rides in argv. Fits because this runtime has exactly ONE chokepoint (`tool_registry.py:366`) reached from exactly THREE call sites, so a transform needs zero backend implementations where an ABC would need N execution environments we don't have. `isolation=` is the declaration; the transform is the application.** **★ Re-confirmed P0 2026-08-17 and the reason is stronger than at filing: with `GUEST-CONFINE-1` closed, `tool_registry.py:366` is the LAST seam where foreign code runs unconfined with ambient authority.** Independent corroboration — the Aider portability analysis attaches exactly one safety precondition to its whole port, and it is this seam: *"a gated path that does not actually confine would be worse than the status quo it replaces."* It is also the enforcement point `FS-SCOPE-1` has none without.
...
- **FLOW-PARALLEL-1** — no fan-out/join/barrier; plan steps are strictly sequential, so independent calls cost the sum of their latencies. Apps needing parallelism route *around* the flow engine, losing history/retry/quota. **Determinism is the load-bearing part of any fix, not speed** — merge output patches in declaration order. **★★ But declaration-order answers ORDERING, not CONFLICT — this line was under-specified (LangGraph research, 2026-08-18). `runner_steps.py:257` is `state.update(patch)`, i.e. LAST-WRITE-WINS, harmless today because there is never a second writer and silently wrong the day this lands. LangGraph's shape is the input: conflict resolution DECLARED per cell (`LastValue` REJECTS >1 writer, `BinaryOperatorAggregate` folds, `NamedBarrierValue` joins) — the runtime should require a declared policy and fail loudly on an undeclared double-write, not pick one. Same question the join semantics asks, from the write side. Settle WITH `EFFECT-PARTIAL-1`.** **★ A worked reference design now exists (MAF, `_edge.py`/`_runner.py`) — read the entry before designing. The reframe makes it smaller than filed: a superstep is the EXISTING per-node commit boundary WIDENED to a barrier-delimited group, not "add concurrency." Their four edge groups are all `DictConvertible` (topology as data), `SwitchCaseEdgeGroup` subclasses `FanOutEdgeGroup` (a switch is a constrained fan-out), and their documented negative result is that PREDICATES DO NOT SERIALIZE — harder for us, since ours are Python closures (`node_executor.py:57`). Open decision: barrier-as-commit-boundary vs. independent branch commits decides whether a partially-failed group is one `FlowHistory` row or several — settle it WITH `EFFECT-PARTIAL-1`. Refuse their durability posture: MAF delegates crash-durable orchestration to Azure Durable Task.**
- **FLOW-GRAPH-SIGNATURE-1** — *(MAF research)* a suspended run resumes against **whatever flow definition exists now**; `graph_signature|topology_hash|definition_hash` = **zero hits repo-wide**. A node renamed or an edge rerouted across a deploy resumes **silently and successfully** against a definition the run was never planned for. **★ Sharpened by `continue_crashed_agent_runs`, which re-drives from a SEGMENT boundary — so a changed graph re-executes a segment whose meaning has moved.** Fix is cheap and loud: hash topology at run start, store on `FlowRun`, quarantine on mismatch (MAF does exactly this — `_runner.py:51` writes it, `:275` compares it). **★ The design question decides whether it is worth anything: hash node identities + edge topology, NOT node bodies or predicates — a hash that changes every deploy quarantines every in-flight run and gets switched off in a week.** Independent of FLOW-PARALLEL-1; do not bundle.
- **COST-GOVERNOR-1** — *(MetaGPT research)* **every quota exists except the one that matters for an LLM runtime.** `resource_manager.py:71-74` has exactly four dimensions — wall-time (300s), memory (256MiB), syscalls/execution (100), concurrent/tenant (5). **No token/cost/spend dimension.** **★ Worse than a missing cap — there is NO METER: a repo-wide grep for `prompt_tokens|completion_tokens|total_tokens` hits only `context_builder.py`, which sizes a MEMORY CONTEXT to a prompt window. Token usage from an LLM response is never captured.** Fix is meter-then-govern, and the meter is the bigger half. **★★ ALL FOUR of the entry's design questions are answered in shipped code (LiteLLM `c696fdf`, MIT): mechanism is RESERVE → call → RECONCILE — atomically pre-fill counters at admission (so N concurrent requests can't all pass a read-then-compare), replace with actual after, with a mid-flight cancellation release. Q3 'estimated or actual' = BOTH. Q1 checked in a pre-call hook against a Redis+in-process counter, never the DB on the hot path. Q4 fail-open is CONFIGURABLE — take the configurability, keep OUR default (`resource_manager` closed-in-prod). ★ And the metering half is bigger than stated: it needs PARTITIONING + a cleanup job for the spend log before anything else.** **★ The line worth keeping: the runtime enforces a 300-second wall-clock and 256MiB memory ceiling on execution units whose dominant cost is tokens, which it does not measure.** **★ Do NOT fold into `BILLING-2` — that is REVENUE metering, deferred to commercial launch; a governor is needed the first time an agent loops, not the first time an invoice is issued.** P1 because it is the only quota whose absence is *unbounded* — the other three self-limit. Converges with the Linux Lesson 5 axes finding on `EXEC-ENV-BIND-1`: same under-served *resources* axis, from an unrelated comparand.
...
- **FS-SCOPE-1** — *(Aider research, see the provenance note)* the capability vocabulary is **verb-shaped**, so no authority statement can name a path. `register_tool` carries `egress_scope` for network; `allowed_paths|path_scope|writable_root|allowed_dirs|fs_scope` returns **one hit repo-wide and it is a comment**. The runtime can say *may this run reach the network under scope X* and cannot say *may this run write `src/**` and nothing else*. **★ Do NOT build it as `fs_scope` beside `egress_scope` — that is a second vocabulary for the question `EXEC-ENV-BIND-1` already asks. It is a field ON the descriptor, and its enforcement point is `TOOL-SEAM-ISOLATION-1`'s. One structural change, three entries.** Smallest true instance available today: `GUEST-CONFINE-1`'s residual. **★ A shipped reference for the vocabulary exists: `codex-rs`'s `SandboxExecRequest` carries `file_system_sandbox_policy` and `network_sandbox_policy` as PEER fields, plus `SandboxablePreference {Auto, Require, Forbid}` for fail-closed — default-on across three OSes, so the shape is proven not proposed.**
...
- **SUBSTRATE-WITNESS-1** — *(Claude Code research)* **the substrate claim has no first-party consumer that exercises it.** Measured against `C:\dev\claw` 2026-08-17: the flagship app (18,766 LOC) integrates in **334 lines across 3 files**, all optional, mostly HTTP, `memory_backend` defaults to `"local"`, and `execute_tool`/`EffectRecord`/`execution_token` appear **zero** times in its own source (only inside its vendored venv copy of AINDY). Its real effects — channel delivery, skills, scheduling — cross no chokepoint. **★ The corollary for reading this file: the coverage percentages across nine comparative audits (90–95% Devika … 15–25% Aider) describe capabilities the runtime HAS, not capabilities anything USES.** **★ This is why the flag backlog is stuck: soak needs production traffic THROUGH the path being flipped, and no first-party consumer sends any — so the soak eight entries wait on cannot currently be run at all. Pair with `PERF-BASELINE-1`: one says no instrument, this says no traffic.** Recommended slice: route **only** Claw's outbound message delivery through `execute_tool` with `EXACTLY_ONCE`. **★ Do NOT close this with a synthetic fixture — a harness proves the path executes (the unit suite already does); what's missing is a consumer that would NOTICE if the guarantee broke.**
- **PERF-BASELINE-1** — *(Aider research)* **zero latency assertions across `tests/`**; every `duration_ms` reference is a type/shape check. **★ P1 for an internal reason: eight registry items (`IDEM-11`, DUR, `RTR-4`, `DB-NODUS-BUDGET-1`, `FR-15 (a)`, `AUTHORITY-VALUE-1`, `INFINITY-RUNTIME-1`, `NODUS-WARMPOOL-1`) have "soak then flip" as their remaining work, and no instrument exists that could produce the evidence.** The flag backlog is blocked on measurement, not courage. **★ Variant-9 trap: a timing assertion on a suite that never exercises the durable path passes trivially — measure with the gate ON, and pair every threshold with a control that makes it fail.**

### Open — P2 and below

- **KEY-SCOPE-ESCALATION-1** — **Re-levelled P0 -> P2 2026-08-18: both escalations FIXED (#463, #465); the residual is tidiness, not a hole.** A `flow.read`-only API key could mint itself `platform.admin` and promote its own user row (**survives revoking the key**), and separately could **rotate the platform signing key**, choosing the new secret — i.e. forge tokens for anyone. Root cause: `require_platform_admin_access` returns ANY `auth_type=="api_key"` unconditionally while its docstring claims per-endpoint enforcement that did not exist. Fixed by a delegation rule (`grantable_scopes()`) plus per-endpoint scopes on 44 `/platform` routes. **★ Gotchas: SQLite CANNOT reproduce it — `platform_api_keys.scopes` is a PG `ARRAY`, so a 201 shows up as a 500 and reads as an unrelated bug; and a 400 from `rotate-secret-key` can mean "same as the current key", raised AFTER authorization, so a status-code-only probe mis-reads validation as authorization.** `POST /platform/syscall` + `GET /syscalls` stay route-ungated **by decision** (authority is per-syscall; gating breaks every SDK caller), pinned by equality. Residual: the two admin guards disagree about what an API key is. Source: `TECH_DEBT.md`.
- **HTTP-SCOPE-GAP-1** — **Re-levelled P0 -> P2 2026-08-18. Route coverage is COMPLETE; the remainder is a design question, not a gap.** Census on a booted app: **91 scope-gated / 12 admin / 21 public / 2 identity-only of 126** (was 7 gated). A JWT session derives scopes from `User.is_admin` **per request, not from a token claim**, so no session is invalidated and a grant lands on the next call. **★ Three gotchas that still bite: (1) a per-route `dependant` walk UNDER-reports enforcement — router-level `dependencies` are excluded and `_IncludedRouter` hides the nesting, which produced a "97 of 126 enforce nothing" figure wrong by 56; (2) `enforce_api_key_scope` takes ANY-OF alternatives, and adding that form silently blinded a regex-based safety scan; (3) scanning `app.routes` for a path prefix finds ZERO — use `_iter_api_routes` + endpoint `__module__`.** The 2 identity-only routes (`/auth/logout`, `/auth/password/change`) act only on the caller's own account and are pinned **by equality**. Remainder: `execution.read` conflates scope with data ownership — a scope cannot answer *"may I read someone else's"*. Source: `TECH_DEBT.md` HTTP-SCOPE-GAP-1.
...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract.** The split may be correct; it is undocumented, so it can be neither relied on nor reviewed. **★ Corrected 2026-08-18 (CrewAI/Nodus research): the filed entry said THREE and missed `nodus_lang_workflow` — a SQLite `LocalWorkflowStore` that independently reimplements this runtime's whole durability vocabulary (claims, waits-with-expiry, retry scheduling, rehydration, terminal classification), not merely a checkpoint file like `task_graph.py`.** Failure mode is narrow and crash-only: `continue_crashed_agent_runs` restarts a partially-executed segment from step one, so a guest that rehydrated its own SQLite claim/wait/retry state disagrees with the host about what already happened, and nothing detects it. **★★ Worse than untracked — UNCONFIGURED, and it loses data: `AINDY/` has ZERO references to `nodus_lang_workflow`/`WorkflowStore`/`NODUS_WORKFLOW_STORE_ROOT`, so the store roots at `.nodus/workflow_framework` relative to the worker's CWD — which no spawn path sets — landing in `/home/aindy`, a directory with NO volume in compose. Guest claims/waits/retries are container-ephemeral, plus an auto-started daemon sweep thread we never stop.** **★ Fix (a) named the wrong hook: `set_effect_store` (`nodus/runtime/embedding.py:530`) reaches store 3, not store 4 — but store 4 IS injectable (`runner.py:437` `self.store = store or LocalWorkflowStore()`), so implementing `WorkflowStore` over our Postgres collapses it into store 1. Interim: set `NODUS_WORKFLOW_STORE_ROOT` to a declared volume-backed path.** Same missing `cwd=` as `GUEST-CONFINE-1`'s residual — one line closes both.
...
- **RECOVERY-GRANULARITY-1** — *(LangGraph research)* **recovery granularity is welded to scheduling granularity: the runtime checkpoints at the boundary of the unit it schedules, so any control flow INSIDE that unit is invisible to recovery.** **★ We already have the repair at one layer and did not carry it down: the flow layer commits a `FlowHistory` row with `input_state`+`output_patch` BEFORE the snapshot/`current_node` advance (`runner.py:347-359`) — that IS pending-writes-then-checkpoint. The agent layer does not: `AgentStep` is a post-segment batch write (its own docstring says so), so `_count_completed_segments` restarts a partially-executed segment from step ONE.** **★ P2 not P0 — `DUR-2` means mediated effects don't double-fire, so the re-run is CORRECT; what it costs is WORK — a crashed agent run re-issues every LLM call in that segment.** **★★ Worked reference (DBOS, 2026-08-19): `operation_outputs` = one row per step keyed `(workflow_uuid, function_id)`; `_core.py:2098` checks it before every step and REPLAYS the recorded output instead of executing. Step granularity — exactly ours. And its monotonic ORDINAL is a cheaper identity than a vector clock, and the right one to build FIRST (a vector clock only earns its keep post-fan-out).** Companion idea worth taking: `versions_seen` as **one** vector clock driving both readiness and resume, so no second "what was done" ledger is invented. **★ Does NOT reopen the declined kernel-replay decision — see `ECOGAP-1`'s taxonomy row 4; none of the three reasons for declining #2 apply to #4.** Three peers (LangGraph, OpenHands, MAF) arrived at it independently.
- **RETRY-CLASSIFY-1** — *(SWE-agent research)* **retryability is decided by SUBSTRING MATCHING on a lowercased error string**, `_NON_RETRYABLE_SUBSTRINGS` at `retry_policy.py:95` (`"404"`, `"invalid"`, `"not found"`, `"permission"`…). **★ Wired in FIVE places, not a helper awaiting adoption: flow-node retry (`runner_steps.py:266`), `nodus_adapter.py:279`, registered as a NODUS HOST FUNCTION (`nodus_worker.py:389`), and EMITTED INTO GENERATED AGENT PLANS (`agent_plan_compiler.py:55/:67/:145`).** Concrete false positives: `"404"` matches `"took 404ms"`; `"invalid"` matches `"invalidated cache"`; `"not found"` matches a DNS message. **★ P2 because the direction is safe — a false positive GIVES UP on a retryable error rather than duplicating an effect — but it is SILENT: nothing records that a classification fired, so it is indistinguishable from a hard failure.** Sharpest instance is the generated one: an agent's retry behaviour inside a guest VM decided by substring-matching an error string a model may have shaped. **★ Build WITH `RETRY-CONTEXT-1` — classify (this) and carry-forward (that) are two halves of one payload.** ★ Gotcha: `is_retryable_error`'s docstring says the system doesn't use it — **stale**, not a published-and-unconsumed instance.
- **RETRY-CONTEXT-1** — *(GPT Engineer research)* **a retry re-attempts the same call; it cannot make a better-informed one.** `execute_with_retry(fn, …)` (`retry_policy.py:224`) calls `fn()` with **no argument carrying the prior failure** — it classifies the error to decide *whether* to retry (`is_retryable_error`) and sleeps, and that is all. Repo-wide `last_error|failure_context|error_context` hits only `plugin_host` reporting fields. **★ Derived twice as two halves of one primitive: gpt-engineer names *why the last attempt failed* (`_improve_loop` re-sends the diff error, `self_heal` re-sends stderr); Aider names *which sub-units failed* (→ `EFFECT-PARTIAL-1`, which does NOT cover the whole-call case).** **★ Runtime-level because the runtime owns the loop: three hand-rolled retry loops across two codebases exist largely to carry a string forward.** **★ The boundary that keeps it small — the runtime CARRIES, it does not INTERPRET. Precedent: `is_retryable_error` classifies, never interprets.** Guard rails: bound the carried history (context-window bomb), no authority/no effect (as `PROGRESS-CHANNEL-1`), and don't let callers branch on error shape.
...
- **INITIATOR-IDENTITY-1** — *(OpenClaw research)* **the identity that initiates work is not the identity the runtime authenticates.** `tenant_context.py:13` — `tenant_id == user_id`, established by the dispatcher requiring an authenticated caller. **★ That holds only while work is REQUESTED.** An inbound-event-driven consumer (OpenClaw: 8 chat channels, work starts when a message ARRIVES) has one authenticated operator and N end-user identities the runtime cannot name. **Collapses: memory namespaces per operator not per peer (one peer can recall another's context), `MAX_CONCURRENT_PER_TENANT` becomes a whole-deployment cap, and the audit trail says "the operator did this" when a peer asked.** **★ NOT `DEPLOY-TARGET-2`** (many operators — opposite axis) **nor `PAYMENTS-ARCHITECTURE-1` q3** (the billing version). Needs an *acting subject* beside `user_id` — **attributed, not authenticated**, and honest about which it holds (cf. `AUTHORITY-VALUE-1`). **★★ The accounting half is SHIPPED by a peer in our domain: LiteLLM budgets at `end_user` scope alongside key/user/team/org — an asserted identity used for limits, never for authorisation. ★ The rule that keeps it safe: an asserted subject may only CONSTRAIN, never widen or SELECT — if it namespaced memory it would be making a read-authorisation decision in accounting clothes.** **★ Do NOT make the peer a `User` row** — that authenticates an identity we cannot verify. **P2 today, P0 the day an inbound-driven consumer ships: first symptom is cross-peer memory recall, a data-leak class.**
- **LEASE-FENCE-1** — *(Pi research)* `BackgroundTaskLease` has `owner_id`+`expires_at` and **no fencing token** — `fence|fencing|epoch` returns **zero** under `AINDY/`; `leadership.py`'s docstring is *"Row expired → take over."* **★ Expiry bounds how LONG two leaders coexist and does nothing about what the stale one WROTE — a process that lost its lease to a GC pause learns it at the next renew, and acted as leader throughout.** Pi's `writer-leases.ts` is the reference: conditional upsert that steals only an expired lease and `fence = fence + 1` on every takeover, so a stale writer is **refused** rather than asked to notice. **★ P2, not a live corruption path — it guards leadership (which instance runs maintenance), not writes, and those jobs are largely idempotent. Filed because the fix is one integer column, the failure mode is INVISIBLE when it happens, and an agent-loop library — the layer this corpus excludes 13× — has the better primitive than the substrate whose job it is.** Check which leader-only job is least idempotent before fencing uniformly. Note store 4 (`nodus_lang_workflow`) has an unfenced `claim` too. **★★ Second witness 2026-08-19: Temporal fences shard ownership the same way — `ShardOwnershipLostError` on a conditional update carrying `PreviousRangeID`/`RangeID` (`data_interfaces.go:144, :183-228`), its CORE ownership mechanism. System-fact ×2. ★ And the gap was named by `TEMPORAL_AINDY_LENS_AUDIT.md` on 2026-06-24 (*"monotonic-RangeID storage-CAS fence vs aindy's DB-lease, now real but weaker"*) and NEVER FILED — opened here 8 weeks later believing it new. The June audits' surviving-gap lists are a filing queue that was never worked; this is the second item found in them with no entry (cf. `COST-GOVERNOR-1`).**
- **AUTHORITY-LIFETIME-1** — *(OpenHands research, its O3)* **the capability token is bound to the clock, not to the execution it authorises.** `capability_service.py:25` — `TOKEN_TTL_HOURS = 24`; `revoke|revocation|invalidate` returns **zero** in that module. **A token minted for a run that finished in 90 seconds stays valid for the rest of the day** — nothing about reaching a terminal state invalidates it. Reference: OpenHands binds its session key to lifecycle, not clock — *"only valid while the sandbox is RUNNING"* (`session_auth.py:13`, enforced `:76`), nulled on pause, rotated on resume. **★ Composes rather than competes: `capability_ceiling` answers WHAT MAY THIS BEARER DO (ours, stronger — they have no grant set at all); RUNNING-only answers WHILE WHAT IS TRUE (theirs, we have no analogue).** **★ The real cost is turning a stateless HMAC check into a stateful one on the hot path — settle where the check lives, whether a NEGATIVE cache is used (terminal is terminal), and fail-open-vs-closed (follow `resource_manager`'s prod/dev split) before writing code.** P2: defence-in-depth on an already-strong component. **Not `AUTHORITY-VALUE-1`** (caller-supplied values) **nor `KEY-SCOPE-ESCALATION-1`** (closed, API-key scopes) — this is a correctly-issued token outliving its run.
- **EVENT-OUTBOX-1** — *(§D absorb-register sweep — the "+ transactional outbox" half of the Temporal bullet, unfiled since June)* **system events are buffered IN MEMORY and emitted AFTER the work commits.** `execution_signal_helper.py:16` appends to `ctx.metadata["queued_execution_signals"]` on a ContextVar and returns a *provisional* UUID; the handler then commits its own work (`runner.py:359` commits `FlowHistory`); only at `pipeline.py:171` does the buffer flush, and `signals.py:111+` swallows emit exceptions. **★ A crash in that window leaves the work durable and the record of it gone — silently.** P2 because it loses the **audit/causal record**, not an effect: nothing duplicates or corrupts. **★ It is the mechanism behind "better index, weaker record" — `build_trace_graph` indexes a record that can lose rows, and a missing row reads as "the work never happened" (worse than `AUDIT-CORRELATION-1`'s missing join, which is visible).** **★ Check first whether `memory_capture_engine`'s `get_downstream_effects` read makes a lost event a missed memory capture — that decides observability item vs memory-loop item.** **★★ Cheaper fix than an outbox (DBOS, 2026-08-19): they have NO outbox and NO window, because their event store IS their workflow store — `set_event_from_workflow` writes the step record and the event on ONE connection. `SystemEvent` already shares our Postgres, so write the event on the handler's own connection inside its transaction; reach for a relay only if something must span two stores.** **★ Do NOT "fix" by emitting eagerly** — that trades this for events recorded for work that rolled back.
...
- **MCP-SDK-2X-1** — `mcp 2.0.0` removed the 1.x API `nodus-mcp 0.1.2` is built on. Both install sites are capped `<2` (`pyproject.toml` **and** the separate CI step — a cap must be repeated in both). **Not a test bug — do not skip the live test to go green.** **★ SECOND INSTANCE 2026-08-17, reversed: `nodus-mcp` now BLOCKS a nodus upgrade.** It requires `nodus-lang<5.0.0`, so #468's bump to 5.0.0 fails CI — and **this must NOT be fixed by isolating the MCP tests**, because `pip install "nodus-lang==5.0.0" "nodus-mcp>=0.1.2"` is `ResolutionImpossible`: pinning 5.0.0 makes **`aindy-runtime[mcp]` uninstallable for users**. Green CI would ship a broken extra. **Ordering (a sequence, not a deadlock): nodus X publishes → nodus-mcp releases accepting `>=X` → runtime bumps BOTH packages in ONE PR across ALL THREE sites (pyproject, requirements.txt, the CI MCP step).** ★ Upstream fix worth making since both are first-party: a prophylactic `<5.0.0` cap on a fast-moving first-party dep turns every nodus major into a two-repo release train. **★ The second instance is RESOLVED as of 2026-08-19 (`NODUS-UPGRADE-2`): `nodus-mcp 0.1.3` floated the cap to `nodus-lang>=4.0.0` unbounded, so `pip install "nodus-lang==5.0.4" "nodus-mcp>=0.1.3" "mcp>=1.0.0,<2"` resolves clean and the CI step no longer needs a `nodus-lang` mention. The FIRST instance (`mcp<2`) is still live — keep both caps.** Consequence: the three-site rule held, but only **two** sites moved for 5.0.4, because the CI step pins `nodus-mcp` and not `nodus-lang`. Keep the rule anyway — checking a site that does not need changing is cheap; the failure it prevents is silent.
...
- **HOOK-PRECEDENCE-1** — *(ADK research)* first-non-`None`-wins hook semantics: **declined.** Our ~40 `register_*` hooks are either **keyed** (one handler per `route_prefix`/`entity_type` — the key is the disambiguator) or **run-all-and-collect** (`_event_handlers`, `_startup_hooks`, `_agent_completion_hooks`, `_capability_definition_providers` — nothing is discarded, so precedence is not a question). **★ The objection is substantive, not stylistic: first-wins makes a handler's effect depend on registration order relative to handlers it cannot see, so one plugin can silently suppress another and nothing records it.** Run-all has the opposite property. **What would change the answer:** a genuine policy-arbitration point where exactly one handler must win and the key cannot express which — none exists today, and if one appears the right shape is an explicit declared arbiter (one registration, Tier 1, like `register_agent_planner_backend`), the same conclusion `DISPATCH-ADMISSION-1` reached. Not to be confused with `CAPABILITY-PROVIDER-TIMEOUT-1`, a defect in a run-all path.
...
- **NODUS-UPGRADE-2** — **CLOSED 2026-08-19.** Pin `nodus-lang` **5.0.1 → 5.0.4**. **★ Filed P3-routine; it was a SECURITY fix.** nodus <=5.0.2 bound `GLOBAL_MEMORY_STORE` at **import**, so every `NodusRuntime` in a process shared one guest memory dict — a guest `.nd` script's `memory_put`/`memory_get` could read another tenant's values. **Reached us via `nodus_worker_pool.py`, whose docstring asserted *"a reused process never leaks state between runs"*** — false, because `run_one` cannot reset a module global inside a dependency; corrected in place. **Latent, not live: warm pool is default-off and never flipped** — so the pool's own safety claim would have been false at the moment its soak began. Reproduced (`5.0.1` → `password123`, `5.0.4` → `nil`), guarded, mutation-tested 2/11. **★ The protocol's gap, now a rule: it says how to bump safely and never says to read the upstream changelog — read the intervening release notes BEFORE assigning severity.** Only **two** of the three sites moved (the CI step pins `nodus-mcp`, which is unbounded). **`MCP-SDK-2X-1`'s second instance is dead** — the `nodus-lang<5.0.0` cap is gone from 0.1.3. Source: `TECH_DEBT.md`.
- **ROUTE-EFFECT-BYPASS-1** — **CLOSED 2026-08-16: A+B (#460), C (#461), D (#462).** Four `memory_router.py` routes reached effects with no dispatcher and no scope check; three now dispatch, and `sys.v1.memory.link` was built for the fourth. **★ It carries `memory.link`, which `memory.write` does NOT grant** — a syscall adding a mediation hop and no authority granularity just relocates the same undifferentiated power behind a longer call path. **★ Trap: `memory.write` REPLACED the caller's `extra` rather than merging, so a naive rewire was silent data loss behind a 201.** **Still bypassing (1, pinned by a test): `POST /nodes/search`** — it uses `find_similar`+`min_similarity`, which `memory.search` neither accepts nor uses, so rewiring would change SEMANTICS under cover of a mediation fix. Source: `TECH_DEBT.md`.
- **CAPABILITY-PROVIDER-TIMEOUT-1** — **FIXED 2026-08-16 (#466)** (filed first as `TEST-ISOLATION-REGISTRY-1`, wrong name, wrong cause, wrong severity). `_load_capability_definition_providers` is reached from `check_tool_capability`, so **every tool capability check spawned a subprocess per provider** — measured 10 lookups = 10 spawns / 56.4s before, 1 / 11.4s after. Under contention it timed out into a `logger.warning` and the capability set came back empty. **★ It fails CLOSED, not open** — established by running it, after the P1 filing assumed the opposite. Fixed by caching per provider, **never caching a failure**, and logging at ERROR. **★ The cache lives ON THE PROVIDER OBJECT, not a module global** — a `_loaded` latch would need adding by hand to two registry-reset dicts, and forgetting either is this bug reintroduced by its own fix. Source: `TECH_DEBT.md`.
- **ISOLATION-DOC-STATUS-1** — **CLOSED 2026-08-16 (#458).** Was trivial. `ISOLATION_MODEL_PLAN.md` says "no implementation has begun" at line 6 and "Scope B1 complete" at line 148. It sits at the repo **root**, outside the `docs/runtime/` frontmatter checks — which is why nothing caught it.
- **MEM-RECALL-N1-1** — **CLOSED 2026-08-16 (#458)**: the four columns are carried on `_node_to_dict` and connectivity is two grouped queries for the whole set (`get_graph_connectivity_scores`). ★ `weight` is newly on memory dicts; the three counts were already written onto candidates by the loop, now just present consistently. Was: `recall()`'s scoring loop is 3 queries per candidate; the re-fetch exists only to read 4 columns the originating SELECT already read. Performance-only.
- **SYSMAX-5** — **CLOSED 2026-08-16 (#453)**, was latent by construction: ~33 scheduler jobs on a default pool of 10, never sized deliberately. Failure mode is a **maintenance brownout** — recovery jobs stop running exactly when the condition they clean up is happening, and nothing raises. **★ Fixed by isolation, not capacity — raising `default` would have been the WRONG fix**, because 30 DB connections are shared with request handling, so more scheduler threads starve the API instead; a test pins total lane threads to half the DB budget so that trade cannot be made accidentally. Source: `TECH_DEBT.md`.
...
- **NODUS-UPGRADE-1** — pin **`nodus-lang==5.0.1`** + **`nodus-mcp>=0.1.3`** (2026-08-17). **★ Bump BOTH packages across ALL THREE sites** — `pyproject.toml`, `AINDY/requirements.txt`, and the `Install MCP extra` step in `runtime-ci.yml`, which installs directly rather than via the extra and so silently re-resolves a constraint fixed only in the first two. **★ `--no-deps` in CI means pyproject's pins are NEVER applied there**; the effective env is `requirements.txt`, which is how CI tested nodus 4.1.0 for four months while the wheel required 4.2.0. Guarded by `tests/unit/test_dependency_pin_agreement.py` (the two sources must agree, and a pin that no installed package permits fails naming the capper) and `tests/unit/test_nodus_upgrade_contract.py` (the probe checklist, executable against the installed package). **★ Confinement depends on someone else's signature: `NodusRuntime.__init__` has NO `**kwargs`, so a renamed deny-flag raises rather than silently unconfining the guest — that absence is now a test.** 5.0.1 exposes `GATED_BUILTINS`, so the confinement sweep no longer scrapes registry source. **Distinguish cosmetic from real before touching the sandbox** — 4 confinement tests went red on 5.0.0 and none was a regression. Source: `TECH_DEBT.md` NODUS-UPGRADE-1.
- **GUEST-CONFINE-1** — **CLOSED 2026-08-15.** The guest VM ran unconfined: `nodus_worker.py` passed none of the confinement args, so a guest script reached subprocess/network/host env without touching the dispatcher, token, ledger, egress guard or tool registry. **Demonstrated, not inferred** — a guest script created a file on the host, read the real PATH, and did real DNS. Fixed with the 3 kwargs (`allow_subprocess/network/env=False`); measured first that no first-party `.nd` script in either repo uses them, so deny-by-default broke nothing. **Deliberately not env-configurable — a global flag re-opens the hole for every run at once; per-execution variation is EXEC-ENV-BIND-1.** Guard: `tests/unit/test_guest_confinement.py`, mutation-tested 4/5 with a liveness control. **★ RESIDUAL OPEN 2026-08-17 — the fix's own step 1 said "(and an explicit `allowed_paths`)" and only 3 of 4 landed. This line used to read "`allowed_paths` already defaulted to cwd, so the VM's filesystem guard was fine" — that is WRONG, because nothing sets the cwd: neither `nodus_worker_pool.py:108` nor `nodus_runtime_adapter.py:278` passes `cwd=`, so the guest inherits the server's working directory — `/home/aindy` in Docker (holding `alembic/`, so a guest can rewrite migrations that run on next boot) and the repo root in dev (including `AINDY/.env`).** The escape is closed; the *bound* is an undeclared inherited default. One kwarg + a test, and it is the smallest true instance of `FS-SCOPE-1` — but do not close `FS-SCOPE-1` with it.
...
- **NODUS-WARMPOOL-1** — CLOSED 2026-07-19. Warm worker pool (`AINDY_NODUS_WARM_POOL`, default off) + boot-allowance clock split, so plugin cold-start is not billed to the script budget. Falls back to a fresh subprocess on any fault. **★ Its soak has a precondition, found 2026-08-19: on `nodus-lang <=5.0.2` a reused worker leaked guest memory between tenants (`NODUS-UPGRADE-2`), so the pool's own docstring claim that *"a reused process never leaks state between runs"* was false. Fixed at `5.0.4`. Before enabling the pool after ANY dependency bump, re-run `test_two_runtimes_in_one_process_do_not_share_guest_memory` — a reused process is only as isolated as its most import-bound dependency.**
