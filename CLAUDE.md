...
| Latest app-team handoff | `docs/upgrades/APP_HANDOFF_v2.17.0.md` — pin bump + rebuild, no schema step, no required code change. Six items #677–#682: **removes** `ExecutionWaitSignal` (the app never imported it — checked); three unflagged behaviour changes on paths the app uses (resume no longer self-vetoes on a payload `correlation_id`; resume can 422 for a typed wait, none declared yet; cancelled runs stop sooner); three opt-ins. **★ Two asks of the app: migrate its 7 conditional-edge lambdas to `when` behind a drain (§3.1, file:line list) and declare `on_denial="wait"`/a variant on ONE real tool so `AUTHORITY-NEGOTIATION-1` ph3 gets evidence (§3.3).** Not yet verified by the app. **★ Verify a version by PRINTING `AINDY.__path__` beside the number, IN THE CONTAINER** |
