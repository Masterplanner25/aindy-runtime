...
Entries are numbered sequentially. Do not reuse a number. IDEM-1 through IDEM-10 are recorded in TECH_DEBT.md. **IDEM-10 — CLOSED at the mechanism level 2026-07-11** via the MEB program (MEB-0 tool path + MEB-1b syscall path). *Corrected 2026-08-14: this said "open", contradicting `TECH_DEBT.md`, which states plainly "IDEM-10 is CLOSED at the mechanism level".* The original finding stands as the diagnosis — the gate was dead in prod (never persisted to an EU, EU-PK lookup can't match) and agent tool calls bypassed the dispatcher entirely. Both paths now have a working boundary, **opt-in and default-off**; the remainder is soak then flip, not build. Next available: **IDEM-12**.
...
- **IDEM-\*** — idempotency audit findings. **IDEM-10 CLOSED at the mechanism level 2026-07-11** (was: "open" — corrected 2026-08-14 against `TECH_DEBT.md`). Diagnosis stands: the EXACTLY_ONCE gate never fired in prod and agent tool calls bypassed it — the real ECOGAP-1 Phase 3a. Shipped opt-in/default-off; remainder is soak then flip. Now consolidated into the **Mediated Effect Boundary program** (`docs/runtime/MEDIATED_EFFECT_BOUNDARY_PROGRAM.md`): IDEM-10 = MEB-0 (tool-path effect boundary, keystone) + MEB-1 (dispatcher gate repair); G4a = MEB-2; multi-tenant MCP = MEB-3. Start at MEB-0 (self-contained, no schema change). Next available: **IDEM-12**.
- **CLI-1** — lazy settings getter / module-level import hazard. Open, deferred post-1.0.
- **★ Module-import-time env reads are invisible to behavioural tests.** Three separate bugs now
  share this shape: FR-10 (`settings = Settings()` at import → an empty Compose var crash-looped
  the container before serving), `ResourceManager._get_backend()` (caches the Redis-vs-in-process
  choice on first call, so setting `REDIS_URL` after boot does nothing), and the
  `AINDY_REDIS_URL` alias in `rate_limiter.py` (removed 2026-08-14) — which survived a cleanup
  that believed it had removed the alias everywhere, because **nothing about the running limiter
  differs when the alias is honoured**. Only reading the source, or a reload-based test
  (`tests/unit/test_rate_limiter_redis_url.py`), can see it. When auditing env-var handling,
  grep the source; do not trust a passing suite.
- **CLI-SANDBOX-FORMAT-1** — `sandbox` raw JSON output wall. Open, deferred to 1.0.1.
- **C2, C3** — cross-platform sandbox tiers. **C2 CLOSED 2026-06-06** (container-grade, escape-tested). **C3 is OPEN**, not closed — *corrected 2026-08-06; this line previously read "All phases closed", contradicting the ECOGAP-2 entry below and `TECH_DEBT.md`.* The remaining gap is deliberate and large: `STRONG_SANDBOX_SUPPORTED_HOST_PLATFORMS` and `HOSTILE_THIRD_PARTY_SUPPORTED_HOST_PLATFORMS` are both `(PLATFORM_LINUX,)`, so non-Linux hosts reach `container-sandbox-certified` but **not** `strong-sandbox-certified`. Closing it needs a platform-native strong-VM runner; the Windows-native and macOS tracks are pre-scoped in `docs/runtime/C3_NON_LINUX_STRONG_SANDBOX_PLAN.md` so either can start when a trigger lands.
- **DOCS-\*** — docset findings. **DOCS-BUCKET-A-1 CLOSED.** **DOCS-STALE-1 CLOSED 2026-08-13** (#395–#399 + boundary closure): seven `docs/runtime/` files declared a `last_verified` *earlier than this repo's first commit* (2026-05-17); all seven read against source. **`Runtime Docs Validation` now asserts `last_verified` is a real `YYYY-MM-DD` and `>= 2026-05-17`** — it only checked key *presence* before, which is why the seven were invisible. **New frontmatter value `status: complete`** (a finished one-time plan; rules may still hold, task lists are history) — vocabulary now defined in `RUNTIME_DOCSET_GOVERNANCE.md` alongside `current`/`outdated`; CI does **not** validate the value, so a typo is silent. **★ The pattern the audit surfaced: four separate docs mis-stated plugin-layer routes as runtime-owned** (`EXECUTION_CONTRACT` `/apps/agent/*`, `MEMORY_BRIDGE` `/apps/memory/metrics*`, `RUNTIME_DOCSET_BOUNDARY` agent+watcher, `PUBLIC_RUNTIME_SURFACES` earlier). One mechanism: `agent_router.py`, `memory_metrics_router.py`, `memory_trace_router.py` are still in `AINDY/routes/` but absent from `APP_ROUTERS`, so they read as runtime-owned until you boot with no plugins. Check `APP_ROUTERS` + `ROUTE_OWNERSHIP_INVENTORY.md`, never file presence. **DOCS-COVERAGE-CLAIM-1 (2026-08-13): docs corrected, coverage gap open** — 6 docs cited 8 test files that have *never existed in either repo*, several with counts ("61 tests (Groups A–K)"); `NATIVE_MEMORY_BRIDGE.md` did it under the heading *"Focused tests that exist in this repository"*, and nothing under `tests/` touches `memory_bridge_rs` at all — so the native scorer has no behavioural coverage in either language (agrees with NATIVE-CI-1). MAS, OS isolation, event bus, native scorer each lack a suite; overlaps ECOGAP-6, don't double-track. **★ COVERAGE HALF CLOSED 2026-08-14 — all four areas now have suites, at the exact paths the docs had cited**: `test_memory_address_space.py` (84), `test_memory_native_scorer.py` (75), `test_os_layer.py` (46), `test_event_bus.py` (44) = **249 tests**, all marked `runtime_only` (without which CI collects none — see CI-MARKER-1). **★ And made to actually run — the fourth variant of the same trap:** 33 native tests (incl. the parity assertions pinning NATIVE-PARITY-1) need the compiled extension, and a skip reads as green; `Native Crate Build (Rust)` builds the crate but never imports it, so they skipped in CI. **`Runtime Contracts` now builds the crate itself and renames the artifact** (`libmemory_bridge_rs.so` → `memory_bridge_rs.so` — cargo's `lib`-prefixed cdylib is not importable, the Linux twin of the `.dll`/`.pyd` mismatch) **and sets `AINDY_REQUIRE_NATIVE_BRIDGE=1`, under which a missing extension is a FAILURE not a skip** (plus a companion test that calls into it, so importable-but-broken is caught too). Verified all three states: unset→skip, set+present→pass, set+removed→2 failures. **Writing them found five real defects the gap was hiding** — `NATIVE-PARITY-1` (**CLOSED 2026-08-15**), `NATIVE-DISCOVERY-1`, `MAS-FLATTEN-1`, `EVENTBUS-PUBLISH-LATCH-1` (**CLOSED 2026-08-15**), `TENANT-FROZEN-SHALLOW-1` (below). Scope note: `test_os_layer.py` covers TenantContext + ResourceManager; the SchedulerEngine WAIT/RESUME half rides on `test_event_bus.py` + ECOGAP-6, not re-tested. **★ Gotcha: `ResourceManager.can_execute` returns `(True, None)` unconditionally under `settings.is_testing`, so quota enforcement is vacuous in tests** — a naive quota test passes without touching a counter; patch it off, and note it's a pydantic *property* so it must be patched on the class (`patch.object(settings, "is_testing", False)` raises `AttributeError`).
- **NATIVE-PARITY-1 — CLOSED 2026-08-15.** The native and Python scorers produced different scores for a negative `impact_score`: Rust clamps `(impact/5.0).clamp(0.0, 1.0)` (`lib.rs:181`), Python used `min(1.0, impact/5.0)` (`scorer.py:120`) — top bound only. Measured +0.300 on a 0.420 score (71%) at `impact=-10`; **now 0.0 across `-1e6 … 25.0` against the real extension.** Fixed by clamping Python (the term is a *bonus*, so flooring is the coherent reading). **★ Severity correction: the original entry said "reachable whenever a stored `impact_score` is negative" — it isn't.** `MemoryNodeDAO.save()` writes `max(0.0, ...)` (`memory_node_dao.py:209`, `:725`) and is the universal write chokepoint, and both producers are non-negative by construction — so it was **latent, not live**. Still worth fixing (no `CheckConstraint` on the column; `_prepare_node` scores any dict handed to it), but the honest severity was defense-in-depth. **Same unclamped shape also fixed at `memory_scoring_service.py:31`** (no native counterpart, so no parity to break). **★ The regression guard is native-independent** — the parity tests skip without a built crate, so the clamp is additionally pinned in `TestPythonFormula`; guarding it only where the crate happens to be built would be the DOCS-COVERAGE-CLAIM-1 mistake in miniature. No schema/migration impact.
- **NATIVE-DISCOVERY-1 — CLOSED 2026-08-15.** The two crate consumers searched different directories: `native_scorer._load_bridge` looked in `target/release`+`target/debug`, `embedding_service.cosine_similarity` in **`target/debug` only** — so on any `--release` build (CI, and any deployment) the C++ cosine kernel was unreachable from the recall fallback while the scorer, *in the same process*, used it. Both now delegate to **`AINDY/memory/native_bridge.py`** (owns search paths, profile order, once-per-process cache; lives under `memory/` because the crate does and `runtime/→memory/` is the established direction). **★ A second defect found by reproducing it:** the plan said native_scorer "already has the right shape", so the new loader copied it — and resolved from `debug` **with a release build present**. `sys.path.insert(0, path)` in *priority* order puts the **lowest**-priority path first, so the documented "release then debug" was inverted and a stale debug build silently shadowed a fresh release one. Fixed with `reversed(search_paths())`, pinned by a test. **Also fixed:** `except (ImportError, AttributeError, Exception)` (a plain `except Exception` in a costume) → explicit `except ValueError` for ragged input (0.0 is *correct* — the sole caller is the recall fallback, where a differently-dimensioned embedding is incomparable, not a bug) plus a **logged** catch-all; and `sys.path.insert` no longer runs on every call. **★ Trip hazard documented in the loader:** `cargo build` emits `libmemory_bridge_rs.so`/`memory_bridge_rs.dll` — Python imports neither; CI renames, a local build needs it by hand.
- **EVENTBUS-PUBLISH-LATCH-1 — CLOSED 2026-08-15.** Three consecutive failed publishes set `self._enabled = False` and **nothing ever set it back** — when Redis recovered, `publish` returned `False` from the top guard without contacting it, so a *transient* blip ended cross-instance WAIT/RESUME for the life of the process (only a restart cured it). **★ Root cause was one field meaning two things:** `_enabled` was both the operator kill switch (`AINDY_EVENT_BUS_ENABLED`) *and* the runtime give-up latch — writing transient health into a config flag is what made it permanent (nothing owns un-setting an operator's choice) **and** invisible (`get_status()` reported a deliberately-enabled bus as `enabled: false`). Fixed by separating them: `_enabled` = config, never mutated at runtime; new `_publish_breaker` = health, using the kernel's existing `CircuitBreaker` (threshold 3, `AINDY_EVENT_BUS_PUBLISH_RECOVERY_SECS` default 60, half-open single probe). **Suspension was kept on purpose** — dropping it would make every `notify_event()` pay a socket connect timeout against a dead Redis; the requirement was *suspend then recover*. Breaker reads `kernel.clock.utcnow` (REPLAY-1) so recovery is tested with `frozen_at`, not `sleep`. **Now visible:** `get_status()` gained `publish_suspended` / `publish_circuit_state` / `publish_retry_after_secs`, and `_get_propagation_mode()` returns `local-only` while open **even if Redis pings OK** (events genuinely aren't propagating). **Behaviour change:** `/health/deep` reports the bus degraded during suspension rather than `ok`. Docstring corrected too — its "reconnects with back-off" line describes the *subscriber*, which was part of the trap.
- **EVENTBUS-COVERAGE-1 — CLOSED 2026-08-15.** The pub/sub wire had never been exercised end to end: 0 integration tests called `EventBus.publish()`, all 4 `notify_event(...)` calls in `tests/integration/` passed `broadcast=False`, and **0** tests anywhere called `start_subscriber()` — so `Integration Tests` was never evidence for an event-bus change (`test_multi_instance_resume.py` covers real cross-instance resume via **`RedisWaitRegistry`**, shared Redis *state*, not the bus). Closed by `tests/integration/test_event_bus_wire.py` (7 tests): `publish()` → real Redis → a live `_subscriber_loop` thread → `_handle_message` → `notify_event`. **★ Mutation-tested — a passing test proves nothing alone:** with `_publish_payload` dropping every message, **5 of 7 fail**. The first draft scored 4; the own-instance-filter test asserts an *absence*, so a broken wire satisfied it trivially — it now runs a **liveness control** first. **Placement:** `tests/integration/`, marked `redis` **not** `integration` (needs Redis, not PG — that marker trips the conftest skip guard); skips only when `REDIS_URL` is *unset*, so a configured-but-unreachable Redis **fails** rather than silently dropping coverage. Verified 7 collected by the integration config, **0** by `-m runtime_only`. **★ Both documented race pitfalls were real:** buses in one process share a hostname-derived `_instance_id` (set distinct ids or the own-instance filter eats everything), and Redis pub/sub has no persistence or readiness signal — so the helper **republishes inside the polling loop** until the effect is observed, never a fixed sleep (the FLAKY-1 pattern).
- **TENANT-FROZEN-SHALLOW-1 — CLOSED 2026-08-15.** `TenantContext` is `@dataclass(frozen=True)` and documented "Immutable", but `frozen` doesn't deep-freeze: while `capability_scope` was a `list`, `ctx.capability_scope.append("admin.everything")` succeeded and `assert_capability` then passed. Fixed by making the field `tuple[str, ...]` (builders normalise any iterable; `in`/`len`/iteration/`has_capability` unchanged — only mutation now raises `AttributeError`). No known exploit path existed; it was a weak invariant, not a live breach. **Still open, adjacent:** `TenantContext.validate_memory_path` *rejects* the exact tenant root `/memory/t1` (requires the trailing slash) while `memory_address_space.validate_tenant_path` *accepts* it — two tenant guards, two answers for one string.
- **★ KERNEL-INIT-DUPLICATE-1 — CLOSED 2026-08-15. `AINDY/kernel/__init__.py` was a byte-identical 171-line copy of `tenant_context.py`**, present since `0d5d382 Initial runtime repo extraction` and never touched. So `from AINDY.kernel import TenantContext` and `from AINDY.kernel.tenant_context import TenantContext` returned **two different class objects** — `isinstance` silently `False` across them, for the class that *is* the tenant isolation boundary — and a fix to one wouldn't reach the other (the TENANT-FROZEN-SHALLOW-1 change would have left a second still-mutable `capability_scope` behind). **Nothing had broken because nothing imported it:** every `from AINDY.kernel import X` here *and* in `aindy-apps-monolith` imports a **submodule**, which resolves identically either way. Fixed by making `__init__.py` a real package init re-exporting the five public names from the single definition (chosen over emptying it, so an unknown external caller can't break). **Safe from the `AINDY.routes` shadowing hazard** — that bites when an exported *name* collides with a *submodule* name; none of the five do, and a test asserts `from AINDY.kernel import tenant_context` still yields the module. **Swept: all 337 `.py` files under `AINDY/` hashed — no byte-identical non-empty duplicates remain**, so this was the only extraction artefact of its kind.
- **MAS-FLATTEN-1 — CLOSED 2026-08-15.** `flatten_tree` dropped any node that was the parent of another: the root set was "every path minus every path that is some node's parent", which removes the *parents*, so an intermediate node was never walked and silently vanished. A root is a node whose **parent is not itself a node** — the inverse. Now also guarantees **every node appears exactly once** (visited-guarded walk + a totality guard appending anything unreachable from a root, since `build_tree` only links `children` when the parent path is itself a node). **Zero callers** under `AINDY/` or the app repo — which is why it was recorded not urgent — but `MEMORY_ADDRESS_SPACE.md` §7 documents it as usable, so it was **fixed rather than deleted**. The `xfail(strict=True)` flipped to `XPASS(strict)` and failed the build until converted, which is what strict xfail is for. **★ The invariant that would have caught it in one line: `len(flatten_tree(tree)) == len(tree)`** — now a test. **Related and still true:** `build_tree` never nests for canonical MAS data (nodes are always 5-segment leaves), so `sys.v1.memory.tree` returns a flat map by construction.
- **★ ROUTE-GUARD-1 — CLOSED 2026-08-15. Every `raise HTTPException` in `admin_router` / `agents_router` / `automation_router` was returning 500.** `enforce_registered_route_execution` wraps *every* route; its **success** path (`_assert_execution_context_entered`) always asked two questions — did this enter the pipeline, and **was it required to** — while its **failure** path asked only the first. So an exception from a router registered deliberately *outside* the contract (all three are plain DB-query handlers, and `routing.py` says so at each call site) became a `RouteExecutionViolation` → `{"error":"internal_error"}`. **★ Measured: `POST /platform/admin/agents/register` with `memory_namespace:"runtime"` returned 500, not 409 — FR-12's reserved-namespace guard, shipped the day before.** It blocked the write correctly and then reported it as a server fault; a caller can't tell "rejected" from "broke" by a 500. `DELETE …/agents/{missing}` was 500 not 404 the same way. **Why nothing caught it: the FR-12 tests assert on the route's *source text*, never calling it** — source assertions prove code was written, not what it returns (same family as `DOCS-COVERAGE-CLAIM-1`). Fixed by giving the failure path the same two-part question (`_is_pipeline_bypass_on_error`). Regression suite calls the real routes over HTTP and carries a **liveness control in the opposite direction** (`TestManagedRoutesStillViolate`) — a "fix" that just stopped raising would satisfy every other assertion. **Flagged, not fixed — `ADMIN-PROMOTE-UUID-1`:** `POST /platform/admin/users/{id}/promote` also 500s on a missing user, but for an unrelated reason (raw string into `User.id ==`; SQLite UUID binding raises `'str' has no attribute 'hex'` before the 404 branch) — **SQLite-harness-only**, psycopg2 casts it on PG.
- **★ IDEM-11 — OPEN, P0. At-most-once is built, tested, and shipped DISABLED.** Three conditions must all hold and the first two default off: `_syscall_idempotency_enabled()` (`syscall_dispatcher.py:163`) is False unless `AINDY_SYSCALL_IDEMPOTENCY` is explicitly set — its own docstring says *"When off (default), the syscall idempotency gate never fires"* — and `SyscallEntry.__init__` defaults `execution_guarantee="AT_LEAST_ONCE"` (`:193`); third is `_gate_scope_engaged`. **★ Measured, sharper than the audit: of 27 registry entries exactly ONE declares `EXACTLY_ONCE` — `sys.v1.memory.delete`.** So flipping the env flag alone would dedup a single syscall; the mechanism is not merely default-off, it is almost entirely **undeclared**. This is IDEM-10/MEB's "soak then flip" remainder, plus the part that framing hid. Fix: make `execution_guarantee` **required** in `register_syscall` and default the flag on in production; the per-syscall audit is the real work, not the flag. Source: `TECH_DEBT.md` IDEM-11.
- **EXEC-ENV-BIND-1 — OPEN, P1. An execution unit cannot declare the environment it needs.** `create_sandbox_runner`/`resolve_sandbox_runner_type` appear in exactly two modules — `plugin_host.py` (Tier-2 extensions) and `deployment_contract.py` (reporting). **The agent tool path, flow path and Nodus path never touch them**; `nodus_runtime_adapter.py:278` calls `subprocess.run` directly. `ExecutionUnit` has 22 columns and **no** field for filesystem/network/CPU/memory/secrets/assurance. So the *provider* half of the contract is built and the *requesting* half does not exist. Proposed: one `ExecutionEnvironmentSpec` on `ExecutionUnit`, resolved at `execution_gate.gate_and_dispatch` (`:294`). **★ This is the one place two independent audits disagree, and evidence settles it:** Hermes rates it P1 ("the paths lacking binding are Tier-1 trusted by contract"); the substrate audit rates the guest case P0. **P0 is right for the guest path — a guest script demonstrably created a host file (GUEST-CONFINE-1) — because guest *script content* is submitted data, not first-party code.** Keep them separate: GUEST-CONFINE-1 = P0, three kwargs; EXEC-ENV-BIND-1 = P1, the primitive. **★ The descriptor's real value is ACCOUNTABILITY, not variation:** today the honest-posture defence (*"we never claimed Tier-1 isolation"*) is **unfalsifiable per execution** — nothing records what confinement a unit required, so *"was this the containment you asked for?"* has no answer for any given run. The descriptor makes the runtime claim only **selection and refusal** (both enforceable in-process, host keeps enforcement) and turns `assurance_ceiling` from a report into a gate — the same claim-vs-evidence split `sandbox_runner_assurance_posture()` already honours on the provider side. **And the posture defence covers deployed code, not submitted content:** a `.nd` script from an authenticated session is data, not something the operator put in a Tier-1 slot. Source: `TECH_DEBT.md` EXEC-ENV-BIND-1.
- **QUEUE-DURABILITY-CLASS-1 — OPEN, P2.** `_fallback_to_memory_backend` (`distributed_queue.py:418`) swaps durable Redis for in-memory; `QueueJobPayload` has no durability field, so the enqueuer's call succeeds identically either way. **★ The audit omits an existing control that lowers severity: `AINDY_REQUIRE_REDIS` makes the fallback raise, and the path already classifies `UNSAFE_DEGRADED` / `production_behavior="startup-fatal"` in distributed mode.** So it is deployment-visible and deployment-preventable; only the *per-job* expression is missing. The audit itself says to fold this into the ownership contract rather than track it separately — agreed. Source: `TECH_DEBT.md` QUEUE-DURABILITY-CLASS-1.
- **ORCHESTRATOR-SPLIT-1 — OPEN, P2.** Durable work state lives in **three** stores with three recovery paths and no shared transaction: `flow_engine/` (`FlowRun` + `FlowHistory`), `distributed_queue` + `worker/` (leases, visibility timeouts, DLQ), and Nodus `task_graph.py` (fsync'd JSON under `.nodus/graphs/`). No single authoritative answer to "what work remains" after a crash. **Do (b) first — publish the ownership contract naming which engine owns which failure domain.** The split may be correct; it is currently undocumented, so it can be neither relied on nor reviewed. Nodus already exposes `runtime.set_effect_store(store)` if (a) is later wanted. Source: `TECH_DEBT.md` ORCHESTRATOR-SPLIT-1.
- **AUDIT-CORRELATION-1 — OPEN, P2.** Three joins the audit trail cannot make: capability→event (the authority that admitted a call is not persisted with it), environment→execution (sandbox attestation not joined to the EU), and `EffectRecord.action_id`→`SystemEvent` (verified: `EffectRecord` has a FK to `execution_units` but **none** to `system_events`, so the join is by `trace_id` convention). (1) and (2) fall out of AUTHORITY-VALUE-1 and EXEC-ENV-BIND-1; only (3) is standalone. Source: `TECH_DEBT.md` AUDIT-CORRELATION-1.
- **★★ ROUTE-AST-UNWIRED-1 — OPEN, P2. The boot-time route AST proof EXISTS AND NEVER RUNS against the application.** `validate_registered_route_execution` has exactly three repo-wide references: its definition and two lines in `tests/unit/test_route_execution_guard.py`. **`routing.py:87` calls `enforce_registered_route_execution` — the request-time *wrapper*, a different function with a near-identical name.** And the validator's only test call site is `test_helper_indirection_route_is_allowed_by_runtime_wrapper_even_if_ast_audit_is_stricter`, on a **synthetic** app, inside `pytest.raises(...)` — it exists to show the AST audit is **stricter** than the wrapper and rejects a legitimate helper-indirection route. **So the defect is the CLAIM, not the absence:** the real guarantee is the request-time wrapper, not a boot-time refusal. **★ Eighth variant of "verification that exists but does not run" — and the first found in a runtime mechanism rather than a test one.** **Do NOT wire it as-is: by its own test it raises on a route that works today.** Fix the claim first; then decide deliberately whether to teach it helper indirection or delete it. Source: `TECH_DEBT.md` ROUTE-AST-UNWIRED-1.
- **★ AUDIT-INVARIANTS-VERIFIED-1 — RECORD. The claimed *guarantees* were swept, not just the gaps.** Held: `waiting`/`resumed` distinction, startup wait-rehydration, `approve_run` CAS, HMAC-SHA256 tokens + rotation window, effect ledger at **both** chokepoints, quota fail-closed-in-prod, 60s/20s lease, ContextVar reset on the failure path. **Two claims did NOT hold:** the boot-time route proof (above), and **"output validation is warn-only" — which is FALSE for `stable` syscalls and is a point where two audits contradict each other.** Source settles it: a stable syscall's output mismatch logs `error`, emits an error event, marks the effect `failed` and **returns an error envelope**; only *experimental* syscalls warn and continue. **★ Method note: verify the guarantees, not just the gaps — both errors were in "already covered" sections, the part of an audit least likely to be re-checked, because a finding invites scrutiny and a reassurance does not.** Source: `TECH_DEBT.md` AUDIT-INVARIANTS-VERIFIED-1.
- **★★ THREE AUDITS CONVERGED ON ONE ROOT — `create_sandbox_runner` is reachable only from `plugin_host.py`.** Claude Code named the **guest VM** (`GUEST-CONFINE-1`, P0, demonstrated), Hermes named the **execution unit** (`EXEC-ENV-BIND-1`, P1), Codex named the **tool seam** (`TOOL-SEAM-ISOLATION-1`, P0). Three reviewers, three starting points, one provider attached to the quietest of the four boundaries it could serve. **The fix is not three fixes — it is one provider re-homed and three call sites taught to ask for it.** Do `GUEST-CONFINE-1` first anyway: it is three kwargs and does not wait on the vocabulary.
- **★ TOOL-SEAM-ISOLATION-1 — OPEN, P0. Every authority check at the tool seam is advisory with respect to the code that runs next.** `execute_tool` evaluates token, granted tools, capabilities, policy, rate limits, egress and secret scope — then calls `entry["fn"](args=args, user_id=user_id, db=db)` **in-process, handing the tool the live DB session** (`tool_registry.py:366`, verified). The tool can `import os`, read the environment, open sockets on a raw thread (which `egress_guard`'s own docstring names as an escape) or issue arbitrary SQL through that session. Proposed: `register_tool(..., isolation="in_process"|"subprocess"|"container"|"strong_vm")`, resolved against the profile's `available_runner_types`, **failing closed** when the host cannot supply it; default stays `in_process` so nothing changes until opted in. **The runtime already concedes the point in config — `AINDY_TRUST_EXTERNAL_PYTHON_EXTENSIONS` is documented trusted-code-only precisely because of this.** Source: `TECH_DEBT.md` TOOL-SEAM-ISOLATION-1.
- **★★ HTTP-SCOPE-GAP-1 — OPEN, P0. The capability model does not reach the runtime's own front door.** **Measured: `Depends(enforce_api_key_scope)` appears on 7 of 147 route decorators** (4 in `flows_router.py`, 3 in `platform_ops_router.py`; the audit's 141 → 147 drift is exactly the 6 agent routes added 2026-08-15). Everything else — including all of `/memory/*` — depends only on `get_current_user`, which is **identity, not authority**. **And the check is conditional: its own docstring says "JWT users carry full trust and are never gated by this check", so a session caller skips it entirely — an interactive JWT session is strictly MORE privileged than any API key.** Worse, the REST path can reach effects without the dispatcher at all: `memory_router.py` has **zero** `SyscallDispatcher`/`dispatch` references and `create_node` calls `MemoryNodeDAO(db).save(...)` directly. Distinct from `AUTHORITY-VALUE-1` (which is the chokepoint checking a caller-supplied value) — this is a large surface **not reaching the chokepoint at all**. **★ DECIDED 2026-08-15 (owner): a JWT must NOT bypass scopes.** ★ **Hard prerequisite, verified: a JWT carries NO scopes today** — `create_access_token` encodes `tv`/`purpose`/`exp` only, so "stop bypassing" implemented literally denies every session request including the SPA's. Decide where JWT authority comes from FIRST: recommend **derive from the user row** (`is_admin` ⇒ full set) — no token change, existing sessions keep working — over adding a `scopes` claim, which invalidates every session (2.0.0 already did that once for `purpose`). Roll out with the derived set matching today's effective privilege, then narrow. Source: `TECH_DEBT.md` HTTP-SCOPE-GAP-1.
- **FLOW-PARALLEL-1 — OPEN, P1.** `resolve_next_node()` returns exactly one successor or `None` (`node_executor.py:49`) and the runner advances one node at a time; plan steps are strictly sequential, so three independent API calls cost the **sum** of their latencies. No fan-out, join or barrier. Applications needing parallelism must route *around* the flow engine, losing `FlowHistory`, retry policy and quota accounting for exactly the workloads that most need them. Proposed: `ParallelGroup([...], join="all"|"any"|"quorum(k)")` with one `FlowHistory` row per branch and **output patches merged in declaration order, not completion order** — determinism is the load-bearing part, not speed, because replayability is what this engine is built around. Source: `TECH_DEBT.md` FLOW-PARALLEL-1.
- **AUTHORITY-NEGOTIATION-1 — OPEN, P1.** A denied capability terminates the step (`CAPABILITY_DENIED` at `nodus_adapter.py:188`); because approval is whole-plan, the only recovery is a human re-approving a **new** run, discarding the durable state the original accumulated. Proposed: one bounded negotiation stage — on denial consult a declared fallback, attempt **at most once**, record both attempts. **★ The runtime has a better fallback than the system that prompted the finding: `sys.v1.agent.simulate` already gives a zero-side-effect rehearsal against virtual tools, which beats retrying with *more* authority.** Keep the shape: bounded, **downgrade-only**, recorded. Source: `TECH_DEBT.md` AUTHORITY-NEGOTIATION-1.
- **EGRESS-INPROC-1 — OPEN, P2, a re-homing not a build.** `egress_guard` wraps `getaddrinfo`/`connect`/`connect_ex` on a contextvar allowlist, is **off by default** (`AINDY_EGRESS_ENFORCEMENT`), and **its own docstring names both bypasses under "Honest limits"** — a raw `threading.Thread` does not inherit the contextvar ("intentionally not done; the sandbox path is the real fix"), and a tool linking its own native resolver evades both hooks. The module already reaches the right conclusion; the finding is that *decision* (runtime) and *enforcement* (host) live in the same place. Fold the egress decision into `TOOL-SEAM-ISOLATION-1`'s provider. Risk: `capability_policy` domain allowlists read as controls in audit output while advisory in the default profile. Source: `TECH_DEBT.md` EGRESS-INPROC-1.
- **★★ GUEST-CONFINE-1 — OPEN, P0. The guest VM runs UNCONFINED, so effects on the primary execution path are not mediated.** `nodus_worker.py:327` builds the VM as `NodusRuntime(project_root=…)` and passes **none** of the confinement args; the VM defaults are `allow_subprocess=True, allow_network=True, allow_env=True`, and `nodus/builtins/registry.py` registers the **real** `std:subprocess` / `std:http` iff the flag is truthy. A guest script therefore reaches subprocess, network and host env **without touching the dispatcher, capability token, effect ledger, egress guard or tool registry**. **★ DEMONSTRATED 2026-08-15, not inferred** (the audit that found it could not attempt this): driving `nodus_worker.run_one()`, `env_get("PATH")` returned the **real host PATH**, `http_get` performed **real DNS**, and `subprocess_shell` returned `exit_code: 0` and **created a file on the host filesystem**. Scope: driven in-process, so this proves the *guest boundary* is open — **not** an unauthenticated remote path (`POST /platform/nodus/run` needs `get_current_user`, 30/min). **The source validator does not help and is worse than reported:** `validate_nodus_source` blocks only Python-isms; `RESTRICTED_OPERATION_SUMMARY` is never read by a check (confirmed); **and `validate_requested_operation_usage()` — the function that *would* gate operations — has ZERO callers outside its own module, with an allowlist covering only memory ops**, so it could never have blocked subprocess/http. **★ Migration risk is far lower than the audit estimated — measured: NO first-party `.nd` script in either repo uses `subprocess_*`/`http_*`/`env_get`.** Deny-by-default breaks nothing today. **Recommended split (deliberately not the audit's ordering): fix now with 3 kwargs (`allow_subprocess=False, allow_network=False, allow_env=False`) — a defect fix, not a new primitive, since the VM already accepts them — then the descriptor (`EXEC-ENV-BIND-1`).** ★ *Correction: an earlier version of this line said "later, when variation earns it". Wrong axis — the descriptor's value is **accountability**, not variation, and accountability holds at n=1. See EXEC-ENV-BIND-1.* Source: `TECH_DEBT.md` GUEST-CONFINE-1.
- **AUTHORITY-VALUE-1 — OPEN, P1. The syscall capability check reads a value the calling frame supplied.** `SyscallContext.capabilities` is a `list[str]` on a caller-constructible dataclass; `if entry.capability not in context.capabilities` (`syscall_dispatcher.py:385`) is faithful but cannot tell a token-derived claim from one written a line earlier. Self-granting paths: `_infer_dispatch_capability` (`:794`, derives `{domain}.read|write` from the syscall *name*), and literals at `entrypoints.py:83` / `nodus_execution_service.py:226`. `child_context()` falls back to `list(parent.capabilities)` — **inherits, never narrows**. `execute_tool` checks capability only `if execution_token is not None`, and `extension_worker.py:344` passes `run_id=None, execution_token=None` — so the extension path has **no per-tool check**, only the coarse `CAP_TOOL_INVOKE`. **Not a vulnerability** (every entry point has its own authorisation) — the point is the chokepoint is not an *independent second* gate. **Also: `if not user_id:` in `execute_intent`/`run_flow`/`run_nodus_script_via_flow` calls a `_*_direct()` variant — absent identity SKIPS the boundary instead of denying, logged at `debug`, which is what makes it quiet.** Source: `TECH_DEBT.md` AUTHORITY-VALUE-1.
- **CANCEL-REACH-1 — OPEN, P1. Cancellation is durable but never reaches an in-flight effect.** `sys.v1.agent.cancel` CASes to `cancelled`; the chain observes it **between segments** only (`nodus_execution_service.py:769`). `SyscallContext` has exactly six fields and carries **no cancellation object**; nothing is threaded to `execute_tool`. Proposed: `ctx.should_stop()` at the two effect chokepoints the effect ledger already brackets. **★ Constraint this repo has already paid for twice and the audit omits: `should_stop()` must NOT do a per-effect DB round-trip on the request-shared session** — that is `RT-MEMTXN-LEAK-1` (pool exhaustion) and `MEM-RECALL-N1-1` (N+1) in the same family. Cache with a short TTL or use a dedicated short-lived session. Source: `TECH_DEBT.md` CANCEL-REACH-1.
- **DISPATCH-ADMISSION-1 — OPEN, P2, deliberately deferred.** No hook/interception exists anywhere in `SyscallDispatcher.dispatch`'s body, and `register_event_handler` returns the handler without its value ever being consulted, so it cannot veto. **Do NOT build a general hook system** — an interception seam runs someone else's code in the kernel process, which the Tiered Isolation Contract reserves for Tier 1; a generic surface quietly widens Tier 1. If ever built: exactly one Tier-1 admission callback, registered like a planner backend. Risk if omitted is low. Source: `TECH_DEBT.md` DISPATCH-ADMISSION-1.
- **ISOLATION-DOC-STATUS-1 — OPEN, trivial.** `ISOLATION_MODEL_PLAN.md` says **"Status: Planning — no implementation has begun"** at line 6 while line 148 of the *same file* says "Scope B1 complete", and the sandbox runners, plugin host, certification and nine-file escape suite are all built and wired. **It lives at the repo ROOT, so it is outside the `docs/runtime/` frontmatter + `last_verified` checks** — which is exactly why nothing caught it. Source: `TECH_DEBT.md` ISOLATION-DOC-STATUS-1.
- **SYSCALL-STABILITY-\*** — **SYSCALL-STABILITY-1 FIXED 2026-08-13.** `SyscallEntry.stable` and `_STABLE_SYSCALLS` measure **different things** and may legitimately differ — the first is *advertised maturity* (→ `GET /platform/syscalls` → the apps API reference), the second is a *rename/removal guard* for SDK-called names. "Experimental shape, won't be renamed" is coherent; `memory.list/tree/trace` are exactly that. **Real bug found: two SDK-dispatched syscalls were outside the guard** — `sys.v1.memory.list` and `sys.v1.execution.get`; a rename would pass CI and break the SDK. (`execution.get` already failed live once — the SDK called it before a handler existed, green in SDK tests **because they mock the dispatcher**; mocked-dispatcher tests can't catch a name break.) Also fixed: `SYSCALL_REFERENCE.md` claimed `stable` for 4 syscalls registered `stable=False`. **Two gotchas worth keeping:** (1) the duplicate-registration guard is on `SyscallRegistry.__setitem__`, **not** `register_syscall` — `SYSCALL_REGISTRY` is a custom mapping, so reading the function body alone wrongly suggests there is no guard (now tested); (2) `stable` defaults to `True`, but `agent.cancel`/`agent.simulate`/`execution.get` are *pinned* `stable is True` by their own unit tests — don't assume an unset flag was accidental. **Open, app-side:** `aindy-apps-monolith`'s `apps/automation/syscalls/syscall_handlers.py` defines `register_all_domain_handlers` **twice**; the dead first copy re-registers `memory.list/tree/trace` under different capabilities and would `ValueError` at plugin load if revived.
...
change-only; `register_user` still has no policy (deliberate, tracked). **Items 2+3 (forgot/reset) OPEN — app team answered 2026-08-01: option (a), the runtime sends the mail.** Sub-questions settled and agreed: stateless signed token carrying `user_id`+`token_version` (single-use falls out by construction — the reset bumps the version so a consumed token stops verifying; no table, no migration), TTL 30–60min, `/forgot` always 200 (enumeration oracle), rate limit 3/min per IP **and** per email. **Remaining blocker is structural, not a preference:** `register_connector` is a hook for *apps*, and the runtime ships **no** `email` connector (verified — none registered anywhere under `AINDY/`), so "the runtime sends it" currently has nothing to send with. Pick one: (1) runtime ships its own SMTP sender routed through `authorized_external_call`; (2) dispatch an app-registered connector with `/forgot` 503 when absent — **inverts the split** and breaks "runtime boots clean without plugins"; (3) hybrid. **Recommend 1 or 3** — option 2 satisfies the letter of (a) while reintroducing the dependency (a) was chosen to avoid. **Sub-item CLOSED 2026-08-01:** `MIN_PASSWORD_LENGTH` now enforced on `register_user` too (400 under 8). The "zero migration cost" argument was app-specific and the objection was raised; owner overruled deliberately — a floor deferred indefinitely is not a floor. Unflagged and not configurable by design. Does NOT invalidate stored passwords or touch login; only new short-password registrations 400. Length check precedes the duplicate-email lookup, so short+taken returns 400 not 409. **Still open (adjacent): `/auth/register` returns 409 on duplicate email — an enumeration oracle, same class as the `/forgot` 200 rule.** Next available: **FR-15**. **★ FR-14 FILED 2026-08-15 by the app team, verified: `bootstrap-schema` — the README's own "blessed deploy primitive" — refuses additive drift, so a bare entrypoint invocation CRASH-LOOPS on any release adding a runtime column (they hit this on 2.1.0 via FR-13's columns). ★ Their write-up says `serve` self-migrates where the CLI does not — VERIFIED FALSE: `_enforce_schema_guard` gates on `AINDY_SCHEMA_RECONCILE` (default false) and raises identically, so removing the CLI call MOVES the failure to `serve`. One behaviour, two gates, two opt-ins, neither default-on, README recommends the bare form. Second time an "additive, nothing to prepare" claim was true of DATA and false of DEPLOYMENT — FR-8 was the first.** **FR-11/12/13 FILED 2026-08-06 (verified). ★ FR-11 SHIPPED 2026-08-15:** `AINDY_RUNTIME_CALLBACK_TIMEOUT_SECS` (default **30s**, was a hardcoded 10s that no call site could override and no env key exposed), **resolved at call time** so it needs no restart and stays visible to behavioural tests (the import-time env-read hazard). Sized on measurement, not taste: **~3.85s median** cold start on the *lightest* profile = only ~2.6× headroom at 10s, while the sibling nodus subprocess budgets **15s for boot alone** on top of a 30s script clock. Bad/non-positive values warn and fall back; the timeout message now names the budget and the env key. Expected to mitigate FLAKY-1 (not claimed to close it). **★ FR-13 SHIPPED 2026-08-15:** `agents` gains a JSONB **`metadata`** column + `updated_at` (Alembic `0015`, schema `2026-08-15`, head constant → `0015`). **Gotcha: the ORM attribute is `Agent.agent_metadata`, the COLUMN is `metadata`** — `metadata` is reserved on a declarative class (`Base.metadata`); both halves are pinned by tests. **Purely additive, no backfill — deliberately unlike FR-8:** both columns nullable, so `NULL` already means "nothing recorded" and every pre-existing row is correct as-is; no `UPDATE`, and a test asserts no `reconcile_backfill` marker was added so that reasoning survives an edit. Verified against a throwaway real PG on five properties (blank-DB skip, additive upgrade preserving rows, idempotent re-run, JSONB actually queryable via `metadata->>'workspace'`, idempotent downgrade). **★ FR-12 SHIPPED 2026-08-15:** `registry.register_agent(...)` — the *identity* hook (the 8 existing `register_agent_*` hooks all register **behaviour**). Declarative: records a spec, touches no DB (plugin load precedes any session); `startup._apply_registered_agents()` upserts by `memory_namespace`, updating an existing row so an app can change its name/metadata between boots. **★ The filed premise was WRONG and the entry says so:** `POST /platform/admin/agents/register` already existed (mounted, runtime-owned — checked against `APP_ROUTERS`, not file location). The real gaps were narrower: no *hook*; **no path ever wrote `owner_user_id`** (which is why `count(owner_user_id)=0`); unscoped reads; and **system namespaces unreserved — so that route's idempotent-*update* branch silently rewrote the platform's own Runtime/Memory rows, with no repair on boot since `_bootstrap_system_agents` only INSERTs when absent.** Both hook and route now reject the 7 reserved namespaces from one shared `SYSTEM_AGENTS` set. **★ Shipping bug caught by its own test:** `INPROC_CAP_REGISTER_AGENT` was declared but missing from `_ALL_INPROC_EXTENSION_CAPABILITIES`, and the guard raises `PermissionError` for anything outside the allowed set — so the hook would have been **denied in the very plugin-load context it exists for**. **★ FR-12b SHIPPED 2026-08-15 — both deferred halves, plus two defects they exposed.** (1) **User-owned agents** at `/platform/agents` (list/create/patch/deactivate/restore). *The "app-layer policy" deferral did not survive contact*: ownership, per-owner name scoping and owner-scoped reads are properties of the **table**, so every app would have rebuilt all three against a schema fighting them. **The namespace is DERIVED (`u:<user_id>:<slug>`), not accepted** — a user-chosen namespace must 409 on a row the caller can't see, i.e. the `/auth/register` enumeration oracle again; deriving makes collision *impossible* not *detected*. `agent_type` forced to `custom` (`agent_capability_mappings` is keyed by it); create deliberately **not** idempotent (that branch is what rewrote platform rows); foreign agent is **404 not 403**; a principal with no user is 400, never `owner_user_id=NULL` (which would silently create a *shared* agent). (2) **`agents.name` is unique per owner** — Alembic `0016`, schema `2026-08-15.1`, head → `0016`. **★ A plain `UNIQUE (owner_user_id, name)` is NOT equivalent: SQL treats NULLs as distinct, so every shared row escapes it and two "Runtime" rows are accepted** — hence two *partial* indexes. Verified on real PG, 9 properties. (3) **Repair path**: `_bootstrap_system_agents` said "idempotent upsert" and was insert-only; boot now repairs *identity* and logs it, but **does NOT re-enable a deactivated system agent** (that silently undoes an operator action) — it warns and names `POST /platform/admin/agents/{namespace}/restore`, which needs no restart. **★ Policy DECIDED 2026-08-15 (owner): an admin MAY deactivate a system agent** — supported operator action, so **no reserved-namespace guard on `DELETE`** (the guard on `register` is not precedent: it stops an *idempotent-update* branch rewriting platform rows, a different thing from an explicit reversible action). **This makes the boot behaviour more clearly right:** not re-enabling was the conservative choice while the policy was open; now that deactivation is sanctioned, silently reversing it on restart would be actively wrong. `DELETE` returns a `warning` naming the restore endpoint. **Two defects found building it:** `memory_agents_list_node` listed **every** active agent to **every** caller (harmless only while all rows were un-owned — now owner-scoped); and `SYSTEM_AGENTS` vs `startup._SYSTEM_AGENTS` were two lists for one roster (now one `SYSTEM_AGENT_SPECS`). **★ Patching gotcha, the INVERSE of the scheduler-job rule:** `startup.py` binds `SessionLocal` at **module level**, so patch `AINDY.startup.SessionLocal` — patching `AINDY.db.database.SessionLocal` silently no-ops and the seed logs `no such table: agents` as a *non-fatal* warning. **FR-11** `invoke_runtime_callback` has a **hardcoded 10s** budget (`runtime_callback_host.py:43`) that neither `registry.py` call site overrides and no env key exposes; invoked from scheduled jobs so it repeats, and hardest when the host is slowest. App team filed it as hardening, NOT a defect — cold-start only, 0 recurrences warm. **Their stated mechanism is wrong and the record says so:** `bootstrap_register` fires only for `runtime_agent_defaults` (`registry.py:410`), not a 16-app bootstrap; the real cost is a fresh subprocess `import_module` pulling an app's transitive graph. **FR-12** there is **no `register_agent`** — 8 `register_agent_*` hooks exist and every one registers *behaviour*, not identity; the roster is a hardcoded 7-spec `_SYSTEM_AGENTS` list at `startup.py:937`. Live data confirms the sharp half: 7 rows, all `agent_type='system'`, **`count(owner_user_id)=0`** — the per-user half of the schema has never been exercised. **FR-13** `agents` has exactly 8 columns and **no JSONB / no `updated_at`**; wants `metadata` so identity (role) can outlive the vendor client. **Build cost:** touches `AINDY/db/models/` → schema-contract protocol + Alembic revision + `RUNTIME_ALEMBIC_HEAD_REVISION` bump; additive/nullable so no `reconcile_backfill` needed (unlike FR-8). **FR-8/9/10 confirmed CLOSED app-side on 2.0.1.** **~~Their FR-7 status is STALE~~ — WITHDRAWN 2026-08-15, this note was itself the stale thing.** Checked against `aindy-apps-monolith/docs/handoffs/RUNTIME_FEATURE_REQUESTS.md`: it has carried **`FR-7 — ✅ CLOSED in 2.0.0`** since 2026-08-06. The note was repeated into `APP_HANDOFF_v2.1.0.md` without checking a repo that is on disk — **do not re-add it.** All four defects did ship in 2.0.0 (`_policy_base_significance`, `normalize_for_dedup`, `_forced_capture_suppressed`, `blend_impact_with_significance`); the error was asserting the app team hadn't noticed. **FR-5 (NEW 2026-07-18,
verified real):** native `run_nodus_workflow` `.nd` couldn't reach app callables via either VM surface. **(a) SHIPPED**: `run_nodus_workflow` gains `capability_token`+`run_id` params threaded to flow state as `execution_token`/`agent_run_id` (the agent path's proven keys) → `call_tool` seam; token binds run_id+user_id (both required). **(b) SHIPPED 2026-07-18 — corrected diagnosis**: handoff said "make `dispatch_syscall` resolve app-registered syscalls"; verify-first found apps DO register into `SYSCALL_REGISTRY` (kernel `register_syscall` at `syscall_registry.py:1813`, with real caps) which `dispatch` consults — the real bug was a **subprocess plugin-load gap** (sibling to PLANNER-SUBPROC-1): the worker's `sys()` seam had no plugin-load entry point (unlike `call_tool`→`execute_tool`→`_ensure_tools_loaded`), so `sys()`-only workflows dispatched against an unpopulated registry → "Unknown syscall". Fix: `nodus_worker.dispatch_worker_syscall` runs `_ensure_tools_loaded()` before dispatch (lazy/idempotent); capability enforcement unchanged (app syscall keeps its cap, granted via `_infer_dispatch_capability`). Contract `NODUS_WORKFLOW_CONTRACT.md` §8.1. **FR-8/9/10 (NEW 2026-08-03, all SHIPPED 2026-08-05, all verified-real) — the 2.0.0 upgrade trio, and they gate a 2.0.1.** Found by the app team upgrading a live deployment to the release we'd just cut. **Common thread: each is invisible from a source checkout AND from CI** — a wheel has no `alembic/` tree, Compose writes `""` where a shell writes nothing, and an app-registered connector type is a shape our tests never see. **FR-10 (#360):** `"${VAR:-}"` renders as `""` → pydantic bool_parsing → and since `settings = Settings()` runs at **module import**, the container restart-loops before serving (27 restarts). Fix: `env_ignore_empty=True`. 28 typed bools were exposed. **FR-8 (#361):** Alembic `0014` grandfathers pre-existing users to verified, but `alembic/` **isn't in the wheel** and `reconcile_runtime_schema` is purely structural → wheel installs left every account unverified = latent lockout when an operator trusts our upgrade notes and flips `AINDY_REQUIRE_VERIFIED_LOGIN`. Fix: columns declare `info={"reconcile_backfill": "<sql>"}`; reconcile `UPDATE`s right after `ADD COLUMN` (no WHERE — the column didn't exist a moment before, so every present row predates it). Tripped the schema-contract protocol (`orm_hash` = content hash of `AINDY/db/models/`) → version `2026-08-05`, though **no DDL changed**. **FR-9 (#362):** runtime transactional mail dispatched to the **`email`** type apps register for automations, in a different undocumented shape; with the (correct) no-fallback rule that meant `/auth/register` 202 + **no account can complete signup**. Fix: reserved **`transactional_email`** type; shape published in `CONNECTOR_CONTRACT.md` §5a; failure now logs ERROR. **Three gotchas worth keeping:** (1) SQLAlchemy renders `server_default="false"` as `DEFAULT 'false'` — a *quoted string*; PG casts it to boolean false but **sqlite stores the characters and reads back truthy**, so a sqlite bool assertion tests type affinity, not your code. (2) `0014`'s re-run guard `created_at < now()` evaluates `now()` at execution time, so a re-run matches **every** row — the comment overstates the protection (flagged, not edited). (3) `send_email`'s "never raises" is **inherited** from `dispatch_connector` normalising handler exceptions, not enforced — mocking the dispatcher tests nothing. *(A stale "Next available: FR-11" sat here; FR-11/12/13 were filed 2026-08-06, so the live number is **FR-14**, stated once above. Corrected 2026-08-14.)* Source: `TECH_DEBT.md` APP-FR-*.
...
- **★ CI-MARKER-1 — a green PR does NOT mean `tests/unit` passed.** `Runtime Contracts` is the only unit-test job and runs `pytest tests -m runtime_only`; the marker is applied per-file as `pytestmark = pytest.mark.runtime_only` and **nothing applies it automatically** (`pytest.ini` only declares it; `tests/conftest.py` has no `pytest_collection_modifyitems`). So a new unit file defaults to **not run**, and the PR looks no different. The Integration job can't compensate — `pytest.integration.ini` sets `testpaths = tests/integration`. Measured 2026-08-14: `-m runtime_only` collects **1230/1579, deselecting 349**; **268 of those are `tests/unit` tests across 24 unmarked files** that run in *no* CI job (the rest are integration/sandbox, which have their own jobs). Not peripheral files: `test_syscall_contract.py` (the file this very doc uses as its Commands example), the FR-8/9/10 regressions (`test_reconcile_backfill`, `test_empty_env_typed_settings`, `test_transactional_email_isolation` — the defects that forced 2.0.1), `test_clock` (REPLAY-1), `test_background_leadership` (LEASE-1), all five `test_infinity_*`, `test_runtime_degraded_modes`. **Those fixes could be reverted with CI still green.** **★ CLOSED 2026-08-15 — both halves.** The 24 files carry the marker explicitly (**four never imported `pytest` at all**, which is its own evidence they were never exercised as pytest files), **and `tests/unit/conftest.py` now makes the default safe**: a `pytest_collection_modifyitems` hook applies `runtime_only` to every `tests/unit` item unless it already carries it or carries a marker handing it to another job (`integration`/`sandbox_escape`/`redis`/`mongo`/`multi_instance`). Opting out is still possible; it now takes a deliberate marker, not an omission. Collection **1587/1943 → 1855/1943, deselected 356 → 88** — the +268 predicted; the 88 left are integration/sandbox. Coverage went **UP to 56.71%**, so the `--cov-fail-under=35` re-baseline wasn't forced (still worth raising deliberately). **★ The guard is mutation-checked 3/3 and had to be:** a test that imported the conftest and called the hook with a fake item would keep passing after pytest stopped loading the hook at all, so `test_ci_marker_default.py` spawns a **real pytest subprocess** against generated probe files; its third test asserts the *complement* (`-m "not runtime_only"` over `tests/unit` must collect **nothing**). **★ Turning them on found exactly the predicted failure:** `test_infinity_async_job_loop`'s contract-gate test asserts an `execution.*` event raises **outside** a pipeline and *assumed* that precondition — `pipeline_active` is a ContextVar an earlier test leaves set, so it failed `DID NOT RAISE` in a full run and passed in isolation. Now sets/asserts/resets it in a `finally`, reproduced deterministically with a throwaway leaking probe. **Two gotchas:** `--collect-only -q` prints `<path>: <count>` **not node ids**; and exit code **5 is `EXIT_NOTESTSCOLLECTED`, not an error** — a naive `returncode == 0` inverts the complement test. Still-expected local red: `test_runtime_packaging` (`python -m build --no-isolation`), which is marked and passes in CI. Source: `TECH_DEBT.md` CI-MARKER-1.
- **FLAKY-1 — `test_platform_only_startup::test_platform_only_registers_runtime_agent_defaults` fails ~50% of full-suite runs, and it gates merges. ★ MITIGATION SHIPPED 2026-08-15 (FR-11 budget 10s→30s, configurable), but deliberately NOT closed — the mechanism was never confirmed, only forced, so this is a well-argued mitigation not a demonstrated fix. A traceback WAS captured 2026-08-15 (below) — it refuted the mechanism rather than closing the issue, so closing now needs either the *next* natural occurrence (which will finally be diagnosable) or ~10+ consecutive clean `-m runtime_only` runs on an otherwise idle machine (4 clean in a row happen ~6% of the time by luck against a 50% base rate).** Measured over six `tests/unit/` runs across two worktrees: `main` 1-fail/1-pass, feature branch 2-fail/2-pass. **Pre-existing, not branch-caused** — and two earlier readings of that same data were wrong, recorded so they aren't repeated: two consecutive failures read as *deterministic* (a third run refuted it), and correlation with a branch adding neighbouring files read as *caused by them* (a second `main` baseline refuted it). **Don't conclude anything here from fewer than ~4 runs per tree.** It passes 100% in isolation. It carries `runtime_only`, so `Runtime Contracts` runs it — and with all ten checks now required, a ~50% test blocks merges at random and trains people to re-run red. **★ Confirmed 2026-08-15 under `python -m pytest tests -m runtime_only -q` — byte-for-byte the `Runtime Contracts` command — so an earlier guess that the marker-filtered collection might dodge the ordering is WRONG.** The required check will red-line at random; it simply hasn't yet. (The same commit passed in CI minutes later — that's what a coin flip looks like, and why "it was green" proves nothing here.) **★★ CLOSED 2026-08-15 — fixed by FR-11's budget raise. 15 healthy runs across two trees, zero reproductions** (11 on `8c1d4ac` + **4 on current `main`** post-CI-MARKER-1 at 1878 collected — both trees were required, since main's ordering changed). ≈0.003% by luck against the ~50% base rate. Run durations 780–1194s, only recurring failure was the known local-only `test_runtime_packaging`; against a ~50% base rate that is ≈0.05% by luck. **★ CORRECTION: this bullet briefly said a captured traceback REFUTED the timeout mechanism. Over-conclusion from one sample, and the worst one available** — that run was on a machine that could not spawn processes at all (43 failures, `0xC0000142`), and a worker that cannot start writes empty stdout, which lands in the `ok:false` branch. Most likely it was **not FLAKY-1 at all** but process exhaustion wearing its clothes, and FR-11's timeout theory was right. The traceback, kept as record: The failure is **not** the timeout branch. It is `RuntimeError: runtime callback failed` raised at `runtime_callback_host.py:159` (the `if not response.get("ok")` branch), reaching the evaluator assertion at `test_platform_only_startup.py:233`. The timeout branch raises a *different*, explicit message (`"runtime callback command timed out after Ns"`), and the natural failure does not carry it. The old "strongly indicated" reading came from *forcing* a timeout and matching the shape — never from reading a real failure, which is exactly the trap this file warns about elsewhere. **Caveat, stated because it matters:** the reproducing run was on a machine that had lost the ability to spawn processes (43 failures, Windows `0xC0000142` `STATUS_DLL_INIT_FAILED`), so this proves the *branch*, not that every natural failure arrives the same way. **★ And the traceback explains why this was never diagnosable: that branch had NO diagnostic content.** A worker that dies before replying writes nothing; `json.loads(stdout or "{}")` turns empty stdout into `{}`; `{}.get("ok")` is falsy; the handler emits its default string. So "the subprocess never started" and "the callback returned `{ok: false}`" were **the same message** — with no exit code, no stderr, no callback name. Fixed: empty stdout is now its own error, every failure names `exit=<code>`, and the three modes are pinned distinct (`tests/unit/test_runtime_callback_diagnostics.py`, mutation-checked 4/8 against the old path). **Next natural occurrence will say what actually happened** — that is the step that can close this. **★ Why no natural traceback existed before: all 3 earlier failures ran as `pytest ... -q | tail`, which discarded the traceback** and kept only the summary; by the time runs switched to `--tb=long`-to-file the flake didn't recur (2 attempts, both clean). Tally then: 7 runs, 3 fail / 4 pass. **When chasing a flake, never pipe the run through `tail`.** Ruled out: stale `_runtime_callback_invocations` (the fixture restores 57 registry globals incl. that one). **Blast radius is two files** — `test_extension_ownership.py` asserts on the same boundary and is exposed to the same race. **★ Do NOT fix by dropping the marker — that's CI-MARKER-1 in reverse: green because it stopped running.** **Open.** Source: `TECH_DEBT.md` FLAKY-1.
- **MEM-EXPAND-DEAD-1** — **`expand()`'s semantic-neighbour half returns `[]` on every call and always has.** Not a logic error — a type mismatch: pgvector 0.4.2's SQLAlchemy column returns `numpy.ndarray`, and `_embedding_is_usable` (`memory_node_dao.py:126`) tests `isinstance(embedding, list)`, so the read-back embedding `expand()` feeds to `find_similar` (`:1395`) fails the guard at `:417`. Invisible because the skip logs at `debug` and the caller just gets an empty list; `include_similar` defaults **True** in all four sites (`:1359`, `memory_router.py:117`, `flow_definitions_memory.py:176`/`:247`). Same mismatch makes the fallback at `:483` (`list(ndarray or [])`) raise `ValueError: truth value of an array ... is ambiguous`, reachable only when the pgvector branch throws. **pgvector 0.5.0 returns `list` and fixes both — which is exactly why dependabot #390 was held and closed, not merged** (2026-08-14): taking it flips expansion on by default everywhere, and `expand()` calls `find_similar` *per node*, stacking on MEM-RECALL-N1-1's N+1 in the same path that caused RT-MEMTXN-LEAK-1's pool exhaustion. **★ Gotcha: widening the guard to accept any sequence is not the safe option it looks like — it activates the path on 0.4.2 too.** No test caught it in either direction (see DOCS-COVERAGE-CLAIM-1); a fix should land with one. **Open.** Source: `TECH_DEBT.md` MEM-EXPAND-DEAD-1.
...
| Latest app-team handoff (v2.1.0) | `docs/runtime/APP_HANDOFF_v2.1.0.md` |
