...
- **ORCHESTRATOR-SPLIT-1** — durable work state lives in **four** stores with four recovery paths and no shared transaction. **Do (b) first: publish the ownership contract** — the split may be correct, but undocumented it can be neither relied on nor reviewed. Failure is crash-only: a guest that rehydrated its own claim/wait state disagrees with the host, undetected. **★★ Store 4 is UNCONFIGURED and loses data: `AINDY/` has ZERO references, so it roots at the worker's CWD — which no spawn path sets — in a directory with NO compose volume.** It IS injectable (`runner.py:437`). **★★ 2026-09-09: 629 records in the repo root, and nodus 6.0.0 flips JSON→SQLite where neither store sees the other. DO NOT trust the migration as the mitigation — `migrate-store` carried 432 of 629 reporting `skipped: 0`, because `list_runs` silently drops files by MTIME past 30d, not status.** **★ The interim fix's `NODUS_WORKFLOW_STORE_ROOT` is the WRONG variable (half the state, and it escaped the Floor) — use `NODUS_RUN_STATE_ROOT`.**
