...
- **TEST-ISOLATION-REGISTRY-1** — **filed 2026-08-16.** `test_platform_only_registers_runtime_agent_defaults` FAILS under `pytest tests/unit -k "key or scope or auth or platform or route"` (`get_capability_definitions()` empty) and PASSES in the full run, alone, or paired — it broke only when one more `runtime_only_app`-booting file joined the selection. **Same defect class as the green-check catalogue with the sign flipped, which is worse not better: if outcome depends on what else ran, a REAL failure can be masked by full-suite ordering exactly as a spurious one was produced by subset ordering.** **Do NOT fix by pinning order** — that makes it reproducible without making it meaningful; the boot must *establish* the precondition (cf. the `pipeline_active` ContextVar leak). Source: `TECH_DEBT.md`.
